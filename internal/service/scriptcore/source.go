...
// ResearchSource is a single research item captured during source resolution.
// Mirrors scripts.ScriptResearchSource for the service layer.
...
// ToDB converts a ResearchSource to the repository type.
func (r ResearchSource) ToDB() scripts.ScriptResearchSource {
	return scripts.ScriptResearchSource{
		Query:          r.Query,
		URL:            r.URL,
		Title:          r.Title,
		Snippet:        r.Snippet,
...
func (r *SourceResolver) ResolveSource(ctx context.Context, topic, sourceText string, searchTimeout time.Duration) (*SourceResult, error) {
	...
	if r.search != nil {
		...
		for _, raw := range rawResults {
			result.RawSources = append(result.RawSources, ResearchSource{
				Query:      topic,
				URL:        raw.URL,
				Title:      raw.Title,
				Snippet:    raw.Content,
...
func (r *SourceResolver) SearchTopics(ctx context.Context, topics []string, timeout time.Duration, concurrency int) (map[string]string, []ResearchSource) {
