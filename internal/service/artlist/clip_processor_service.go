...
p.svc.log.Warn("clip indexing failed", zap.String("clip_id", clip.ClipID), zap.Error(err))
}
...
if clip.PrimaryURL == "" {
return fmt.Errorf("clip has no primary URL")
}

downloadDir := filepath.Join(p.svc.cfg.Harvester.DownloadDir, "artlist")
if err := os.MkdirAll(downloadDir, 0755); err != nil {
return fmt.Errorf("failed to create download dir: %w", err)
}

filename := fmt.Sprintf("%s.mp4", clip.ClipID)
...
cmd := exec.CommandContext(ctx, "yt-dlp", "-o", localPath, "--cookies-from-browser", "chrome", clip.PrimaryURL)
...
return p.svc.clipIndexer.IndexClip(ctx, clip)
}
