...
1.  **Google Vids image synthesis (Primary)**:
    - Calls the `google-accounting` Python server.
    - Supports generating images in parallel via the pooled Playwright session path.
    ...
    - `Google Flow` remains a separate endpoint for manual/specialized use, but it is not the primary path used by `GenerateSmartImage`.

2.  **NVIDIA Fallback (Secondary)**:
    - If Google Vids fails or returns zero images, the system automatically falls back to NVIDIA NIMs (e.g., `flux-1-dev`).
...
- `POST /generate-vids-images`: Starts the job.
- `GET /status/{job_id}`: Polls for completion.
- Models are defined in `pkg/googleaccounting/models.go`.
