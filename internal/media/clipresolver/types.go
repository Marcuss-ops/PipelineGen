...
type OntologyScorer interface {
	Apply(score float64, clip *assets.Asset, topic string) float64
}
...
type RecommendedClip struct {
	...
	// LLM-enhanced evaluation fields
	LLMReason string `json:"llm_reason,omitempty"` // Natural language justification from LLM
	LLMRating string `json:"llm_rating,omitempty"` // HIGH / MEDIUM / LOW relevance rating
}
