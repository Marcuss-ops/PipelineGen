...
func (s *Service) Recommend(ctx context.Context, req *RecommendRequest) (*RecommendResponse, error) {
	...
	for _, wq := range weightedQueries {
		...
		for source, repo := range s.repos {
			...
			if s.vectorStore != nil {
				...
				if ok && len(emb) > 0 {
					emb32 := textutil.Float64To32(emb)
						...
						if err == nil && len(results) > 0 {
							for _, r := range results {
								...
								if clipScores[globalID] == nil {
									...
									clipScores[globalID].Clip.MediaType = asset.MediaType(source)
								}
	...
	// ── LLM Decision Layer ──────────────────────────────────────────────
	// After standard scoring + filtering, let the LLM evaluate the top-K
	// candidates and pick the best one with a human-readable justification.
	if s.llmDecision != nil && s.llmDecision.IsEnabled() && len(resp.Recommended) > 0 {
		topK := s.llmDecision.cfg.TopK
		if topK <= 0 {
			topK = 5
		}

		result, err := s.llmDecision.EvaluateCandidates(ctx, req, topK, resp.Recommended)
		if err == nil && result != nil {
			// The LLM selected a clip. If it's different from the top-ranked one,
			// log the re-ranking decision but still pass through all candidates.
			for i := range resp.Recommended {
				if resp.Recommended[i].ClipID == result.ChosenClipID {
					// Mark the chosen clip with LLM-enhanced reason and rating
					resp.Recommended[i].LLMReason = result.Reason
					resp.Recommended[i].LLMRating = result.Rating
					break
				}
			}

			// If the LLM chose a different clip, promote it to the top of the list
			// but keep the rest of the order intact.
			if len(resp.Recommended) > 1 && resp.Recommended[0].ClipID != result.ChosenClipID {
				for i, c := range resp.Recommended {
					if c.ClipID == result.ChosenClipID {
						// Move chosen clip to front by swapping
						resp.Recommended[0], resp.Recommended[i] = resp.Recommended[i], resp.Recommended[0]
						break
					}
				}
			}
	...
	// ── End LLM Decision Layer ───────────────────────────────────────────

	// ── Auto-Harvest ──────────────────────────────────────────────────────
	// Use HarvestPolicy to decide whether to trigger background harvesting.
	// Policy checks: no clips found, not enough high-quality clips, or
	// top clip score below threshold.
	harvestPolicy := DefaultHarvestPolicy()
	if harvestPolicy.ShouldHarvest(resp) {
		...
		if req.AutoHarvest && s.harvestSvc != nil {
			// Enqueue Artlist harvest jobs for each term
