...
func (m *mockEmbedder) EmbedTextWithNormalized(ctx context.Context, text string) ([]float64, string, error) {
	return m.embedding, text, m.err
}
...
func newTestService(t *testing.T, store *mockVectorStore, embedder EmbeddingClient, jobSvc *mockJobService, minScore float64) *Service {
	t.Helper()
	log := zap.NewNop()
...
// ── Additional Semantic Search & Hybrid Search Tests ──

func TestEmbedTextWithNormalizedReturnsEmbeddingAndNormalizedText(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, "/embed", r.URL.Path)
		w.Header().Set("Content-Type", "application/json")
		resp := map[string]any{
			"embedding":       []float64{0.1, 0.2, 0.3},
			"normalized_text": "run park",
		}
		json.NewEncoder(w).Encode(resp)
	}))
	defer server.Close()
	...
	adapter := NewPythonEmbeddingAdapter(server.URL)
	emb, norm, err := adapter.EmbedTextWithNormalized(context.Background(), "running in the park")
	require.NoError(t, err)
	assert.Equal(t, []float64{0.1, 0.2, 0.3}, emb)
	assert.Equal(t, "run park", norm)
}

type mockEmbedderWithNormalized struct {
	mockEmbedder
	normalized string
}

func (m *mockEmbedderWithNormalized) EmbedTextWithNormalized(ctx context.Context, text string) ([]float64, string, error) {
	return m.embedding, m.normalized, m.err
}
...
func TestHybridSearchUsesNormalizedQueryForBM25(t *testing.T) {
	...
	embedder := &mockEmbedderWithNormalized{
		mockEmbedder: mockEmbedder{embedding: makeEmbedding(384)},
		normalized:   "run park",
	}
	jobSvc := &mockJobService{}
	svc := newTestService(t, store, embedder, jobSvc, 0.85)
	...
	resp, err := svc.Match(context.Background(), &MatchRequest{
		Query: "running in the park", Mode: "text", MinScore: 0.85,
	})
	require.NoError(t, err)
	assert.True(t, resp.OK)
	assert.Equal(t, "instant_match", resp.Status)
	assert.Equal(t, "artlist_bm25", resp.Asset.ID)
}

func TestSearchFallsBackWhenEmbeddingServerUnavailable(t *testing.T) {
	// Adapter pointing to bad address
	adapter := NewPythonEmbeddingAdapter("http://127.0.0.1:1")
	store := &mockVectorStore{}
	...
	resp, err := svc.Match(context.Background(), &MatchRequest{
		Query: "testing fallback", Mode: "text", MinScore: 0.85, AllowBackgroundGen: true,
	})
	require.NoError(t, err)
	assert.Equal(t, "embedding_failed", resp.Status)
}
...
func TestSearchReturnsRRFResultsWhenRerankerUnavailable(t *testing.T) {
	...
	embedder := &mockEmbedder{embedding: makeEmbedding(384)}
	// Reranker is enabled but backend server is offline/unavailable
	rerankCfg := config.RerankerConfig{
		Enabled: true,
		URL:     "http://127.0.0.1:1/rerank",
	}
	client := reranker.NewClient(reranker.Config{
		Enabled: true,
		URL:     "http://127.0.0.1:1/rerank",
	})
	svc := NewService(
	...
	resp, err := svc.Match(context.Background(), &MatchRequest{
		Query: "test", Mode: "text", MinScore: 0.85,
	})
	require.NoError(t, err)
	// Must not crash, should fallback to Qdrant order
	assert.Equal(t, "instant_match", resp.Status)
	assert.Equal(t, "artlist_rrf", resp.Asset.ID)
}
...
func TestSearchHandlesMultilingualQuery(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		resp := map[string]any{
			"embedding":       []float64{0.1, 0.2, 0.3},
			"normalized_text": "correre parco",
		}
		json.NewEncoder(w).Encode(resp)
	}))
	defer server.Close()
	...
	adapter := NewPythonEmbeddingAdapter(server.URL)
	emb, norm, err := adapter.EmbedTextWithNormalized(context.Background(), "correndo nel parco")
	require.NoError(t, err)
	assert.Equal(t, []float64{0.1, 0.2, 0.3}, emb)
