...
func FromEnv() *Config {
	...
	// VELOX_SECRETS_DIR overrides the default location
	...
	// Set defaults based on DataDir if not explicitly configured
	if c.SecretsDir == "" && c.DataDir != "" {
		c.SecretsDir = c.DataDir + "/secrets"
	}
	...
	if c.DriveTokensDir == "" {
		...
		if c.DataDir != "" {
			driveTokenCandidates = append(driveTokenCandidates, c.DataDir+"/drive/tokens")
		}
		if c.SecretsDir != "" {
			driveTokenCandidates = append(driveTokenCandidates, c.SecretsDir+"/drive/tokens")
		}
		c.DriveTokensDir = firstExistingDir(driveTokenCandidates)
		if c.DriveTokensDir == "" {
			if c.SecretsDir != "" {
				c.DriveTokensDir = c.SecretsDir + "/drive/tokens"
			} else if c.DataDir != "" {
				c.DriveTokensDir = c.DataDir + "/drive/tokens"
			}
		}
	}
	...
	if c.DriveCredentialsDir == "" {
		...
		if c.DataDir != "" {
			driveCredCandidates = append(driveCredCandidates, c.DataDir+"/drive/credentials")
		}
		if c.SecretsDir != "" {
			driveCredCandidates = append(driveCredCandidates, c.SecretsDir+"/drive/credentials")
		}
		c.DriveCredentialsDir = firstExistingDir(driveCredCandidates)
		if c.DriveCredentialsDir == "" && c.SecretsDir != "" {
			c.DriveCredentialsDir = c.SecretsDir + "/drive/credentials"
		}
	}
	...
	if c.YouTubeTokensDir == "" {
		if c.SecretsDir != "" {
			c.YouTubeTokensDir = c.SecretsDir + "/youtube/tokens"
		} else if c.DataDir != "" {
			c.YouTubeTokensDir = c.DataDir + "/youtube/tokens"
		}
	}

	// YouTube credentials: prefer explicit, then secrets dir
	if c.YouTubeCredentialsDir == "" && c.SecretsDir != "" {
		c.YouTubeCredentialsDir = c.SecretsDir + "/youtube/credentials"
	}
