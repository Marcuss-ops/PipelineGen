...
type Store struct {
	...
	treeService   any // *media.assettree.Service â€” accepted as `any` to break the import cycle
...
// Arguments:
//
//	resolver         â€” path-only Resolver (for ResolveDest)
//	driveUploader    â€” concrete uploader; may be nil in tests
//	rootFolder       â€” Drive root folder ID
//	imagesFolder     â€” Drive images sub-folder (or "")
//	videoAIRoot      â€” Drive video-AI sub-folder (or "")
//	soundEffectsRoot â€” Drive sound-effects sub-folder (or "")
//	log              â€” zap logger
func NewStore(
	resolver *Resolver,
	driveUploader *Uploader,
...
// avoid importing the assettree package directly â€” assettree is being
...
//
//  1. audio/sfx request    â†’ soundEffectsRoot (if non-empty)
//  2. image/video request  â†’ videoAIRoot or imagesFolder (style-aware)
//  3. fallthrough          â†’ rootFolder
//
...
// Behaviour contract:
//
//   - Store nil  â†’ silent no-op (`"", "", nil`). Callers explicitly nil-check
//     the receiver to skip uploads in offline / test paths.
//   - driveUploader nil â†’ silent no-op when no Drive client has been wired.
//     This matches the historical contract â€” the legacy wiring builds a
//     Store with no uploader when the Drive flow is disabled, and callers
//     expect empty IDs rather than an error.
//   - real Drive client â†’ call EnsureDriveFolder, derive filename from
...
func (s *Store) UploadToDrive(ctx context.Context, req AssetDestinationRequest, filePath string) (string, string, error) {
	...
	if result == nil {
		// drive.Uploader returned no error AND no result â€” treat as silent
...
// do NOT synthesise a fallback from reqExt â€” generating "upload"+ext
...
// ResolveDest is the path-only side of Resolve â€” it MUST NOT call Drive.
// Falls through to Resolver.Resolve when wired; returns empty struct
// when no resolver is configured (callers must handle that â€” they already
