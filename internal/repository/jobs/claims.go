...
// Implements job.Repository.ClaimNext.
func (r *Repository) ClaimNext(ctx context.Context, workerID string, leaseTTL time.Duration, types []string) (*job.Job, error) {
	r.claimMu.Lock()
...
func (r *Repository) Start(ctx context.Context, cmd StartJob) (*job.Job, error) {
	now := time.Now().UTC()
...
func (r *Repository) RenewLease(ctx context.Context, id string, workerID string, leaseTTL time.Duration) error {
