// Package searchqueries provides persistence for scheduled YouTube topic searches.
...
// NewRepository creates a new search queries repository.
...
func (r *Repository) Upsert(ctx context.Context, q *models.SearchQuery) error {
...
func (r *Repository) ListAll(ctx context.Context) ([]*models.SearchQuery, error) {
...
func (r *Repository) ListActive(ctx context.Context) ([]*models.SearchQuery, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT id, query, category, drive_folder_id, min_score, max_results,
			check_interval, last_run_at, last_video_published_at, is_active,
			created_at, updated_at
...
func (r *Repository) GetByID(ctx context.Context, id string) (*models.SearchQuery, error) {
...
func (r *Repository) UpdateLastRun(ctx context.Context, id string, lastRun, lastPublishedAt string) error {
	_, err := r.db.ExecContext(ctx, `
...
func (r *Repository) InsertResult(ctx context.Context, res *models.SearchQueryResult) error {
	_, err := r.db.ExecContext(ctx, `
...
func (r *Repository) IsVideoProcessed(ctx context.Context, videoID string) (bool, error) {
	var count int
...
func (r *Repository) ListResultsByQuery(ctx context.Context, queryID string) ([]*models.SearchQueryResult, error) {
	rows, err := r.db.QueryContext(ctx, `
...
func scanQueries(rows *sql.Rows) ([]*models.SearchQuery, error) {
	var results []*models.SearchQuery
	for rows.Next() {
...
func scanQuery(row *sql.Row) (*models.SearchQuery, error) {
...
func scanResults(rows *sql.Rows) ([]*models.SearchQueryResult, error) {
	var results []*models.SearchQueryResult
	for rows.Next() {
...
func scanResult(scanner interface {
	Scan(dest ...any) error
}) (*models.SearchQueryResult, error) {
	r := &models.SearchQueryResult{}
