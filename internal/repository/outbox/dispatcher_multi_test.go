...
func (f *fakeClipsUpserter) UpsertClipTx(ctx context.Context, tx *sql.Tx, clip *assets.Asset) error {
	if f.err != nil {
		return f.err
	}
	if clip != nil {
...
func TestMultiClipsUpserterRoutesBySource(t *testing.T) {
	...
	require.NoError(t, m.UpsertClipTx(ctx, nil, &assets.Asset{ID: "yt_a", Source: "youtube"}))
	require.NoError(t, m.UpsertClipTx(ctx, nil, &assets.Asset{ID: "stock_b", Source: "stock"}))
	require.NoError(t, m.UpsertClipTx(ctx, nil, &assets.Asset{ID: "artlist_c", Source: "artlist"}))
	require.NoError(t, m.UpsertClipTx(ctx, nil, &assets.Asset{ID: "unknown_d", Source: "something_else"}))
...
func TestMultiClipsUpserter_NoDefault(t *testing.T) {
	...
	err := m.UpsertClipTx(context.Background(), nil, &assets.Asset{ID: "x", Source: "unknown"})
	require.Error(t, err)
...
func TestMultiClipsUpserter_NilReceiverErrors(t *testing.T) {
	...
	err := m.UpsertClipTx(context.Background(), nil, &assets.Asset{ID: "x", Source: "youtube"})
	require.Error(t, err)
}
