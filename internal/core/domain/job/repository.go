...
// Implementations live in the infrastructure layer (SQLiteJobRepository).
//
// All worker-originated operations (Complete, Fail, ScheduleRetry)
// REQUIRE job_id, worker_id, lease_id, and expected_revision so the
// repository can fence stale operations: a completion with an expired
// lease returns ErrLeaseLost.
