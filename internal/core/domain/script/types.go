...
//
// Scripts are the central artifact of the PipelineGen system: an AI-generated
// narrative text (with sections, metadata, and stock matches) that drives
// video production from clips, images, and voiceovers.
//
// The canonical repository interface is defined in repository.go; the
// concrete SQLite implementation lives in internal/repository/scripts.
//
// Design note: Timestamps use time.Time (proper Go types) rather than the
// string-based storage format of the concrete scripts.ScriptRecord. This
// means the adapter layer (when implemented) will pay a parse/format cost
// on every read/write — the trade-off is type safety and domain purity
// over raw I/O performance. Scripts are read orders-of-magnitude less
// frequently than clips, so this cost is acceptable.
package script
...
// ResearchSource represents a web/youtube/transcript source used during
// script generation.
