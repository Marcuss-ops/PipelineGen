// Package asset — asset.Asset projection scanner (Wave C / Phase 3 slim).
//
// Phase 3 (Wave C / Blocco 1 asset.Asset SSOT, June 2026): the previous
...
// scanMediaAsset scans a canonical asset.Asset directly from any SQL
...
func scanMediaAsset(s mediaAssetScanner) (*asset.Asset, error) {
	...
	a.ID = id
	a.Source = asset.Source(sourceStr)
	a.Name = name
	if duration != 0 {
		a.Duration = time.Duration(duration) * time.Millisecond
	}
	a.SourceURL = urlStr
	a.MediaType = asset.MediaType(mediaType)
	...
	if lifecycle != "" {
		a.LifecycleState = asset.LifecycleState(lifecycle)
	}
...
func scanCanonicalAssetRows(rows mediaAssetScanner) (*asset.Asset, error) {
