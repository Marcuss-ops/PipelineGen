...
// ClassifyImageOrigin returns the canonical asset.ImageOrigin for a media
...
// ClassifyImageProvider returns the canonical asset.ImageProvider for a media
...
// tags.
func (b *CanonicalImageMetadataBuilder) WithExtra(extra asset.ImageMetadataMap) *CanonicalImageMetadataBuilder {
	if extra == nil {
		return b
	}
	for k, v := range extra {
...
func AppendImageProvenance(metadataJSON, imageURL, pageURL, sourceName, query string) string {
	...
	var payload asset.ImageMetadataMap
