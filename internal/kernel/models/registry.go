...
type Model struct {
	...
	Enabled bool
	// Languages is the number of languages the model supports (ASR models
	// only; 0 = unknown or not applicable). Whisper large-v3-turbo covers
	// 99 languages.
	Languages int
}
...
func (m Model) Validate() error {
	...
	if m.Languages < 0 {
		return fmt.Errorf("models: %s has negative languages=%d", m.ID, m.Languages)
	}
	return nil
}
