...
func voiceoverPlaceholderHandler(ctx context.Context, j *domainjob.Job, tools *appjobs.JobTools) (map[string]any, error) {
	_ = ctx
	_ = tools
...
func registerVoiceoverGenerateItemPlaceholder(dispatcher *appjobs.Dispatcher) error {
	...
	if err := dispatcher.Register(domainjob.TypeVoiceoverGenerateItem, voiceoverPlaceholderHandler); err != nil {
		return fmt.Errorf("registerVoiceoverGenerateItemPlaceholder: bind %q to dispatcher: %w",
			domainjob.TypeVoiceoverGenerateItem, err)
	}
	return nil
}
