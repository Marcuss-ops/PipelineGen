...
// clipsVectorAdapter wraps *qdrant.Service to satisfy
// clips.VectorStorePort. UpsertAsset converts the narrowed DTO into
// the concrete qdrant.VectorAsset, so the use case (enrich.go) can
// build everything from domain types without importing
// internal/infrastructure/qdrant.
type clipsVectorAdapter struct {
	inner *qdrant.Service
}

// Compile-time assertion: clipsVectorAdapter satisfies clips.VectorStorePort.
var _ clips.VectorStorePort = (*clipsVectorAdapter)(nil)

func newClipsVectorAdapter(svc *qdrant.Service) clips.VectorStorePort {
	if svc == nil {
		return nil
	}
	return &clipsVectorAdapter{inner: svc}
}

func (a *clipsVectorAdapter) UpsertAsset(ctx context.Context, dto clips.ClipVectorAssetDTO) error {
	if a.inner == nil {
		return fmt.Errorf("clipsVectorAdapter: vector service not wired")
	}
	...
	return a.inner.UpsertAsset(ctx, va)
}
...
type clipsAdapterBundle struct {
	...
	VectorStore    clips.VectorStorePort
}
...
func newClipsAdapterBundle(
	...
	return clipsAdapterBundle{
		...
		VectorStore:    newClipsVectorAdapter(vectorSvc),
	}
}
