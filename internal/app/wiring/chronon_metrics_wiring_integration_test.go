...
// chronon_metrics_wiring_integration_test.go verifies the composition-root
// seam: wireChrononMetricsAdapter builds the Chronon Metrics Adapter over the
// primary SQLite DB (performance_operations lives in the primary database),
// and a publish driven through the adapter lands real rows in that DB during
// a run-bound execution — the automatic per-run persistence guarantee.
...
// chrononMetricsWiringSchema is the canonical performance_operations DDL
// (migration 217) needed by this integration test.
...
func TestWireChrononMetricsAdapterNilDBIsNil(t *testing.T) {
	if got := wireChrononMetricsAdapter(nil, zap.NewNop()); got != nil {
		t.Fatalf("wireChrononMetricsAdapter(nil DB) = non-nil, want nil")
	}
}

func TestWireChrononMetricsAdapterPersistsDuringARun(t *testing.T) {
	...
	if count != 7 {
		t.Fatalf("persisted %d rows, want 7 (one per measured exclusive-wall phase)", count)
	}
	var startupMS int64
