...
//
// Extracted from internal/app/go per AGENTS.md Pattern 5.
...
//
//   - outboxMonitorAdapter        (jobs.MonitorPort ← outboxevents.Repository)
...
// jobs.MonitorPort (application-layer) is satisfied.
type outboxMonitorAdapter struct {
	repo *outboxevents.Repository
}

var _ jobs.MonitorPort = (*outboxMonitorAdapter)(nil)

func newOutboxMonitorAdapter(repo *outboxevents.Repository) jobs.MonitorPort {
	if repo == nil {
		return nil
	}
...
func (a *outboxMonitorAdapter) ListPending(ctx context.Context) ([]jobs.EventDTO, error) {
	if a == nil || a.repo == nil {
		return nil, nil
	}
	...
	dtos := make([]jobs.EventDTO, len(events))
...
func (a *outboxMonitorAdapter) ListByStatus(ctx context.Context, status string) ([]jobs.EventDTO, error) {
	if a == nil || a.repo == nil {
		return nil, nil
	}
...
func eventToDTO(e outboxevents.Event) jobs.EventDTO {
	return jobs.EventDTO{
		ID:            e.ID,
