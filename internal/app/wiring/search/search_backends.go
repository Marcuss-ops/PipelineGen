// Package search — search_backends.go is the Wave 21 canonical
...
// zapSearchLogAdapter adapts *zap.Logger to search.Logger (the leaf-local
// aggregator logger port). Moved from the root assets_adapters.go with the
// search split; the root has no other consumer.
