...
func WireRegistry(ctx context.Context, cfg *config.Config, log *zap.Logger, coreDeps *CoreDeps) (*RegistryWiring, error) {
	...
	if coreDeps.DB != nil && coreDeps.DB.DB != nil {
		registerModule(registry, log, channelsapi.NewModule(log, channelsrepo.NewRepository(coreDeps.DB.DB)))
		registerModule(registry, log, sourcesapi.NewSearchQueriesModule(log, searchqueriesrepo.NewRepository(coreDeps.DB.DB)))
	}
...
func wireScriptFlowExtras(handler *scriptapi.ScriptFlowHandler, ollamaClient *client.Client, vectorStore *vectorstore.Service, clipsOnlyRepo *clips.Repository, engine *scriptcore.Engine, cfg *config.Config, log *zap.Logger) {
	if ollamaClient == nil {
		return
	}
	...
	if (vectorStore != nil || clipsOnlyRepo != nil) && engine != nil {
		handler.SetMediaCurator(mediacurator.NewService(vectorStore, cfg.ClipIndexer.ServerURL, clipsOnlyRepo, clipSourceBuilder, engine, log))
	}
}
...
func buildSyncTargets(cfg *config.Config, clipsOnlyRepo *clips.Repository, clipsRepo *clips.Repository, artlistRepo *clips.Repository) []catalogsync.Target {
