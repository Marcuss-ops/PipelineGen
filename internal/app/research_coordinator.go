...
// ── Resolver-port adapter ─────────────────────────────────────────────
// The resolver defines a narrow port (researchSearchCoordinatorPort)
// that the coordinator satisfies via this adapter. The adapter converts
// between the app-level SearchResult and the resolver-level
// CoordinatorSearchResult, keeping the usecase package free of app
// imports.

// coordinatorAdapter wraps a ResearchSearchCoordinator to satisfy the
// researchSearchCoordinatorPort defined in the usecase package.
type coordinatorAdapter struct {
	coordinator *ResearchSearchCoordinator
}

func (a *coordinatorAdapter) SearchWithFallback(ctx context.Context, subject string, queries []string, targetPool int) []usecase.CoordinatorSearchResult {
