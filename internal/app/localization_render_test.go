...
func TestLocalizationRenderPlanExecutor_ExecutesViaClipRender(t *testing.T) {
	...
	srcSHA, err := files.SHA256File(srcPath)
	if err != nil {
		t.Fatal(err)
	}
	...
	wantSHA, err := files.SHA256File(outPath)
	if err != nil {
		t.Fatal(err)
	}
	...
	if got.Output.ContractID != cliprender.OutputContractVeloxEditingClipV1 || got.Output.Width != 1920 || got.Output.Height != 1080 || got.Output.FPS != 30 || got.Output.VideoCodec != "h264" {
