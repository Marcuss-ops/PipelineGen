...
//
// PR-2 (June 2026): BuildSearchBackends now also constructs the
// canonical search.SearchFanOut — the telemetry decorator that
// wraps the aggregator and exposes the user-spec
// Option{Hits, Latencies} Stats surface. The legacy
// providers.SearchAggregator is gone (git-rm'd in this PR); all
// callers consume SearchFanOut via the AssetsWiring struct.
package app
...
func (b *semanticSearchBackend) Search(ctx context.Context, q search.Query) ([]search.Candidate, error) {
	// PR-2 (June 2026): semantic backend ignores Query.Hash.
	// The hash-match path is in-domain for the local catalog;
	// the semantic vector store has no equivalent concept.
	if q.Hash != "" {
		return nil, nil
	}
