...
func TestSearchPixabayVideosUsesConfiguredBaseURL(t *testing.T) {
	...
	p := NewPixabayProvider(cfg.External.PixabayAPIKey, cfg.External.PixabayBaseURL)

	clips, err := p.Search(context.Background(), "forest trail", 3)
	if err != nil {
		t.Fatalf("pixabayProvider.Search returned error: %v", err)
	}
	if len(clips) != 1 {
...
func TestSearchPexelsVideosUsesConfiguredBaseURL(t *testing.T) {
	...
	p := NewPexelsProvider(cfg.External.PexelsAPIKey, cfg.External.PexelsBaseURL)

	clips, err := p.Search(context.Background(), "city night", 2)
	if err != nil {
		t.Fatalf("pexelsProvider.Search returned error: %v", err)
	}
	if len(clips) != 1 {
		t.Fatalf("expected 1 clip, got %d", len(clips))
	}
...
func TestSearchLiveFallsBackToPixabayWhenArtlistUnavailable(t *testing.T) {
	...
	chain := NewFallbackChain(NewPixabayProvider(cfg.External.PixabayAPIKey, cfg.External.PixabayBaseURL))

	clips, err := chain.Search(context.Background(), "waterfall", 1)
	if err != nil {
		t.Fatalf("FallbackChain.Search returned error: %v", err)
	}
	if len(clips) != 1 {
		t.Fatalf("expected 1 clip, got %d", len(clips))
	}
