...
func TestRootFolderNameFallbacks(t *testing.T) {
	...
	got = RootFolderName(&RunInput{SearchQueries: []string{"boxing training"}})
	if got != "boxing training" {
		t.Fatalf("RootFolderName() query fallback = %q, want boxing training", got)
	}

	got = RootFolderName(&RunInput{DirectURLs: []string{"https://example.test/video/Some_Movie.mp4?v=1#clip"}})
	if got != "Some_Movie" {
...
func TestResolvedFolderIDNeverUsesWorkflowID(t *testing.T) {
	if got := ResolvedFolderID(&RunInput{FolderID: "workflow-only"}); got != "" {
		t.Fatalf("ResolvedFolderID() = %q, want empty for workflow-only ID", got)
	}
	if got := ResolvedFolderID(&RunInput{DriveFolderID: "drive-folder", DriveFolderResolved: false}); got != "" {
		t.Fatalf("ResolvedFolderID() = %q, want empty for unverified Drive ID", got)
	}
	if got := ResolvedFolderID(&RunInput{DriveFolderID: " drive-folder ", DriveFolderResolved: true}); got != "drive-folder" {
...
func TestExplicitClipNamingPreservesExistingFallbacks(t *testing.T) {
	...
	plan = ClipPlan{StartSec: 32, EndSec: 51}
	if got := PerClipLeafName(plan); got != "00-00-32_to_00-00-51" {
		t.Fatalf("PerClipLeafName() empty fallback = %q, want timestamp literal", got)
	}

	in := &RunInput{Subfolder: "root/Round_7/00-00-32_to_00-01-27"}
	if got := TimestampParentGroupName(in); got != "Round_7" {
		t.Fatalf("TimestampParentGroupName() = %q, want Round_7", got)
	}

	if got := ClipFolderName(&RunInput{}, ClipPlan{Round: 7}, "metadata"); got != "Round 7" {
		t.Fatalf("ClipFolderName() round = %q, want Round 7", got)
	}
	if got := ClipFolderName(&RunInput{Subfolder: "root/parent/child"}, ClipPlan{Round: 7}, "parent"); got != "parent" {
		t.Fatalf("ClipFolderName() explicit subfolder = %q, want parent", got)
	}
	if got := TimestampParentLeafName(ClipPlan{ParentSlug: "parent_slug", Title: "ignored"}); got != "parent_slug" {
