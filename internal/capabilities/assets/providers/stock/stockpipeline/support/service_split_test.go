...
// home of the 4 constructor-input types: stockpipeline.PipelineConfig,
// stockpipeline.StorageDeps, stockpipeline.MediaDeps, stockpipeline.Deps. They MUST be declared in
// service_types.go (not service.go / service_errors.go).
func TestSplit_ConstructorInputTypesLiveInServiceTypes(t *testing.T) {
	// We probe the types via reflect — a future "moved stockpipeline.Deps back to
	...
	if reflect.TypeOf(stockpipeline.PipelineConfig{}).Name() != "stockpipeline.PipelineConfig" {
		t.Errorf("stockpipeline.PipelineConfig type identity lost")
	}
	if reflect.TypeOf(stockpipeline.StorageDeps{}).Name() != "stockpipeline.StorageDeps" {
		t.Errorf("stockpipeline.StorageDeps type identity lost")
	}
	if reflect.TypeOf(stockpipeline.MediaDeps{}).Name() != "stockpipeline.MediaDeps" {
		t.Errorf("stockpipeline.MediaDeps type identity lost")
	}
	if reflect.TypeOf(stockpipeline.Deps{}).Name() != "stockpipeline.Deps" {
		t.Errorf("stockpipeline.Deps type identity lost")
	}
	// stockpipeline.DefaultPipelineConfig lives in service_types.go too.
	if got := stockpipeline.DefaultPipelineConfig(); got.ChunkDuration != 25 || got.MaxResults != 25 || got.EffectInterval != 4 || got.EffectsDir != "assets/effects/EffettiVisiv" {
		t.Errorf("stockpipeline.DefaultPipelineConfig drifted: %+v", got)
	}
}

// TestSplit_NewProductionStockPipelineSurfacesTypedSentinelForMissingCfg pins the
// canonical fail-closed contract: stockpipeline.NewProductionStockPipeline(deps) with deps.Cfg ==
// nil MUST return (nil, ErrStockPipelineNilCfg) verbatim (no
// wrapping, no fake-availability fallthrough). The test runs the
// minimum stockpipeline.Deps literal that the constructor accepts as the nil-only
...
func TestSplit_NewProductionStockPipelineSurfacesTypedSentinelForMissingCfg(t *testing.T) {
	_, err := stockpipeline.NewProductionStockPipeline(stockpipeline.Deps{})
	if err == nil {
		t.Fatalf("stockpipeline.NewProductionStockPipeline(stockpipeline.Deps{}) with nil Cfg: expected ErrStockPipelineNilCfg, got nil")
	}
	if !errors.Is(err, ErrStockPipelineNilCfg) {
		t.Errorf("stockpipeline.NewProductionStockPipeline(stockpipeline.Deps{}) err = %v; want errors.Is(err, ErrStockPipelineNilCfg) == true", err)
	}
}
...
// stockpipeline.NewProductionStockPipeline MUST surface the EARLIEST missing dep as a typed
...
func TestSplit_NewProductionStockPipelineSurfacesTypedSentinelForFirstMissingDep(t *testing.T) {
	cfg := &stockpipeline.RuntimeConfig{WorkDir: t.TempDir(), ClipDurationSec: 5, ChunkDurationSec: 25, MaxResults: 25, PolicyVersion: "test"}
	deps := stockpipeline.Deps{
	...
	_, err := stockpipeline.NewProductionStockPipeline(deps)
	if err == nil {
		t.Fatalf("stockpipeline.NewProductionStockPipeline(deps) with nil Storage.ClipsRepo: expected ErrStockPipelineNilClipsRepo, got nil")
	}
	if !errors.Is(err, ErrStockPipelineNilClipsRepo) {
		t.Errorf("stockpipeline.NewProductionStockPipeline(deps) err = %v; want errors.Is(err, ErrStockPipelineNilClipsRepo) == true (validation ladder short-circuits at the earliest missing dep)", err)
	}
}
