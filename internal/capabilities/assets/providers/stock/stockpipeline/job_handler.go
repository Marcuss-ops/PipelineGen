...
func (s *Service) HandleJob(ctx context.Context, queuedJob *appjobs.Job, tools *appjobs.JobTools) (map[string]any, error) {
	...
	s.log.Info("stock job payload received",
		...
		zap.Int("known_source_durations", len(payload.SourceDurations)),
	...
	input := &RunInput{
		...
		SourceDurations:                copySourceDurations(payload.SourceDurations),
		...
		TargetDurationPerSourceSeconds: payload.TargetDurationPerSourceSeconds,
		ClipsPerSource:                 payload.ClipsPerSource,
		ClipDurationSeconds:            payload.ClipDurationSeconds,
