...
//
//	plan.HasPostprocessor(string(adapters.ProcessorVoiceover)) → emit a
//	    DownstreamRequest{Kind: DownstreamVoiceover, ...} entry
//	    with the canonical VoiceoverRequirements sub-struct.
//	plan.HasPostprocessor(string(adapters.ProcessorImages))    → emit a
...
//     processor_names.go (adapters.ProcessorVoiceover, adapters.ProcessorImages).
...
func buildManifestV2(plan *scriptpkg.ResolvedGenerationPlan, _ adapters.ProcessInput) *scriptpkg.ManifestV2 {
	...
	if plan.HasPostprocessor(string(adapters.ProcessorVoiceover)) {
	...
	if plan.HasPostprocessor(string(adapters.ProcessorImages)) {
