...
func TestVoiceoverProcessor_Policy_RequiredTimingIsRequired(t *testing.T) {
	...
	require.Equal(t, adapters.ProcessorRequired, proc.Policy(&scriptpkg.ResolvedGenerationPlan{
		Timing: &audio.TimingRequest{Mode: audio.TimingRequired},
	}))
	require.Equal(t, adapters.ProcessorBestEffort, proc.Policy(&scriptpkg.ResolvedGenerationPlan{
		Timing: &audio.TimingRequest{Mode: audio.TimingBestEffort},
	}))
	require.Equal(t, adapters.ProcessorBestEffort, proc.Policy(&scriptpkg.ResolvedGenerationPlan{}))
	require.Equal(t, adapters.ProcessorBestEffort, proc.Policy(nil))
}
...
func TestVoiceoverProcessor_RequiredTiming_FailureFailsJob(t *testing.T) {
	...
	_, err := proc.Process(context.Background(), requiredTimingPlan(), adapters.ProcessInput{
		Text:      "Generated body.",
...
func TestVoiceoverProcessor_BestEffortTiming_FailureWarnsNotFails(t *testing.T) {
	...
	result, err := proc.Process(context.Background(), plan, adapters.ProcessInput{
		Text:      "Generated body.",
		SpecScene: *singleSceneSpec(),
	})
