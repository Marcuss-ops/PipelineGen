...
func (p *ImageProcessor) Name() adapters.ProcessorName { return adapters.ProcessorImages }

// Policy classifies images as adapters.ProcessorBestEffort: a missing image
...
func (p *ImageProcessor) Process(ctx context.Context, plan *scriptpkg.ResolvedGenerationPlan, input adapters.ProcessInput) (*adapters.PostProcessResult, error) {
	scenes := adapters.FilterMediaResolutionScenes(specScenesFromInput(input))
	if len(scenes) == 0 {
	...
	images := make([]adapters.SceneImage, 0, len(outcomes))
	var warnings []string
	...
	// adapters.PostProcessResult.Warnings so MergePostProcessResult's
	...
	// is not a real concern. adapters.PostProcessResult.Warnings may
	// already carry translation-processor warnings for the same
	// Run; appending here keeps both surfaces visible.
	return &adapters.PostProcessResult{SceneImages: images, Warnings: warnings}, nil
}
