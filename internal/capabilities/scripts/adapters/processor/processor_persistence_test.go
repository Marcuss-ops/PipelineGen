...
// *scriptpkg.ModelScriptOutputV1 (not a adapters.ProcessInput envelope) since
...
// baseInputForIdem() adapters.ProcessInput helper. PersistenceProcessor
...
// PR 3: the typed walk now returns *adapters.PostProcessResult{ScriptID}
...
// absence is enforced by compile-time: the adapters.PostProcessResult
...
// is treated as a no-op (returns a zero adapters.PostProcessResult with no
// repo call).
...
// field is compile-time absent from adapters.PostProcessResult.
var _ = adapters.PostProcessResult{}

func baseProcessInput() adapters.ProcessInput {
	return adapters.ProcessInput{Text: "test", ModelUsed: "llama3.2", CacheStatus: "generated"}
}
