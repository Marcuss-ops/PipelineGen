...
func TestDocumentsProcessor_DoesNotRewriteSingleSceneSpecScene(t *testing.T) {
	...
	if _, err := NewDocumentsProcessor(stub).Process(context.Background(), plan, adapters.ProcessInput{Text: global, SpecScene: scriptpkg.SpecSceneOutput{Version: 1, Scenes: []scriptpkg.SpecScene{{ID: "scene-0", Text: canonical}}}}); err != nil {
		t.Fatal(err)
	}
...
func TestDocumentsProcessor_ReportsCanonicalRendererMetadata(t *testing.T) {
	...
	result, err := NewDocumentsProcessor(stub).Process(context.Background(), plan, adapters.ProcessInput{SpecScene: spec})
...
func TestDocumentsProcessor_OutputMatchesCanonicalRenderer(t *testing.T) {
	...
	if _, err := NewDocumentsProcessor(stub).Process(context.Background(), plan, adapters.ProcessInput{SpecScene: spec}); err != nil {
		t.Fatal(err)
	}
...
func TestDocumentsProcessor_PublishesExplicitLanguages(t *testing.T) {
	...
	result, err := NewDocumentsProcessor(stub).Process(context.Background(), plan, adapters.ProcessInput{Text: "generated text", SpecScene: scriptpkg.SpecSceneOutput{Version: 1}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestDocumentProcessorRejectsEmptyPublisherReference(t *testing.T) {
	...
	_, err := NewDocumentsProcessor(&emptyDocumentReferencePublisherStub{}).Process(context.Background(), plan, adapters.ProcessInput{Text: "generated text"})
...
func TestDocumentsProcessor_PreservesReferenceAfterIdempotencyAnnotationFailure(t *testing.T) {
	...
	result, err := NewDocumentsProcessor(&preservedReferencePublisherStub{}).Process(context.Background(), plan, adapters.ProcessInput{Text: "generated text", SpecScene: scriptpkg.SpecSceneOutput{Version: 1}})
	require.NoError(t, err)
...
func TestDocumentsProcessor_DisabledPlanDoesNotRequirePublisher(t *testing.T) {
	result, err := NewDocumentsProcessor(nil).Process(context.Background(), &scriptpkg.ResolvedGenerationPlan{}, adapters.ProcessInput{})
...
func TestDocumentsProcessor_UsesMetadataTitleOnly(t *testing.T) {
	...
	if _, err := NewDocumentsProcessor(stub).Process(context.Background(), plan, adapters.ProcessInput{Text: "Testo"}); err != nil {
		t.Fatal(err)
	}
	if len(stub.content) != 1 {
		t.Fatalf("expected one document, got %d", len(stub.content))
...
func TestDocumentsProcessor_RefreshesExistingDocWhenASSLinkAppears(t *testing.T) {
	...
	_, err := NewDocumentsProcessor(stub).Process(context.Background(), plan, adapters.ProcessInput{SpecScene: scriptpkg.SpecSceneOutput{Version: 1, Scenes: []scriptpkg.SpecScene{{Bindings: scriptpkg.SceneBindings{Clip: &scriptpkg.ClipBinding{SubtitleLink: "https://drive.google.com/file/d/ass/view"}}}}}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestDocumentsProcessor_RefreshesWhenVoiceoverLinkExists(t *testing.T) {
	...
	_, err := NewDocumentsProcessor(stub).Process(context.Background(), plan, adapters.ProcessInput{SpecScene: scriptpkg.SpecSceneOutput{Version: 1, Scenes: []scriptpkg.SpecScene{{Bindings: scriptpkg.SceneBindings{Voiceover: &scriptpkg.VoiceoverBinding{Links: map[string]string{"it": "VOICE-IT"}}}}}}})
	if err != nil {
		t.Fatal(err)
	}
	if len(stub.titles) != 2 || stub.titles[1] != "refresh" {
...
func TestDocumentsProcessor_RefreshesWhenMultiClipSubtitleAppears(t *testing.T) {
	...
	_, err := NewDocumentsProcessor(stub).Process(context.Background(), plan, adapters.ProcessInput{SpecScene: scriptpkg.SpecSceneOutput{Version: 1, Scenes: []scriptpkg.SpecScene{{Bindings: scriptpkg.SceneBindings{Clips: []scriptpkg.ClipBinding{{SubtitleLink: "https://drive.google.com/file/d/ass-b/view"}}}}}}})
	if err != nil {
		t.Fatal(err)
	}
	if len(stub.titles) != 2 || stub.titles[1] != "refresh" {
...
func TestDocumentsProcessor_BuildsLanguageSpecificContent(t *testing.T) {
	...
	_, err := NewDocumentsProcessor(stub).Process(context.Background(), plan, adapters.ProcessInput{
...
func TestPostProcessResult_IsEmptyFalseForDocumentReference(t *testing.T) {
	if (&adapters.PostProcessResult{DocID: "doc-1", DocLink: "https://docs.google.com/document/d/doc-1/edit"}).IsEmpty() {
