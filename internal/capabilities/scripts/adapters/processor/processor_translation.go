...
//
// Policy is adapters.ProcessorBestEffort (godlike/07 NO-FAKE-AVAILABILITY): a
...
func (p *TranslationProcessor) Name() adapters.ProcessorName { return adapters.ProcessorTranslation }

// Policy classifies translation as adapters.ProcessorBestEffort. The
...
// pattern).
func (p *TranslationProcessor) Policy(_ *scriptpkg.ResolvedGenerationPlan) adapters.ProcessorPolicy {
...
// translator success, the result is a non-empty adapters.PostProcessResult
...
func (p *TranslationProcessor) Process(
	...
	if tErr != nil {
		...
		// (errors.Is/As probes work on the returned adapters.PostProcessResult
		// error envelope).
	...
	// surface the translated bundle into adapters.PostProcessResult.TranslatedText
	// + TranslatedSpecScene so cross-Run observability + the
	// buildGenerationResult preference path can see the translated
	// surface WITHOUT relying on the in-place adapters.ProcessInput mutation
