...
func TestAssetLocationReconciliation_PolicyReflectsCommitter(t *testing.T) {
	...
	if got := withoutCommitter.Policy(nil); got != adapters.ProcessorBestEffort {
		t.Fatalf("verification-only policy = %q, want %q", got, adapters.ProcessorBestEffort)
	}
	withCommitter := NewDurableAssetLocationReconciliationProcessor(verifier, &recordingAssetMutator{})
	if got := withCommitter.Policy(nil); got != adapters.ProcessorRequired {
		t.Fatalf("durable policy = %q, want %q", got, adapters.ProcessorRequired)
	}
}
...
func TestAssetLocationReconciliation_NoChangesDoesNotCommit(t *testing.T) {
	...
	committer := &recordingAssetMutator{}
	processor := NewDurableAssetLocationReconciliationProcessor(verifier, committer)
	if _, err := processor.Process(context.Background(), nil, adapters.ProcessInput{
		SpecScene: scriptpkg.SpecSceneOutput{Version: 1, Scenes: []scriptpkg.SpecScene{
