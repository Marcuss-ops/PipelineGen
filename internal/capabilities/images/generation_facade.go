...
type Registry interface { Resolve(providerID string) (Provider, error) }

func NewGenerationProviderRegistry(log *zap.Logger, provider GenerationProvider) *GenerationProviderRegistry { return imggeneration.NewRegistry(log, provider) }
func NewDefaultProviderRegistry(log *zap.Logger, generator ImageGenerator) *GenerationProviderRegistry { return imggeneration.NewDefaultRegistry(log, generator) }
func NewGoogleSlidesProvider(generator ImageGenerator, log *zap.Logger) *GoogleSlidesProvider { return imggeneration.NewGoogleSlidesProvider(generator, log) }
func NewGenerationService(registry *GenerationProviderRegistry, styles StyleResolver, log *zap.Logger, storage *ImageStorageService) *GenerationService { return imggeneration.NewGenerationService(registry, styles, log, storage) }
func NewJobHandler(registry *GenerationProviderRegistry, styles StyleResolver, log *zap.Logger) *JobHandler { return imggeneration.NewJobHandler(registry, styles, log) }
func RunUsage(ctx context.Context, deps UsecaseDeps, cmd UsecaseCommand) (*UsecaseOutput, error) { return imggeneration.RunUsage(ctx, deps, cmd) }
func GenerateSync(ctx context.Context, svc *GenerationService, cmd SyncCommand) (*detail.ImageAsset, error) { return imggeneration.GenerateSync(ctx, svc, cmd) }
func ClassifyError(errMsg string) error { return imggeneration.ClassifyError(errMsg) }
func ComputeSourceHash(provider, prompt, style string, width, height int, model string) string { return imggeneration.ComputeSourceHash(provider, prompt, style, width, height, model) }
func IsRetryable(err error) bool { return imggeneration.IsRetryable(err) }
func dispatchToRegistry(ctx context.Context, registry *GenerationProviderRegistry, req GenerateImageRequest) (*GeneratedImage, error) { return imggeneration.Dispatch(ctx, registry, req) }
