...
func TestGenerationService_GenerateSmartImage_FailsClosedWhenPrimaryFails(t *testing.T) {
	...
	if err == nil { t.Fatal("expected GenerateSmartImage to fail closed when the primary provider fails") }
	if got != nil { t.Fatalf("GenerateSmartImage returned asset %+v, want nil on fail-closed path", got) }
}
