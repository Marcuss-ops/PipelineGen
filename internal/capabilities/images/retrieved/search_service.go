...
//     SearchServicePort into a Service so the parent
...
// tight — only what's needed by the Router's
...
// that consume Service (e.g. the parent Router) can
...
// the result into a SearchResponse.
...
func (a *SearchServiceAdapter) Search(ctx context.Context, req SearchRequest) (SearchResponse, error) {
	if a == nil || a.port == nil {
		return SearchResponse{}, ErrPortNotWired
	}
