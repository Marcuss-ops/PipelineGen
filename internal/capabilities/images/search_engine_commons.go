...
func (s *ImageStorageService) searchWikimediaCommons(ctx context.Context, query string) retrieved.RetrievalSearchResult {
	...
	commonsOptions := &httpjson.Options{UserAgent: userAgent, Headers: map[string]string{"Api-User-Agent": userAgent}}
	...
	for _, page := range searchPayload.Pages {
		...
		licensePayload, licenseErr := httpjson.GetJSON[commonsRESTPagePayload](ctx, s.client, "https://api.wikimedia.org/core/v1/commons/page/"+url.PathEscape(page.Key), commonsOptions)
		if licenseErr != nil || !commonsLicenseIsExplicit(licensePayload.License.Title) {
			continue
		}
		filePayload, fileErr := httpjson.GetJSON[commonsRESTFilePayload](ctx, s.client, "https://api.wikimedia.org/core/v1/commons/file/"+url.PathEscape(page.Key), commonsOptions)
		if fileErr != nil {
			continue
		}
