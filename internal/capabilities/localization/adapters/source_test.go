...
func newTestSourceResolver(t *testing.T, resolve cliprender.AssetResolver, material cliprender.AssetMaterializer, probe MediaProber) *SourceResolver {
	t.Helper()
	return NewSourceResolver(resolve, material, probe)
}
...
func TestLocalizationSourceResolver_NilDepsFailClosed(t *testing.T) {
	sha := srcTestSHA(0)
	if _, err := NewSourceResolver(nil, &fakeLocalizationAssetMaterializer{mat: srcTestMaterial(sha)}, &fakeLocalizationMediaProber{info: srcTestInfo(30, 1)}).ResolveSource(context.Background(), "source-1"); err == nil {
		t.Fatal("nil resolver must fail closed")
	}
	if _, err := NewSourceResolver(&fakeLocalizationAssetResolver{ref: srcTestRef(sha)}, nil, &fakeLocalizationMediaProber{info: srcTestInfo(30, 1)}).ResolveSource(context.Background(), "source-1"); err == nil {
		t.Fatal("nil materializer must fail closed")
	}
	if _, err := NewSourceResolver(&fakeLocalizationAssetResolver{ref: srcTestRef(sha)}, &fakeLocalizationAssetMaterializer{mat: srcTestMaterial(sha)}, nil).ResolveSource(context.Background(), "source-1"); err == nil {
