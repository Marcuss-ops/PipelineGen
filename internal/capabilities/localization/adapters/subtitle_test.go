...
func (f *fakeTextTrackByIDReader) FindByID(_ context.Context, trackID int64) (*detail.TextTrack, []detail.TimedCue, error) {
	f.gotID = trackID
	if f.err != nil {
...
func readyTrack(id int64, lang, text string) *detail.TextTrack {
	return &detail.TextTrack{
		...
		TextContent:  text,
		TextHash:     detail.TextHash(text, lang, detail.TextTrackTranscript),
		Status:       detail.TextTrackReady,
	}
}

func twoCues() []detail.TimedCue {
...
func TestLocalizationSubtitleResolver_ResolvesReadyTrack(t *testing.T) {
	...
	r := NewSubtitleResolver(&fakeTextTrackByIDReader{track: trk, cues: twoCues()})
...
func TestLocalizationSubtitleResolver_RecomputesEmptyHash(t *testing.T) {
	...
	got, err := r.ResolveSubtitleTrack(context.Background(), 202, detail.TextHash(trk.TextContent, "es", detail.TextTrackTranscript))
	if err != nil {
		t.Fatalf("ResolveSubtitleTrack: %v", err)
	}
	if got.TextHash != detail.TextHash(trk.TextContent, "es", detail.TextTrackTranscript) {
...
func TestLocalizationSubtitleResolver_RejectsNonReady(t *testing.T) {
	trk := readyTrack(202, "es", "hola mundo")
	trk.Status = detail.TextTrackPending
	r := NewSubtitleResolver(&fakeTextTrackByIDReader{track: trk, cues: twoCues()})
...
func TestLocalizationSubtitleResolver_RejectsNotFound(t *testing.T) {
	r := NewSubtitleResolver(&fakeTextTrackByIDReader{track: nil, cues: nil})
...
func TestLocalizationSubtitleResolver_RejectsEmptyCues(t *testing.T) {
	trk := readyTrack(202, "es", "hola mundo")
	r := NewSubtitleResolver(&fakeTextTrackByIDReader{track: trk, cues: nil})
...
func TestLocalizationSubtitleResolver_PropagatesRepoError(t *testing.T) {
	r := NewSubtitleResolver(&fakeTextTrackByIDReader{err: errors.New("db down")})
