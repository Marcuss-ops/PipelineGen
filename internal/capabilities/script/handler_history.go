// Package script (api/script) — handler_history.go: the script-history
// HTTP transport (ScriptHistoryHandler + ScriptHistoryModule).
//
// CLEANUP (September 2026): helpers.go historically held a grab-bag of
// cross-cutting helpers. The grab-bag was drained across successive
// compactions (Wave 14, post-write context + forwarding aliases
// deleted) until only the script-history transport remained. The file
// is now named for what it actually contains — the ScriptHistoryHandler
// that serves GET /scripts + GET /scripts/:id and the companion
// ScriptHistoryModule that mounts it on the /scripts prefix.
package script
