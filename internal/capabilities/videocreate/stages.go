...
//
// Each is the typed shape a child records in its job result and the
// workflow reads. They carry DURABLE identities first (asset id,
// content SHA-256, Drive file id, duration, media type); the local path
// is an execution detail the media plane needs and is rehydrated after
// a restart by re-reading the child's result (recovery.go).

// ScriptChildResult is the script.generate child result (§9: script +
// scene plan + canonical timeline facts).
type ScriptChildResult struct {
	...
	// script/voiceover stage produced one. The workflow reuses it
	// verbatim — it never re-derives mix decisions (§13).
...
// already-produced canonical audio master (the §9 reuse input).
...
// (youtube_clip.extract / media.stock).
...
// VoiceoverChildResult is the voiceover.generate child result.
...
// ── 01_script ────────────────────────────────────────────────────────
//
// The §9 first stage. ONE script.generate child (key
// "<root>:script") produces the script, the scene plan and the
// canonical timeline facts. The §9 reuse rule lands here too: when the
// child's result already carries the canonical final audio, the
// voiceover and audio-master steps will be SKIPPED and that master is
// reused — video.create never fans out work the children already did.

func runScriptStage(ctx context.Context, run *Run, spec StepSpec) (StageOutput, error) {
	out := StageOutput{}
	childID, err := run.enqueue(ctx, spec, ChildKey(run.RootKey, "script"), appjobs.TypeScriptGenerate, ScriptChildRequest{
		Topic:           run.Request.Topic,
		Language:        run.Request.Language,
		DurationSeconds: run.Request.DurationSeconds,
		MediaSources:    run.Request.MediaSources,
	})
	if err != nil {
	...
	var res ScriptChildResult
	if err := childResult(child, &res); err != nil {
		return out, fmt.Errorf("%w: script child result: %v", ErrWorkflowFailed, err)
	}
	if res.ScriptAssetID == "" || len(res.Scenes) == 0 {
		return out, fmt.Errorf("%w: script child produced no script_asset_id/scenes", ErrWorkflowFailed)
	}
	...
	if res.FinalAudio != nil {
		...
		out.FinalAudio = &ref
		out.AudioMaster = &MasteredAudio{
			Asset: audio.FinalAudioAsset{
				AssetID:          res.FinalAudio.AssetID,
				FinalAudioSHA256: res.FinalAudio.ContentSHA,
				DurationMS:       res.FinalAudio.DurationMS,
				SampleRate:       res.FinalAudio.SampleRate,
				Channels:         res.FinalAudio.Channels,
				Codec:            res.FinalAudio.Codec,
			},
			Path: res.FinalAudio.LocalPath,
		}
...
// media.stock for stock/Artlist) with its derived per-scene key. The
// children own download / cut / normalize / register; the workflow
// records only their durable outputs (asset id, content hash, Drive
// identity, duration, media type).
...
func runMediaAcquireStage(ctx context.Context, run *Run, spec StepSpec) (StageOutput, error) {
	...
	for i, cand := range selection {
		...
		childID, err := run.enqueue(ctx, spec, SceneChildKey(run.RootKey, family, sceneIndex), jobType, MediaAcquireChildRequest{
			Project:    run.Job.Project,
			VideoName:  run.Job.VideoName,
			SceneIndex: sceneIndex,
			Source:     cand.Source,
			SourceURL:  cand.SourceURL,
			Query:      run.Request.Topic,
		})
		if err != nil {
			return out, fmt.Errorf("%w: media acquire enqueue: %v", ErrWorkflowFailed, err)
		}
		issued = append(issued, pending{id: childID, src: cand.Source})
	}
	for _, p := range issued {
		...
		if err := childResult(child, &res); err != nil {
			return out, fmt.Errorf("%w: acquire child result: %v", ErrWorkflowFailed, err)
		}
		if res.AssetID == "" {
			return out, fmt.Errorf("%w: acquire child %s produced no asset_id", ErrWorkflowFailed, p.id)
		}
...
func runVoiceoverStage(ctx context.Context, run *Run, spec StepSpec) (StageOutput, error) {
	...
	childID, err := run.enqueue(ctx, spec, ChildKey(run.RootKey, "voiceover"), appjobs.TypeVoiceoverGenerate, VoiceoverChildRequest{
		Project:         run.Job.Project,
		Language:        run.Request.Language,
	...
	if err := childResult(child, &res); err != nil {
		return out, fmt.Errorf("%w: voiceover child result: %v", ErrWorkflowFailed, err)
	}
	if res.AssetID == "" {
		return out, fmt.Errorf("%w: voiceover child produced no asset_id", ErrWorkflowFailed)
	}
	ref := StageArtifactRef{
		AssetID:    res.AssetID,
		Kind:       "voiceover",
	...
	run.Facts.Voiceover = &VoiceoverFact{
		Ref:        ref,
		LocalPath:  res.LocalPath,
...
// media plane's resolved inputs (asset id + local materialization).
func (r *Run) resolvedAudioAssets() audio.ResolvedAudioAssets {
	var out audio.ResolvedAudioAssets
	if r.Facts.Voiceover != nil && r.Facts.Voiceover.LocalPath != "" {
		out = append(out, audio.ResolvedAudioAsset{
			AssetID: r.Facts.Voiceover.Ref.AssetID,
			Path:    r.Facts.Voiceover.LocalPath,
		})
	}
	return out
}
