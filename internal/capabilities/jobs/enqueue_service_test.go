...
func TestNewService_HappyPath_ReturnsService(t *testing.T) {
	...
	if err != nil || svc == nil { t.Fatalf("NewService() = (%v, %v)", svc, err) }
	if svc.registry != reg { t.Fatal("service registry mismatch") }
}
...
func TestEnqueue_HappyPath_PopulatesMaxRetriesFromRegistry(t *testing.T) {
	...
	if got == nil || got.MaxRetries != 7 { t.Fatalf("Enqueue() = %#v, want MaxRetries=7", got) }
}
...
func TestEnqueue_ExistingCorrelationID_DedupReturnsExisting(t *testing.T) {
	...
	if err != nil { t.Fatalf("first Enqueue: %v", err) }
	req2 := *req
	req2.Payload = map[string]any{"run": 2}
	second, err := svc.Enqueue(context.Background(), &req2)
	if err != nil { t.Fatalf("second Enqueue: %v", err) }
	if first == nil || second == nil || first.ID != second.ID { t.Fatalf("dedup mismatch: first=%#v second=%#v", first, second) }
}
...
func TestEnqueue_PopulatesRootJobID_CanonicalLineage(t *testing.T) {
	...
	if err != nil { t.Fatalf("root Enqueue: %v", err) }
	if root.RootJobID != root.ID || root.ParentJobID != "" { t.Fatalf("root lineage = %#v", root) }
	...
	if err != nil { t.Fatalf("child Enqueue: %v", err) }
	if child.ParentJobID != root.ID || child.RootJobID != root.ID { t.Fatalf("child lineage = %#v", child) }
}
...
func TestEnqueue_M2M_IdempotencyIsScopedPerClient(t *testing.T) {
	...
	if err != nil { t.Fatalf("same-key Enqueue: %v", err) }
	...
	if err != nil { t.Fatalf("other-client Enqueue: %v", err) }
	if first.ID != same.ID { t.Fatalf("same client/key did not dedup: %s != %s", first.ID, same.ID) }
	if first.ID == other.ID { t.Fatalf("different clients collided on idempotency key: %s", first.ID) }
}
