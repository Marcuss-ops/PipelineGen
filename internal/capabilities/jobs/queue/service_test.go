...
func (b *duplicateBroker) FindByTypeAndCorrelation(context.Context, string, string) (*job.Job, error) {
	return b.existing, nil
}
func (b *duplicateBroker) FindActiveByKey(context.Context, string) (*job.Job, error) { return nil, nil }
func (b *duplicateBroker) FindByClientAndIdempotencyKey(context.Context, string, string) (*job.Job, error) {
	return nil, nil
}
...
func TestServiceEnqueue_DuplicateUsesKernelSentinelAndReturnsExisting(t *testing.T) {
	...
	if got == nil || got.ID != existing.ID {
		t.Fatalf("Enqueue() = %#v, want existing %#v", got, existing)
	}
}
...
func TestServiceEnqueue_DuplicateWithoutRecoverableKeyPropagates(t *testing.T) {
	...
	if err == nil || !errors.Is(err, job.ErrDuplicate) {
		t.Fatalf("Enqueue() error = %v, want job.ErrDuplicate", err)
	}
}
