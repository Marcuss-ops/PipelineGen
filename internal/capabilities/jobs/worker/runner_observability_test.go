...
// obsLeaseBroker is a minimal jobs.Broker recording the terminal
...
func (b *obsLeaseBroker) RegisterWorker(context.Context, jobs.RegisterWorkerCommand) (*jobs.WorkerSession, error) {
	return nil, nil
}
func (b *obsLeaseBroker) Heartbeat(context.Context, jobs.HeartbeatCommand) error { return nil }
func (b *obsLeaseBroker) Claim(context.Context, jobs.ClaimCommand) (*jobs.Lease, error) {
	return nil, nil
}
func (b *obsLeaseBroker) Renew(_ context.Context, cmd jobs.RenewCommand) (*jobs.Lease, error) {
	return &jobs.Lease{Job: &job.Job{ID: cmd.JobID, Type: "obs.test", Revision: 1, LeaseID: cmd.LeaseID}, LeaseID: cmd.LeaseID}, nil
}
func (b *obsLeaseBroker) Progress(context.Context, jobs.ProgressCommand) error { return nil }
func (b *obsLeaseBroker) Complete(_ context.Context, _ jobs.CompleteCommand) error {
	b.mu.Lock()
	defer b.mu.Unlock()
	b.completeCalls++
	return b.completeReturnErr
}
func (b *obsLeaseBroker) CompleteWithArtifacts(_ context.Context, _ jobs.CompleteWithArtifactsCommand) ([]string, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
	b.completeWithArtifacts++
	return nil, b.completeWithArtifactsE
}
func (b *obsLeaseBroker) Fail(_ context.Context, _ jobs.FailCommand) error {
	b.mu.Lock()
	defer b.mu.Unlock()
	b.failCalls++
...
func obsTestLease(createdAt time.Time, startedAt time.Time, retryCount int) *jobs.Lease {
	return &jobs.Lease{
		Job: &job.Job{
			ID:          "obs-remote-job",
			Type:        "obs.test",
