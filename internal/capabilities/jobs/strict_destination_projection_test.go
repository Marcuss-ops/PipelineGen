...
//
//   - kindFromDestination(dest) → (finalization.ArtifactKind, error)
//   - sourceFromDestination(dest) → (string, error)
//
...
// (ErrUnmappedCompletionDestination) is unreachable from the canonical
...
// is the SOLE source of ErrUnmappedCompletionDestination).
func TestKindFromDestination_StrictMode_LocksCutoverFromKindDocument(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.dest, func(t *testing.T) {
			got, err := kindFromDestination(c.dest)
			if err != nil {
				t.Fatalf("kindFromDestination(%q): unexpected err=%v (canonical destination MUST succeed)", c.dest, err)
			}
			if got != c.want {
				t.Errorf("kindFromDestination(%q): got %v want %v (FASE 3 cutover LOCK; pre-FASE-3 silently cast to KindDocument)",
					c.dest, got, c.want)
			}
			if c.cutover {
				cutoverCount++
				...
				if got == finalization.KindDocument {
					t.Errorf("kindFromDestination(%q) MUST NOT return KindDocument (FASE 3 cutover regression)", c.dest)
				}
			}
		})
	}
...
// ErrUnmappedCompletionDestination (typed sentinel, errors.Is
...
func TestKindFromDestination_StrictMode_RejectsUnknownDestination(t *testing.T) {
	...
	for _, dest := range cases {
		t.Run("dest="+dest, func(t *testing.T) {
			got, err := kindFromDestination(dest)
			if err == nil {
				t.Fatalf("kindFromDestination(%q): expected wrapped ErrUnmappedCompletionDestination, got nil (silent auto-cast FASE 3 regression)", dest)
			}
			if !errors.Is(err, ErrUnmappedCompletionDestination) {
				t.Errorf("kindFromDestination(%q): err=%v; want wraps ErrUnmappedCompletionDestination", dest, err)
			}
			...
			if !strings.Contains(err.Error(), `"`+dest+`"`) {
				t.Errorf("kindFromDestination(%q): err message=%q; want quoted destination in the message", dest, err.Error())
			}
			if got != "" {
				t.Errorf("kindFromDestination(%q): on unknown destination want zero ArtifactKind, got %q", dest, got)
			}
		})
	}
}
...
func TestSourceFromDestination_StrictMode_LocksCanonicalSourceEnum(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.dest, func(t *testing.T) {
			got, err := sourceFromDestination(c.dest)
			if err != nil {
				t.Fatalf("sourceFromDestination(%q): unexpected err=%v", c.dest, err)
			}
			if got != c.want {
				t.Errorf("sourceFromDestination(%q): got %q want %q (FASE 3 strict-mode LOCK; pre-FASE-3 used `default: return dest` which propagated unknown values verbatim)",
					c.dest, got, c.want)
			}
		})
	}
...
// ErrUnmappedCompletionDestination AND return an empty source string
...
func TestSourceFromDestination_StrictMode_RejectsUnknownDestination(t *testing.T) {
	...
	for _, dest := range cases {
		t.Run("dest="+dest, func(t *testing.T) {
			got, err := sourceFromDestination(dest)
			if err == nil {
				t.Fatalf("sourceFromDestination(%q): expected wrapped ErrUnmappedCompletionDestination, got nil", dest)
			}
			if !errors.Is(err, ErrUnmappedCompletionDestination) {
				t.Errorf("sourceFromDestination(%q): err=%v; want wraps ErrUnmappedCompletionDestination", dest, err)
			}
			if got != "" {
				t.Errorf("sourceFromDestination(%q): on unknown destination want empty source, got %q (pre-FASE-3 silently stashed the destination string)", dest, got)
			}
		})
	}
}
