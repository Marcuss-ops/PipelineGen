...
func TestCodecIDForPayload_ByteStable(t *testing.T) {
	// Empty payload → "empty"
	if got := CodecIDForPayload(nil); got != "empty" {
		t.Errorf("empty/nil payload discriminator: want %q, got %q", "empty", got)
	}
	if got := CodecIDForPayload([]byte{}); got != "empty" {
	...
	for i, payload := range tests {
		if got := CodecIDForPayload(payload); got != "json.v1" {
	...
	canonical := CodecIDForPayload([]byte(`{"ok":true}`))
	for i := 0; i < 1000; i++ {
		if got := CodecIDForPayload([]byte(`{"ok":true}`)); got != canonical {
...
func TestTxContext_HasSevenMethods(t *testing.T) {
	iface := reflect.TypeOf((*TxContext)(nil)).Elem()
	want := 7
