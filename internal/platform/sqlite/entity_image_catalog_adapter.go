...
func NewSQLiteEntityImageCatalogAdapter(db *sql.DB) entitycatalog.Repository {
	return &SQLiteEntityImageCatalogAdapter{db: db}
}

var _ entitycatalog.Repository = (*SQLiteEntityImageCatalogAdapter)(nil)

func (a *SQLiteEntityImageCatalogAdapter) UpsertEntity(ctx context.Context, entity entitycatalog.Entity) error {
	if a == nil || a.db == nil {
	...
	if err := entitycatalog.ValidateEntity(entity); err != nil {
		return err
	}
...
func (a *SQLiteEntityImageCatalogAdapter) GetEntity(ctx context.Context, canonicalID string) (*entitycatalog.Entity, error) {
	if a == nil || a.db == nil {
		return nil, fmt.Errorf("entity image catalog: database unavailable")
	}
	canonicalID = entitycatalog.NormalizePersonEntityID(canonicalID)
	if canonicalID == "" {
...
func (a *SQLiteEntityImageCatalogAdapter) SetRefreshState(ctx context.Context, canonicalID, status string, refreshedAt time.Time, lastError string) error {
	...
	if status == "" {
		status = entitycatalog.RefreshStatusNever
	}
	switch status {
	case entitycatalog.RefreshStatusNever, entitycatalog.RefreshStatusRunning, entitycatalog.RefreshStatusSucceeded, entitycatalog.RefreshStatusFailed:
	default:
...
func (a *SQLiteEntityImageCatalogAdapter) UpsertCandidate(ctx context.Context, candidate entitycatalog.Candidate) (int64, error) {
	if a == nil || a.db == nil {
	...
	canonicalID := entitycatalog.NormalizePersonEntityID(candidate.CanonicalEntityID)
	status := candidate.Status
	if status == "" || status == entitycatalog.CandidateStatusActive {
...
func (a *SQLiteEntityImageCatalogAdapter) ListCandidates(ctx context.Context, canonicalID string, limit int) ([]entitycatalog.Candidate, error) {
	if a == nil || a.db == nil {
		return nil, fmt.Errorf("entity image catalog: database unavailable")
	}
	canonicalID = entitycatalog.NormalizePersonEntityID(canonicalID)
	...
	defer rows.Close()
	out := make([]entitycatalog.Candidate, 0, limit)
	for rows.Next() {
		var candidate entitycatalog.Candidate
...
func (a *SQLiteEntityImageCatalogAdapter) UpsertMaterialization(ctx context.Context, materialization entitycatalog.Materialization) error {
	if a == nil || a.db == nil {
		return fmt.Errorf("entity image catalog: database unavailable")
	}
	if err := entitycatalog.ValidateMaterialization(materialization); err != nil {
		return err
	}
	status := materialization.Status
	if status == "" {
		status = entitycatalog.MaterializationStatusPending
	}
	materializedAt := ""
...
func (a *SQLiteEntityImageCatalogAdapter) ListCandidatesForRecertification(ctx context.Context, now time.Time, limit, maxAttempts int) ([]entitycatalog.RecertificationCandidate, error) {
	if a == nil || a.db == nil {
		return nil, fmt.Errorf("entity image catalog: database unavailable")
	}
	if limit <= 0 {
	...
	if maxAttempts <= 0 {
		maxAttempts = entitycatalog.DefaultRecertificationMaxAttempts
	}
	cutoff := now.UTC().Add(-entitycatalog.CandidateFreshAfter).Format("2006-01-02 15:04:05")
	...
	rows, err := a.db.QueryContext(ctx, `
		...
		LIMIT ?
	`, entitycatalog.CandidateSemanticAccepted,
		entitycatalog.CandidateStatusFresh, entitycatalog.CandidateStatusActive, entitycatalog.CandidateStatusStale,
		cutoff, entitycatalog.CandidateStatusBroken, maxAttempts, nowValue, limit)
	if err != nil {
		return nil, fmt.Errorf("entity image catalog: list recertification candidates: %w", err)
	}
	defer rows.Close()
	out := make([]entitycatalog.RecertificationCandidate, 0, limit)
	for rows.Next() {
		var item entitycatalog.RecertificationCandidate
		var firstSeen, lastSeen, updatedAt string
		var lastValidationAt, nextRetryAt string
		var materialization entitycatalog.Materialization
...
func (a *SQLiteEntityImageCatalogAdapter) RecordCandidateValidation(ctx context.Context, candidateID int64, result entitycatalog.ValidationResult) error {
	if a == nil || a.db == nil {
		return fmt.Errorf("entity image catalog: database unavailable")
	}
	if candidateID < 1 {
	...
	if result.Success {
		res, err := a.db.ExecContext(ctx, `
			...
			WHERE candidate_id = ?
		`, entitycatalog.CandidateStatusFresh, checkedAt, checkedAt, candidateID)
		if err != nil {
	...
	res, err := a.db.ExecContext(ctx, `
		...
		SET status = ?, updated_at = CURRENT_TIMESTAMP,
		    ...
		    last_validation_error = ?
		WHERE candidate_id = ?
	`, entitycatalog.CandidateStatusBroken, result.FailureCount, checkedAt, nextRetryAt, result.Error, candidateID)
	if err != nil {
...
func (a *SQLiteEntityImageCatalogAdapter) GetMaterialization(ctx context.Context, candidateID int64) (*entitycatalog.Materialization, error) {
	if a == nil || a.db == nil {
		return nil, fmt.Errorf("entity image catalog: database unavailable")
	}
	if candidateID < 1 {
...
func defaultSemanticStatus(status string) string {
	status = strings.ToLower(strings.TrimSpace(status))
	if status == "" {
		return entitycatalog.CandidateSemanticUnknown
	}
	return status
}
...
func defaultRefreshStatus(status string) string {
	if status == "" {
		return entitycatalog.RefreshStatusNever
	}
	return status
}
