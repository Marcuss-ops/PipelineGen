...
// ports.MaintenanceRepository port for SQLite.
...
var _ ports.MaintenanceRepository = (*MaintenanceRepositorySQLite)(nil)
...
// be missing on disk.
func (r *MaintenanceRepositorySQLite) ScanLocalOrphans(ctx context.Context, batch int) ([]ports.LocalOrphanCandidate, error) {
	...
	var candidates []ports.LocalOrphanCandidate
	for rows.Next() {
		var c ports.LocalOrphanCandidate
...
func (r *MaintenanceRepositorySQLite) ScanDriveOrphans(ctx context.Context, batch int) ([]ports.DriveOrphanCandidate, error) {
	rows, err := r.db.QueryContext(ctx, `
	...
	var candidates []ports.DriveOrphanCandidate
	for rows.Next() {
		var c ports.DriveOrphanCandidate
