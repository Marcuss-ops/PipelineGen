...
func HardDeleteTx(ctx context.Context, tx *sql.Tx, id string) error {
	if tx == nil {
		return fmt.Errorf("HardDeleteTx: tx is required (caller MUST supply the open *sql.Tx)")
	}
	if id == "" {
		return fmt.Errorf("HardDeleteTx: id is required")
	}
	...
	case err == sql.ErrNoRows:
		...
		log.Debug("HardDeleteTx: parent row missing (idempotent no-op)", zap.String("id", id))
		return nil
	case err != nil:
		return fmt.Errorf("HardDeleteTx %s: parent probe: %w", id, err)
	}
	...
	for _, ct := range hardDeleteChildTables {
		...
		if derr != nil {
			return fmt.Errorf("HardDeleteTx %s: delete %s(%s): %w", id, ct.table, ct.column, derr)
		}
		affected, _ := res.RowsAffected()
		log.Debug("HardDeleteTx: child row deletion",
			zap.String("id", id),
	...
	log.Warn("HardDeleteTx: orphan-row audit log (admin tool gate opened; downstream children cleaned; parent row removed)",
		zap.String("id", id),
