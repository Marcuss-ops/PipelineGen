...
func TestObservabilityEndToEndPersistsAndReadsWithoutRawDuplicates(t *testing.T) {
	...
	if _, err := db.Exec(`INSERT INTO performance_operations VALUES ('obs-1','run-e2e','render',9000,0,1,10000,?)`, time.Now().UTC().Format(time.RFC3339Nano)); err != nil {
		t.Fatal(err)
	}
	if _, err := db.Exec(`INSERT OR REPLACE INTO performance_operations VALUES ('obs-1','run-e2e','render',9000,0,1,10000,?)`, time.Now().UTC().Format(time.RFC3339Nano)); err != nil {
		t.Fatal(err)
	}
	var count int
