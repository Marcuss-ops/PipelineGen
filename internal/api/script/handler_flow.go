...
// ── Batch generation endpoints (/generate-batch, /generate-batch/progress) ─
...
func (h *ScriptFlowHandler) GenerateBatch(c *gin.Context) {
	...
	if req.Async && h.jobsSvc != nil {
		...
		jb, err := h.jobsSvc.Enqueue(c.Request.Context(), &jobservice.EnqueueRequest{
			Type:    "script.generate_batch",
			Payload: payload,
		})
		if err != nil {
		...
		c.JSON(http.StatusOK, gin.H{"ok": true, "async": true, "job_id": jb.ID, "status": jb.Status})
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{"ok": true, "doc_title": resp.DocTitle, "doc_id": resp.DocID, "doc_link": resp.DocLink, "scripts": resp.Scripts})
}
