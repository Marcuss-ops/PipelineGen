...
func (h *DriveHandler) FoldersTree(c *gin.Context) {
	...
	maxDepth := common.ParseIntQuery(c, "max_depth", 2)
	maxItems := common.ParseIntQuery(c, "max_items", 50)

	opts := driveupload.ListFoldersOptions{
		ParentID: folderID,
		MaxDepth: maxDepth,
		MaxItems: maxItems,
	}
...
func (h *DriveHandler) CreateFolder(c *gin.Context) {
	...
	var req driveupload.CreateFolderRequest
...
func (h *DriveHandler) CreateFolderStructure(c *gin.Context) {
	...
	var req driveupload.FolderStructureRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"ok": false, "error": err.Error()})
		return
	}
	...
	group := req.Group
	if group == "" {
		group = driveupload.DetectGroupFromTopic(req.Topic)
	}
	groupName := driveupload.StockDriveGroups[group]
	...
	c.JSON(http.StatusOK, gin.H{
		...
		"group_link":      driveupload.GetFolderLink(groupFolderID),
		"topic_link":      driveupload.GetFolderLink(topicFolderID),
	})
}
