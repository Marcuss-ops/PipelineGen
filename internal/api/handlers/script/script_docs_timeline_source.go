...
// isIntroLikeSegment checks if a segment is like an introduction.
func isIntroLikeSegment(seg timelineLLMSegment) bool {
	keywords := strings.ToLower(strings.Join(seg.Keywords, " "))
	return strings.Contains(keywords, "intro") || strings.Contains(keywords, "introduzione") || strings.Contains(keywords, "inizio")
}

// isOutroLikeSegment checks if a segment is like an outro.
func isOutroLikeSegment(seg timelineLLMSegment) bool {
	keywords := strings.ToLower(strings.Join(seg.Keywords, " "))
	return strings.Contains(keywords, "outro") || strings.Contains(keywords, "conclusione") || strings.Contains(keywords, "fine")
}
...
// semanticBlocksFromText splits text into semantic blocks.
func semanticBlocksFromText(text string, segmentCount int) []string {
	paragraphs := semanticParagraphs(text)
	if len(paragraphs) <= segmentCount {
		return paragraphs
	}

	blocks := make([]string, 0, segmentCount)
	targetSize := len(paragraphs) / segmentCount
	...
	for i := 0; i < segmentCount; i++ {
		...
		block := strings.Join(paragraphs[start:end], "\n\n")
