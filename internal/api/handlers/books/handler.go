...
func NewHandler(svc *booksService.Service, jobsSvc *jobs.Service, log *zap.Logger) *Handler {
	return &Handler{
		svc:     svc,
		jobsSvc: jobsSvc,
		log:     log,
...
func (h *Handler) ProcessBookFromDrive(c *gin.Context) {
	if !handlerutil.RequireService(c, h.svc, "books service") {
		return
	}

	req, ok := apiutil.BindJSON[ProcessBookFromDriveRequest](c)
	if !ok {
		return
	}
