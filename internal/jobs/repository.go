...
func (r *SQLiteStore) Create(ctx context.Context, j *Job) error {
	query := `
...
func scanJobColumns(s scanner, j *Job) error {
...
func (r *SQLiteStore) Get(ctx context.Context, id string) (*Job, error) {
...
func (r *SQLiteStore) List(ctx context.Context, filter Filter) ([]Job, error) {
...
func (r *SQLiteStore) GetStats(ctx context.Context) (*JobStats, error) {
	stats := &JobStats{
		...
		ByType:   make(map[string]map[Status]int),
	}
		...
		for rows.Next() {
			...
			if err := rows.Scan(&jt, &js, &cnt); err != nil {
				...
				if _, ok := stats.ByType[jt]; !ok {
					stats.ByType[jt] = make(map[Status]int)
				}
...
func (r *SQLiteStore) FindActiveByKey(ctx context.Context, activeKey string) (*Job, error) {
...
func (r *SQLiteStore) FindByTypeAndCorrelation(ctx context.Context, jobType string, correlationID string) (*Job, error) {
	if correlationID == "" {
		return nil, nil
	}
...
func (r *SQLiteStore) ListEvents(ctx context.Context, jobID string) ([]Event, error) {
