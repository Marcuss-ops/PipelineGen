...
// Transaction is a narrow write-only surface that AssetFinalizerTx
// consumes to participate in the JobFinalizer's transaction. The
// production concrete is *sql.Tx (or a thin wrapper).
//
// This interface is intentionally minimal: only ExecContext is exposed.
