...
func TestGeneratePayloadRoundTrip(t *testing.T) {
	...
	payload := script.NewGeneratePayload(script.PresetCustom, spec)
	...
	var decoded script.GeneratePayload
	...
	if decoded.Preset != script.PresetCustom {
...
func TestGeneratePayloadDecodeEmpty(t *testing.T) {
	_, err := script.DecodeGeneratePayload(nil)
	if err == nil {
		t.Fatal("empty payload should return an error")
	}
	if !errors.Is(err, script.ErrInvalidPayload) {
...
func TestGeneratePayloadDecodeRawJSON(t *testing.T) {
	...
	p, err := script.DecodeGeneratePayload(raw)
	if err != nil {
	...
	if p.Preset != script.PresetWithImages {
...
func TestDecodeLegacyFlatPayload(t *testing.T) {
	...
	if p.Preset != script.PresetCustom {
