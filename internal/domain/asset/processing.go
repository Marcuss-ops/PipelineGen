...
func (a *processingRepositoryAdapter) Transition(ctx context.Context, assetID, step string, from, to ProcessingStatus) error {
	return a.store.TransitionProcessing(ctx, assetID, step, from, to)
}

func (a *processingRepositoryAdapter) Get(ctx context.Context, assetID, step string) (*ProcessingRecord, error) {
	return a.store.GetProcessingRecord(ctx, assetID, step)
}

func (a *processingRepositoryAdapter) GetByAssetID(ctx context.Context, assetID string) ([]ProcessingRecord, error) {
	return a.store.GetProcessingRecordsByAssetID(ctx, assetID)
}

func (a *processingRepositoryAdapter) GetFailed(ctx context.Context) ([]ProcessingRecord, error) {
...
// ProcessingRepository returns the ProcessingRepository adapter for the store.
...
func (s *AssetStoreSQLite) TransitionProcessing(ctx context.Context, assetID, step string, from, to ProcessingStatus) error {
	now := timeutil.FormatRFC3339(time.Now())
	_, err := s.db.ExecContext(ctx, `
		UPDATE asset_processing
...
func (s *AssetStoreSQLite) GetProcessingRecord(ctx context.Context, assetID, step string) (*ProcessingRecord, error) {
...
// GetProcessingRecordsByAssetID retrieves all processing records for an asset.
func (s *AssetStoreSQLite) GetProcessingRecordsByAssetID(ctx context.Context, assetID string) ([]ProcessingRecord, error) {
...
func (s *AssetStoreSQLite) GetFailedProcessingRecords(ctx context.Context) ([]ProcessingRecord, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT asset_id, step, status, started_at, completed_at, error_message, attempt_count, metadata_json
		FROM asset_processing
		WHERE status = 'failed'
...
// DeleteAllProcessingRecords removes all processing records for an asset.
...
func scanProcessingRecord(scanner interface{ Scan(dest ...any) error }) (*ProcessingRecord, error) {
	...
	rec.Status = ProcessingStatus(statusStr.String)
