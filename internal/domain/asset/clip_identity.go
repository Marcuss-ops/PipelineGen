...
// DetectSourceFromAssetID infers the canonical source type from an
// asset_id prefix. This is a read-only classification helper — it
// does NOT construct IDs, only inspects them.
//
// Returns:
//   - "youtube" if the ID starts with "yt_"
//   - "stock" if the ID starts with "planner:"
//   - "artlist" if the ID starts with "artlist_"
//   - "voiceover" if the ID starts with "vo_" or "voiceover_"
//   - "" for unknown prefixes
//
// godlike/06 SSOT: this is the domain-level mirror of
// clipindexer/indexing_state.go::sourceFromClipID. Both functions
// classify the same prefixes; this one lives in the domain layer
// so consumers that can't import clipindexer can still classify.
//
// KNOWN DIVERGENCE: the infra-layer sourceFromClipID does NOT
// handle the vo_/voiceover_ prefix (it only checks yt_ and artlist_).
// This domain version adds voiceover classification for completeness.
// Future agents updating one MUST check the other — the two functions
// are NOT byte-equivalent mirrors.
func DetectSourceFromAssetID(assetID string) string {
	switch {
	case strings.HasPrefix(assetID, "yt_"):
		return "youtube"
	case strings.HasPrefix(assetID, "planner:"):
		return "stock"
	case strings.HasPrefix(assetID, "artlist_"):
		return "artlist"
	case strings.HasPrefix(assetID, "vo_") || strings.HasPrefix(assetID, "voiceover_"):
		return "voiceover"
	default:
		return ""
	}
