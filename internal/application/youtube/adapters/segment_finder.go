...
func splitLongSegments(segs []youtubetypes.Segment) []youtubetypes.Segment {
	if len(segs) == 0 {
		return segs
	}
	out := make([]youtubetypes.Segment, 0, len(segs))
...
func (s *Service) findInterestingSegments(ctx context.Context, videoURL string) ([]youtubetypes.Segment, error) {
	videoID, err := urlutil.ExtractVideoID(videoURL)
	if err != nil || videoID == "" {
		videoID = s.callbacks.MD5String(videoURL)[:12]
	}
	...
	meta, metaErr := s.callbacks.GetVideoInfo(ctx, videoURL)
