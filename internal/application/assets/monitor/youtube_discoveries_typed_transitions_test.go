// Package monitor — youtube_discoveries_typed_transitions_test.go:
// FASE 1.3 typed transition tests + FASE 3.7 sentinel-alias pin tests.
// Step 8 split (July 2026): extracted from the 1555-LOC
// youtube_discoveries_test.go canonical and placed alongside 3 sibling
// FASE leaf files.
//
// ── FASE 1.3: Typed transition result tests ────────────────────────────────
//
// MarkEnqueued + MarkRejected return TYPED errors (ErrAlreadyApplied /
// ErrStateConflict / ErrNotFound) instead of plain `error` so callers can
// branch via `errors.Is` without string matching.
//
...
// sides of the alias.
//
// The dedupe contract pre-Commit-3/6 tests live in
// youtube_discoveries_contract_test.go; the retry-semantics + backoff
// tests live in youtube_discoveries_retry_test.go; the FASE 1.1
// atomic concurrent tests live in youtube_discoveries_concurrent_test.go.
// Helpers (migrationsSQLite114, newInMemoryLedger) live in
// youtube_discoveries_test_helpers_test.go.
//
// These tests FAIL on regression only if:
//   - MarkEnqueued returns nil instead of ErrAlreadyApplied on repeat,
//   - MarkEnqueued swallows non-pendingrow states,
//   - ErrNotFound shape flips to a generic errors.New for both MarkEnqueued/MarkRejected,
//   - ErrStateConflict becomes wrapped in fmt.Errorf differently from production,
//   - the sentinel-alias (ErrLedgerStateConflict = sqlassets.ErrStateConflict)
//     is replaced by a fresh errors.New (TestSentinelWrap_Conflict + TestSentinelWrap_InfraReturnsCanonical
//     both flip false),
//   - retry.IsTransient's classifier loses substring coverage for the
//     7 transient shapes listed in the table.
