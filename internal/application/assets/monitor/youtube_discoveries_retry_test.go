// Package monitor — youtube_discoveries_retry_test.go: Commit 3/6
// (June 2026, PR-C YouTube Channel Monitor cutover) P1 #5/#6/#7 retry
// semantics + backoff curve + DateAfter bridge + MarkRejected flag-lock.
//
// Step 8 split (July 2026): the retry-semantics subset of the 1555-LOC
// youtube_discoveries_test.go canonical. The dedupe contract pre-Commit-3/6
// tests live in youtube_discoveries_contract_test.go; concurrent
// TryReserve tests in youtube_discoveries_concurrent_test.go; typed
// transitions + sentinel-alias in youtube_discoveries_typed_transitions_test.go.
//
...
//     attempt_count unchanged, no next_retry_at pinned.
//
//   - Policy version bump produces a FRESH ledger row alongside the
//     historical one (UNIQUE includes policy_version).
//
//   - Backoff curve: min(30 * 2^(attempt - 1), 300), capped at 300s
//     to avoid exponential infinity parking.
//
//   - ResolveDateAfter: RFC3339 LastCursor truncates to YYYYMMDD,
//     lookbackDays falls back when LastCursor empty.
//
//     Pre-existing failing tests inside the monitor package
//     (e.g. TestAnalyzeVideo_ScoreBelowThresholdSoftSkips) are NOT
//     moved by this split; they live alongside in analyzer_test.go
//     and will block `go test ./internal/application/assets/monitor/...`
//     on a pre-existing SIGSEGV (analyzer.go:114) until fixed separately.
//
// These tests FAIL on Commit 3/6 regression only if:
//   - the MarkRejected retryable flag is swapped (terminal when retryable,
//     retryable when terminal);
//   - the backoff curve diverges from `min(30 * 2^(attempt-1), 300)`;
//   - next_retry_at pinning is dropped on retryable path;
//   - PolicyVersion_Bump uses adversarial sha256 input that omits policy_version;
//   - ResolveDateAfter's RFC3339→YYYYMMDD conversion is dropped;
//   - the MarkRejected retryable path increments attempt_count on the wrong side.
