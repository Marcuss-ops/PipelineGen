// Package monitor — youtube_discoveries_concurrent_test.go: FASE 1.1
// (July 2026) atomic TryReserve concurrent tests. Step 8 split:
// extracted from the 1555-LOC youtube_discoveries_test.go canonical
// and placed alongside 3 sibling FASE leaf files.
//
...
// tests live in youtube_discoveries_retry_test.go; the typed transitions
// + sentinel-alias live in youtube_discoveries_typed_transitions_test.go.
...
//   - the WHERE clause on `tryReserveConflict` is dropped or relaxed
//     (a single goroutine may win but multiple are still allowed to claim),
//   - the SELECT-then-UPDATE pre-fix shape is reintroduced,
//   - attempt_count increment is moved off the winning UPDATE path,
//   - lease_until is computed in production code with off-by-one drift,
//   - SetMaxOpenConns(1) pool isolation is broken (goroutines see separate
//     in-memory databases).
