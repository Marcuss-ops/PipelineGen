// Package drive — stager_adapter_test.go (ART-002 P4.2, July 2026).
//
// 1 TDD test pinning the DriveStager skeleton behavior:
//
//   - TestDriveStager_StageSource_NotImplemented: every StageSource
//     call returns ErrDriveStagerNotImplemented reachable via
...
// truth that the Drive provider has not landed. When the real adapter
...
func TestDriveStager_StageSource_NotImplemented(t *testing.T) {
	s := NewDriveStager()
	ctx := context.Background()
	ref := assets.SourceRef{URL: "https://drive.google.com/file/d/xyz/view"}

	staged, err := s.StageSource(ctx, ref)
	if err == nil {
		t.Fatalf("expected error from DriveStager.StageSource, got staged=%+v", staged)
	}
	if staged != nil {
		t.Errorf("expected nil staged on error, got %+v", staged)
	}
	if !errors.Is(err, ErrDriveStagerNotImplemented) {
		t.Errorf("expected errors.Is(err, ErrDriveStagerNotImplemented) to be true, got err=%v", err)
	}
