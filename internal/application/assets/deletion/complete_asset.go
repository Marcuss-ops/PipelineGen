...
func (s *DeletionService) CompleteAsset(ctx context.Context, assetID string) error {
	...
	if s.repositories.Clips == nil {
		return fmt.Errorf("complete asset: clips repository not wired")
	}
	clip, err := s.repositories.Clips.Get(ctx, assetID)
	if err != nil {
		logger.Warn("complete asset: pre-flight Get failed", zap.String("asset_id", assetID), zap.Error(err))
	...
	if !isPastDriveDeleted(currentState) {
		return fmt.Errorf("complete asset: drive_file_alive_block guard fired for state %q", currentState)
	}

	fileID := extractDriveFileID(clip)
	if fileID != "" && s.completion.DriveGone != nil {
		gone, err := s.completion.DriveGone.CheckDriveGone(ctx, fileID)
		if err != nil {
			return fmt.Errorf("complete asset: Drive-gone check for %q failed: %w", fileID, err)
		}
		if !gone {
			return fmt.Errorf("complete asset: drive_file_alive_guard_recheck fired for %q", fileID)
		}
	}

	if s.completion.Tx == nil {
		return fmt.Errorf("complete asset: completion transaction not wired")
	}
	if err := s.completion.Tx.RunCompletionTx(ctx, assetID); err != nil {
		return fmt.Errorf("complete asset: atomic cleanup TX for %q failed: %w", assetID, err)
	}
	logger.Info("asset_completed", zap.String("asset_id", assetID), zap.String("file_id", fileID))
	return nil
}
