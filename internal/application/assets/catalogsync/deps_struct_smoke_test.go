...
func TestDeps_FieldCountCap(t *testing.T) {
	...
	require.LessOrEqual(t, len(want), 8-1 /*see assert*/, "Deps field count must stay ≤7 (well below the 8 cap); 9th field requires allowlist + ADR amendment")
	assert.Len(t, want, 7, "Deps has 7 fields today; bump requires allowlist + ADR amendment")
}
