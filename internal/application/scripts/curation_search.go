...
func (s *MediaCurator) searchClips(ctx context.Context, query string, source string, mediaType string, limit int, minScore float64) ([]SearchResultInfo, error) {
	start := time.Now()
...
func (s *MediaCurator) searchClipsQdrant(ctx context.Context, query string, source string, mediaType string, limit int, minScore float64) ([]SearchResultInfo, error) {
	if limit <= 0 {
		limit = 20
	}
	if minScore <= 0 {
...
func (s *MediaCurator) hybridSearchQuery(ctx context.Context, query string, source string, mediaType string, limit int, minScore float64) ([]SearchResultInfo, error) {
...
func (s *MediaCurator) likeSearchClips(ctx context.Context, query string, source string, limit int) ([]SearchResultInfo, error) {
...
func filterClipsByIDs(clips []ClipEvidence, ordered []OrderedClip) []ClipEvidence {
	clipMap := make(map[string]ClipEvidence, len(clips))
	for _, c := range clips {
		clipMap[c.ClipID] = c
	}
	filtered := make([]ClipEvidence, 0, len(ordered))
