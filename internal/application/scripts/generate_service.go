// Package scripts provides script generation orchestration (use case layer).
// Merged from internal/application/scriptflow/generate/.
package scripts
...
func (s *GenerationService) EnqueueFromClips(ctx context.Context, spec script.GenerationSpec) (*FromClipsResult, error) {
	...
	if err := genValidate(&spec); err != nil {
		return nil, err
	}

	genApplyPreset(&spec, scriptsCfg, script.PresetCustom)

	if spec.GenerateMetadata {
		spec.ExtractEntities = true
	}

	return s.genEnqueue(ctx, spec, script.PresetCustom, "unified")
}
...
func (s *GenerationService) EnqueueWithImages(ctx context.Context, spec script.GenerationSpec) (*FromClipsResult, error) {
	...
	genApplyPreset(&spec, scriptsCfg, script.PresetWithImages)

	return s.genEnqueue(ctx, spec, script.PresetWithImages, "generate-with-images")
}
...
// genApplyPreset applies the preset-specific defaults and feature flags.
func genApplyPreset(spec *script.GenerationSpec, scriptsCfg config.ScriptsConfig, preset script.Preset) {
	// Common defaults.
	if spec.Language == "" {
	...
	title, outputName := genResolveTitleOutputName(spec.Title, spec.Topic)
	spec.Title = title
	spec.OutputName = outputName

	spec.TargetWords = genResolveTargetWords(spec.TargetWords, spec.MinWords, spec.Duration, scriptsCfg.MinWordFloor)
...
func genValidate(spec *script.GenerationSpec) error {
...
// genEnqueue builds the GeneratePayload and enqueues the
// script.generate_from_clips job with the typed payload.
func (s *GenerationService) genEnqueue(
	ctx context.Context,
...
func genResolveTitleOutputName(topicTitle, topic string) (resolvedTitle, outputName string) {
...
func genResolveTargetWords(targetWords, minWords, duration, minWordFloor int) int {
	tw := targetWords
	if tw <= 0 {
		tw = minWords
	}
