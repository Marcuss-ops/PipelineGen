// Package scripts provides scene-level image and voiceover generation for
// the script pipeline. Merged from internal/application/scriptflow/scenes/.
package scripts
...
// SceneImageService captures the image generation method the scenes package needs.
type SceneImageService interface {
...
// SceneVoiceoverService captures the voiceover generation method the scenes package needs.
type SceneVoiceoverService interface {
...
// ScenesService generates scene images and voiceovers for script pipelines.
type ScenesService struct {
	imgSvc        SceneImageService
	voSvc         SceneVoiceoverService
	log           *zap.Logger
	cfg           *config.Config
...
// NewScenesService creates a new ScenesService.
...
func (s *ScenesService) GenerateImages(
	ctx context.Context,
	spec *script.GenerationSpec,
	scr string,
	reportProgress ProgressReporter,
) []SceneImage {
	if s.imgSvc == nil || strings.TrimSpace(scr) == "" {
		return nil
	}
	...
	scenesText := splitScenesTextIntoImages(scr, sentencesPerImage)
	if len(scenesText) == 0 {
		return nil
	}
	...
	for idx, para := range scenesText {
		...
		group.Go(fmt.Sprintf("scene-image-%d", idx), func() error {
			...
			shortPrompt := scenesTruncatePrompt(para, 200)
...
func (s *ScenesService) GenerateVoiceovers(
	ctx context.Context,
	spec *script.GenerationSpec,
	scenes []SceneImage,
) []SceneVoiceover {
...
// splitScenesTextIntoImages splits a script into scenes of N sentences each.
func splitScenesTextIntoImages(scr string, sentencesPerImage int) []string {
	if sentencesPerImage <= 0 {
		sentencesPerImage = 10
	}
	sentences := scenesSplitIntoSentences(scr)
	if len(sentences) == 0 {
		return nil
	}
	var scenes []string
...
// scenesSplitIntoSentences splits text into sentences based on punctuation.
func scenesSplitIntoSentences(text string) []string {
...
// scenesTruncatePrompt extracts a short visual description from scene text.
// Takes the first 1-2 sentences and caps at maxLen characters.
func scenesTruncatePrompt(text string, maxLen int) string {
	sentences := scenesSplitIntoSentences(text)
	if len(sentences) == 0 {
		if len(text) > maxLen {
			return text[:maxLen]
		}
...
// scenesBuildVoiceoverDestination builds a *voiceover.DestinationRequest.
func scenesBuildVoiceoverDestination(
	ctx context.Context,
...
// ensure errors import is used by scenesBuildVoiceoverDestination.
var _ = errors.Is
