...
// and splitItemCount.
func (s *BatchService) parallelBatchWebSearch(ctx context.Context, req *GenerateBatchRequest, batchItems []BatchTopic) ([]batchWorkItem, []ScriptResearchSource, int, error) {
	...
	var researchSources []ScriptResearchSource
	splitItemCount := 0
	...
	for _, future := range futures {
		...
		for _, sr := range res.results {
			researchSources = append(researchSources, ScriptResearchSource{
				Query:      topicClean,
				URL:        sr.URL,
				Title:      sr.Title,
				Snippet:    sr.Content,
