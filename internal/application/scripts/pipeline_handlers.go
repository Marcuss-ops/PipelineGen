...
func (pu *PipelineUseCase) handleClipPathExplicit(ctx context.Context, payload *scriptpkg.GenerationSpec, tools *appjobs.JobTools) (*ClipSourcePathResult, error) {
	...
	clipScenes, err := parseStructuredOutput(writeResult.Script, payload.ClipIDs, payload.MaxChars)
	if err != nil {
		return nil, fmt.Errorf("clip-script structured output invalid: %w", err)
	}
...
// MaxChars > 0. It rejects prose, malformed JSON, and partial arrays so
// the caller can fail fast instead of silently accepting a bad format.
func parseStructuredOutput(script string, clipIDs []string, maxChars int) ([]ClipScene, error) {
	if maxChars <= 0 {
		return BuildScenesWithMarkers(script, nil), nil
	}
	...
	if err := json.Unmarshal([]byte(clean), &pairs); err != nil {
		if extracted, ok := extractJSONArray(clean); ok {
			if err := json.Unmarshal([]byte(extracted), &pairs); err != nil {
				return nil, fmt.Errorf("response is not valid JSON array: %w", err)
			}
		} else {
			return nil, fmt.Errorf("response is not valid JSON array: %w", err)
		}
	...
	if len(pairs) == 0 {
		return nil, fmt.Errorf("response JSON array is empty")
	}
	clipScenes := make([]ClipScene, 0, len(pairs))
	for i, p := range pairs {
		if strings.TrimSpace(p.ClipID) == "" {
			return nil, fmt.Errorf("response item %d is missing clip_id", i)
		}
		if len(clipIDs) > 0 {
			expected := strings.TrimSpace(clipIDs[i])
			if expected != "" && p.ClipID != expected {
				return nil, fmt.Errorf("response item %d has clip_id %q, expected %q", i, p.ClipID, expected)
			}
		}
		if strings.TrimSpace(p.Text) == "" {
			return nil, fmt.Errorf("response item %d has empty text", i)
		}
