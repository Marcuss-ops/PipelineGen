...
func (p *Pipeline) Run(ctx context.Context, spec interface{}, script string, tools interface{}) (*PipelineResult, error) {
	...
	return p.RunWithClipScenes(ctx, genSpec, script, nil, tools)
}
...
func (p *Pipeline) RunWithClipScenes(ctx context.Context, spec *scriptpkg.GenerationSpec, script string, clipScenes []ClipScene, _ interface{}) (*PipelineResult, error) {
	...
	if spec != nil && spec.CreateDoc {
		...
		if p.docsSvc == nil {
			...
			folderID := strings.TrimSpace(spec.DriveFolderID)
			if folderID == "" {
				folderID = strings.TrimSpace(p.docsSvc.defaultFolderID)
			}
			content := BuildScriptDocumentContent(title, script, clipScenes, result.EntitiesJSON)
			result.DocLink, result.DocID = p.docsSvc.CreateDoc(ctx, title, content, p.resolveFolder, folderID)
