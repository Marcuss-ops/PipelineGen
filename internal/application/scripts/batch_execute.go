...
func (s *BatchService) ExecuteBatchGeneration(ctx context.Context, req *GenerateBatchRequest, onProgress func(int, string)) (BatchGenerateResponse, error) {
	...
	if coherenceErr != nil {
		...
		sections = make([]ScriptSectionRecord, 0, len(GeneratedParts))
		for idx, part := range GeneratedParts {
			status := "completed"
			if part.timing.Status == "failed" {
				status = "failed"
			}
			sections = append(sections, ScriptSectionRecord{
				SectionType:  "item",
	...
	outlineSections := make([]ScriptOutlineSectionRecord, 0, len(batchItems))
	for i, item := range batchItems {
		...
		outlineSections = append(outlineSections, ScriptOutlineSectionRecord{
			SectionIndex:  i + 1,
			Title:         item.Topic,
			Purpose:       purpose,
	...
	generationLogs := make([]ScriptGenerationLog, 0, len(timings))
	for _, t := range timings {
		...
		generationLogs = append(generationLogs, ScriptGenerationLog{
			Phase:       phase,
			Model:       req.Model,
