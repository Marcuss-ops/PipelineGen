...
func TestClassify_NoDrift(t *testing.T) {
	...
	got := classify(sqlite, qdrant, schema, func(s string) string { return "pt-" + s })
	if len(got) != 0 {
...
func TestClassify_Orphan(t *testing.T) {
	...
	sqlite := map[string]AssetSnapshot{}
	qdrant := map[string]pointWithID{
		"orphan1": {ID: "pt-orphan1", Payload: map[string]interface{}{
			"asset_id": "orphan1",
		}},
	}
	got := classify(sqlite, qdrant, schema, func(s string) string { return "pt-" + s })
	...
	if got[0].QdrantPointID != "pt-orphan1" {
		t.Fatalf("expected QdrantPointID 'pt-orphan1', got %q", got[0].QdrantPointID)
	}
}
...
func TestClassify_NonCanonicalPointID_PriorityWins(t *testing.T) {
	...
	// Point UUID mismatches the canonical AssetIDToQdrantPointID(a1) = "pt-a1".
	qdrant := map[string]pointWithID{
...
func TestClassify_VersionStale_LegacyGlobalFallback(t *testing.T) {
	...
	got := classify(sqlite, qdrantMatchLegacy, schema, func(s string) string { return "pt-" + s })
	if len(got) != 0 {
	...
	got = classify(sqlite, qdrantMismatchLegacy, schema, func(s string) string { return "pt-" + s })
...
func TestClassify_LegacyKeys(t *testing.T) {
	...
	got := classify(sqlite, qdrantStatus, schema, func(s string) string { return "pt-" + s })
	...
	got = classify(sqlite, qdrantDriveLink, schema, func(s string) string { return "pt-" + s })
