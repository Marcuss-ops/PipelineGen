...
func NewClipDriveAssociation(repo *assets.ClipsRepository) *ClipDriveAssociation {
...
func (a *ClipDriveAssociation) Associate(ctx context.Context, input SegmentInput) ([]ScoredMatch, error) {
	...
	clipsList, err := a.repo.SearchClips(ctx, "youtube", searchTerm)
	if err != nil {
		clipsList, _ = a.repo.SearchClips(ctx, "youtube", input.Subject)
	}
