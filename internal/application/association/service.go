...
func (s *Service) BuildCandidates(ctx context.Context, req CandidatesRequest) (*CandidatesResponse, error) {
	...
	if folders, err := s.buildStockFolderCandidates(ctx); err == nil {
		rawCandidates = append(rawCandidates, scoreFolderCandidates("media.db.sqlite", "stock_folder", folders, terms, req.Subject, req.Topic)...)
	}

	if folders, err := s.buildArtlistFolderCandidates(ctx); err == nil {
		rawCandidates = append(rawCandidates, scoreFolderCandidates("media.db.sqlite", "artlist_folder", folders, terms, req.Subject, req.Topic)...)
	}
