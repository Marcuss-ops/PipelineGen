...
// disallowedSymbols enumerates the canonical direct-IO bindings the
// application layer MUST NOT use without a matching allowlist entry.
...
// TestNoDirectIOBindingsInApplicationLayer walks internal/application/
...
func TestNoDirectIOBindingsInApplicationLayer(t *testing.T) {
	...
	allowlistPath := filepath.Join(repoRoot, "internal", "application", "iobinder", "allowlist.txt")
	...
	violations, err := ScanDirectory(appDir, disallowedSymbols, ScannerConfig{
		Root:           repoRoot,
	...
	if len(newViolations) > 0 || len(stale) > 0 {
		...
		b.WriteString("I/O binder gate failed.\n")
		b.WriteString("\n")
		if len(newViolations) > 0 {
			sort.Strings(newViolations)
			b.WriteString("NEW direct-IO bindings (refactor to a typed port, or add to allowlist.txt with owner/deadline/rationale):\n")
