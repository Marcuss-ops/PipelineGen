...
func registerVoiceoverEntries(r *Registry) {
	...
	r.Register(JobPolicy{Type: TypeVoiceoverPromo, Description: "Voiceover promo generation (translate + generate) (per-item artifacts persisted inside the per-item caller-owned tx via Service.GeneratePromo → promo.NewGenerator → promoVoiceoverAdapter → ProcessVoiceoverItemUseCase.Execute → ProcessSegmentUseCase.Execute → voiceover.Finalizer.Finalize → tx.Commit; broker's legacy Complete is the canonical mark-SUCCEEDED seam)", Timeout: 30 * time.Minute, DefaultMaxRetries: 2})
