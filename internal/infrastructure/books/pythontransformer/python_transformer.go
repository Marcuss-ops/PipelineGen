...
// extractGoogleDocID is a helper kept here for any future
// transformer that pre-resolves Google Doc URLs to a drive file
// ID. The pre-Fase-7 books/service.go::extractGoogleDocID has
// been moved here verbatim so the apply-layer Service.ProcessBook
// can still derive --google-doc-id from a raw URL form when the
// caller passes a URL rather than a bare ID.
