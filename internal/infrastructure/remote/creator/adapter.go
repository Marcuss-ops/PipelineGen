...
type jobBrokerClient interface {
	PrepareArtifactUpload(ctx remote.PrepareContext) (*remote.UploadSession, error)
	UploadArtifactFile(ctx remote.PrepareContext, sessionID, localPath, idempotencyKey string) (*remote.UploadSession, error)
	FinalizeArtifactUpload(ctx remote.PrepareContext, sessionID, sha256Hex, idempotencyKey string) (*remote.UploadSession, error)
}
...
func (a *Adapter) Prepare(ctx remote.PrepareContext) (*remote.UploadSession, error) {
	...
	derivedKey := remote.ArtifactIdempotencyKey(ctx.JobID, ctx.ArtifactID, ctx.SHA256)
	if derivedKey == "" {
	...
	ctx.IdempotencyKey = derivedKey

	session, err := a.broker.PrepareArtifactUpload(ctx)
	if err != nil {
...
func (a *Adapter) Upload(ctx remote.PrepareContext, session remote.UploadSession, localPath string) (*remote.UploadSession, error) {
	if a == nil || a.broker == nil {
	...
	// Defensive idempotency-key derivation: if ctx.IdempotencyKey
	...
	idemKey := ctx.IdempotencyKey
	if idemKey == "" {
		derived := remote.ArtifactIdempotencyKey(ctx.JobID, ctx.ArtifactID, ctx.SHA256)
		if derived == "" {
			return nil, fmt.Errorf("creator adapter: idempotency-key derivation returned empty (godlike/07 no-fake-availability): %w",
				remote.ErrArtifactIdempotencyKeyConflict)
		}
	...
	updatedSession, err := a.broker.UploadArtifactFile(ctx, session.ID, localPath, idemKey)
	if err != nil {
...
func (a *Adapter) Finalize(ctx remote.PrepareContext, session remote.UploadSession) (*remote.UploadSession, error) {
	if a == nil || a.broker == nil {
		return nil, fmt.Errorf("creator adapter: not configured: %w",
			remote.ErrArtifactUploaderNotConfigured)
	}
	...
	updatedSession, err := a.broker.FinalizeArtifactUpload(ctx, session.ID, ctx.SHA256, idemKey)
	if err != nil {
