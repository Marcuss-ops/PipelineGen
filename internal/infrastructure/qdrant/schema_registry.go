...
// SchemaRegistry holds the canonical IndexSchema instances for every
...
// the previous facade `qdrant.DefaultV3Schema()` that every boot
...
// and every subsystem receives the resolved *IndexSchema from the
...
// canonical construction path; legacy DefaultV3Schema() calls stay
...
func (r *SchemaRegistry) Register(schema *IndexSchema) {
	if schema == nil {
...
// from a nil deref.
func (r *SchemaRegistry) Resolve(version string) (*IndexSchema, error) {
	if version == "" {
		version = "v3"
	}
...
func (r *SchemaRegistry) MustResolve(version string) *IndexSchema {
...
func init() {
	DefaultSchemaRegistry.Register(DefaultV3Schema())
...
// DefaultV3Schema by adding a dedicated `speaker` dense vector
// channel (256-dim, Cosine) for accent / speaker-diarisation use
// cases that the canonical v3 doesn't carry. CompareSchema() reports
...
func DefaultV3SpeakerSchema() *IndexSchema {
	s := DefaultV3Schema() // copy base
	s.Version = "v3-multilingual-speaker"
	s.PhysicalName = "media_assets_v3_e5_768_siglip_768_speaker_256"
	s.DenseVectors = append(s.DenseVectors, EmbeddingSpec{
		Channel:       "speaker",
