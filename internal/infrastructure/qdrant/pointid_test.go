...
func TestPipelineGenQdrantNamespace_ValidUUID(t *testing.T) {
	got := PipelineGenQdrantNamespace.String()
	if len(got) != 36 || strings.Count(got, "-") != 4 {
		t.Fatalf("PipelineGenQdrantNamespace is not a canonical UUID string: %q", got)
	}
}
...
// the empty string. The caller (PayloadMapper.AssetToPoint) is
...
// PipelineGenQdrantNamespace cannot collide with hashes under any
