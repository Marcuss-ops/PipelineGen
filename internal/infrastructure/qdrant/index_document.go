...
//     the per-channel counters in SwitchReport.VersionMismatchPerChannel
...
//     <doc.SearchText>, model: <DefaultSparseModel>}` on the wire.
...
// (DefaultV3Schema().PayloadIndexes) and hydration on read-back.
//
...
type IndexedMetadata struct {
	...
	// legacy discriminator — see DefaultV3Schema.PayloadIndexes).
	YouTubeChan string
...
// emitted to Qdrant; the schema's EmbeddingSpec.ModelVersion is what
...
// Point via IndexDocumentToPoint (see payload_mapper.go).
//
...
type IndexDocument struct {
	...
	// AssetIDToQdrantPointID at the very last step before the
