...
func TestSchemaRegistry_ResolveBootProbe(t *testing.T) {
	...
	gotV3, err := DefaultSchemaRegistry.Resolve("v3")
	if err != nil {
		t.Fatalf("DefaultSchemaRegistry.Resolve(\"v3\") = nil err, want nil; got %v", err)
	}
	if gotV3 == nil {
		t.Fatalf("DefaultSchemaRegistry.Resolve(\"v3\") returned nil schema; want *IndexSchema")
	}
	if gotV3.Version != "v3" {
		t.Errorf("Resolve(\"v3\") returned Version=%q, want \"%s\"", gotV3.Version, "v3")
	}

	gotSpeaker, err := DefaultSchemaRegistry.Resolve("v3-multilingual-speaker")
	if err != nil {
		t.Fatalf("DefaultSchemaRegistry.Resolve(\"v3-multilingual-speaker\") returned err = %v; want nil", err)
	}
	if gotSpeaker == nil {
		t.Fatalf("DefaultSchemaRegistry.Resolve(\"v3-multilingual-speaker\") returned nil schema; want *IndexSchema")
	}
	...
	gotEmptyDefault, err := DefaultSchemaRegistry.Resolve("")
	if err != nil {
		t.Fatalf("DefaultSchemaRegistry.Resolve(\"\") returned err = %v; want nil (boot compatibility)", err)
	}
	if gotEmptyDefault == nil || gotEmptyDefault.Version != "v3" {
		t.Errorf("DefaultSchemaRegistry.Resolve(\"\") returned Version=%v, want v3 (boot compat fallback)", gotEmptyDefault)
	}

	// (3) Unknown version returns ErrSchemaVersionNotFound.
	_, err = DefaultSchemaRegistry.Resolve("nonexistent-version-zzz")
	if err == nil {
		t.Fatalf("DefaultSchemaRegistry.Resolve(\"nonexistent\") returned nil err; want ErrSchemaVersionNotFound")
	}
	if !errors.Is(err, ErrSchemaVersionNotFound) {
		t.Errorf("Resolve(\"nonexistent\") returned err = %v; want errors.Is(err, ErrSchemaVersionNotFound)", err)
	}

	// (4) Versions() returns ≥2 entries.
	vs := DefaultSchemaRegistry.Versions()
	if len(vs) < 2 {
		t.Errorf("DefaultSchemaRegistry.Versions() returned %d entries; need ≥2 to demonstrate the mechanism is not a single-value facade", len(vs))
	}
}

// channelsOf returns the list of dense vector channel names in s.
// Helper for the test assertion message.
func channelsOf(s *IndexSchema) []string {
