...
const (
	...
	defaultCollectionAliasSuffix = "_current"
...
type aliasUpdateAction struct {
	CreateAlias *struct {
		CollectionName string `json:"collection_name"`
		AliasName      string `json:"alias_name"`
	} `json:"create_alias,omitempty"`
	DeleteAlias *struct {
		AliasName string `json:"alias_name"`
	} `json:"delete_alias,omitempty"`
}

type aliasUpdateRequest struct {
	Actions []aliasUpdateAction `json:"actions"`
}
...
func (s *Service) EnsureCollection(ctx context.Context) error {
	...
	collection := s.physicalCollectionName()
	...
	if err == nil {
		...
		if resp.StatusCode == http.StatusOK {
			...
			return s.ensureCollectionAlias(ctx, collection)
		}
...
func (s *Service) collectionName() string {
	...
	if alias := s.aliasName(); alias != "" && !s.config.DisableAlias {
		return alias
	}
	return s.physicalCollectionName()
}

func (s *Service) physicalCollectionName() string {
	if s == nil {
		return defaultCollection
	}
	base := s.collectionBaseName()
	if version := strings.TrimSpace(s.config.CollectionVersion); version != "" {
		return base + "_" + version
	}
	return base
}

func (s *Service) collectionBaseName() string {
	if s == nil {
		return defaultCollection
	}
...
func (s *Service) aliasName() string {
	if s == nil {
		return ""
	}
	if s.config.DisableAlias {
		return ""
	}
	if alias := strings.TrimSpace(s.config.CollectionAlias); alias != "" {
		return alias
	}
	if strings.TrimSpace(s.config.CollectionVersion) != "" {
		return s.collectionBaseName() + defaultCollectionAliasSuffix
	}
	return ""
}

func (s *Service) ensureCollectionAlias(ctx context.Context, physical string) error {
	alias := s.aliasName()
	if alias == "" || alias == physical {
		return nil
	}
	current, err := s.currentAliasTarget(ctx, alias)
	if err != nil {
		return err
	}
	if current == physical {
	...
	actions := []aliasUpdateAction{}
	if current != "" {
		actions = append(actions, aliasUpdateAction{
			DeleteAlias: &struct {
				AliasName string `json:"alias_name"`
			}{AliasName: alias},
		})
	...
	actions = append(actions, aliasUpdateAction{
		CreateAlias: &struct {
			CollectionName string `json:"collection_name"`
			AliasName      string `json:"alias_name"`
		}{CollectionName: physical, AliasName: alias},
	})
	resp, err := s.do(ctx, http.MethodPost, "/collections/aliases", aliasUpdateRequest{Actions: actions}, true)
	if err != nil {
		return err
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return readHTTPError(resp, "update aliases")
	}
	return nil
}

func (s *Service) currentAliasTarget(ctx context.Context, alias string) (string, error) {
	resp, err := s.do(ctx, http.MethodGet, "/aliases", nil, false)
	if err != nil {
		return "", err
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return "", readHTTPError(resp, "list aliases")
	}
	var decoded aliasListResponse
	if err := json.NewDecoder(resp.Body).Decode(&decoded); err != nil {
		return "", fmt.Errorf("decode alias list: %w", err)
	}
	for _, entry := range decoded.Result.Aliases {
...
func (s *Service) ensurePayloadIndexes(ctx context.Context, collection string) error {
	fields := []string{
		"asset_id",
		"source",
		"category",
	...
	for _, field := range fields {
		resp, err := s.do(ctx, http.MethodPut, "/collections/"+collection+"/index?wait=true", payloadIndexRequest{
			FieldName:   field,
			FieldSchema: "keyword",
		}, true)
		if err != nil {
		...
		if resp.StatusCode != http.StatusOK {
			resp.Body.Close()
			return readHTTPError(resp, "create payload index")
		}
		resp.Body.Close()
	}
	return nil
