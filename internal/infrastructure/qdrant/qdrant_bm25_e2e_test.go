...
//
//  2. Pre-PR2 legacy raw-vector path: SparseQueryVector with
...
func TestPR2_BM25_HybridEndToEnd_ServerSideText(t *testing.T) {
	...
	if err := client.UpsertPoints(context.Background(), "test", []Point{
		{
			ID: "test-asset-id",
	...
	hits, err := client.HybridSearchPoints(context.Background(), "test", HybridSearchRequest{
		DenseVector:      smallDenseVector(),
		DenseVectorName:  "text",
		SparseVectorName: "bm25_text",
		SparseText:       "quick fox",
		SparseModel:      DefaultSparseModel,
		Limit:            10,
	})
	if err != nil {
...
// vector path (SparseQueryVector with Indices+Values). Reserved for
...
func TestPR2_BM25_HybridEndToEnd_LegacyRawVector(t *testing.T) {
	...
	hits, err := client.HybridSearchPoints(context.Background(), "test", HybridSearchRequest{
		...
		SparseText: "",
		SparseQueryVector: &SparseQueryVector{
...
// (PR2 path) nor SparseQueryVector (legacy path) MUST return a
...
func TestPR2_BM25_HybridRejects_MissingBothSparseSources(t *testing.T) {
	...
	_, err := client.HybridSearchPoints(context.Background(), "test", HybridSearchRequest{
		DenseVector:      smallDenseVector(),
		DenseVectorName:  "text",
		SparseVectorName: "bm25_text",
		// Both SparseText and SparseQueryVector are nil → fail closed.
		Limit: 10,
	})
	if err == nil {
		t.Fatal("expected typed error when both SparseText and SparseQueryVector are empty; got nil")
	}
