...
func TestComputeReady_EachGateBlocks(t *testing.T) {
	...
	tests := []struct {
		name   string
		mutate func(r *SwitchReport)
	}{
		{"CompleteScan false", func(r *SwitchReport) { r.CompleteScan = false }},
		{"count mismatch", func(r *SwitchReport) { r.ActualPoints = 2 }},
		{"zero expected", func(r *SwitchReport) {
			r.ExpectedPoints = 0
			r.ActualPoints = 0
		}},
		{"has missing", func(r *SwitchReport) { r.MissingCount = 1 }},
		{"has orphan", func(r *SwitchReport) { r.OrphanCount = 1 }},
		{"has payload issues", func(r *SwitchReport) { r.PayloadIssues = 1 }},
		{"has version mismatch", func(r *SwitchReport) {
			r.VersionMismatchPerChannel["text"] = 1
		}},
		{"has non-canonical", func(r *SwitchReport) { r.NonCanonicalPointCount = 1 }},
		{"has dead letters", func(r *SwitchReport) { r.DeadLetterOpen = 1 }},
		{"golden smoke failed", func(r *SwitchReport) { r.GoldenQueriesOK = false }},
		{"filter smoke failed", func(r *SwitchReport) { r.FiltersOK = false }},
		{"has errors", func(r *SwitchReport) { r.Errors = append(r.Errors, "err") }},
	}
...
// TestCheckDeadLetters_NilChecker verifies that a nil DeadLetterChecker
