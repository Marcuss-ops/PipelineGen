// Package drive — publisher_test_helpers_test.go: shared test
// infrastructure for the 6 publisher test files. Step 6 split
// (July 2026): extracted fakeFolderManager + fakeFileUploader +
// folderCall + uploadCall + testRegistry from the canonical
// publisher_test.go. The 6 leaf files (basic + p1_1_policies +
// p0_1_audit + pathbuilder + vo_subfolder + dod_matrices) ALL
// reuse these stubs via package-level scope (Go test files in a
// single package share symbols automatically).
//
// godlike/06 SSOT: this helpers file is the SOLE canonical surface
// for the test doubles that gate publisher tests. Splitting into
// per-file duplicates would re-introduce the drift risk the original
// single-file layout avoided. The fakeFolderManager satisfies both
// FolderManagerPort (EnsureFolder + ProbeFolderAccess) and the
// fakeFileUploader satisfies FileUploaderPort (PutFile).
//
//   - publisher_basic_test.go          (FASE pre-Commit-3/6 + error paths + nil-deps — 14 tests)
//   - publisher_p1_1_policies_test.go  (Commit 3/6 P1.1 ConflictPolicy plumbing — 8 tests)
//   - publisher_p0_1_audit_test.go     (P1.5/P0.9 enrichment + P0.1 audit pins — 4 tests)
//   - publisher_pathbuilder_test.go    (PR-VO-ERR-PATHBUILDER + PR-VO-SUBFOLDER + DoD 9 pathbuilder — 11 tests)
//   - publisher_vo_subfolder_test.go   (PR-VO-3-LANGUAGE-MATRIX — 1 test)
//   - publisher_dod_matrices_test.go   (DoD 10 — 2 tests)
//
// Total across 6 leaf files: 14 + 8 + 4 + 11 + 1 + 2 = 40 tests
// (matches the count preserved across the split per godlike/06
// SSOT audit-trail invariant).
package drive
