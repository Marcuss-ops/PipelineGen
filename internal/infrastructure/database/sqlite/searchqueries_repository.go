...
func (r *SearchQueriesRepository) Upsert(ctx context.Context, q *media.SearchQuery) error {
...
func (r *SearchQueriesRepository) ListAll(ctx context.Context) ([]*media.SearchQuery, error) {
...
func (r *SearchQueriesRepository) ListActive(ctx context.Context) ([]*media.SearchQuery, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT id, query, category, drive_folder_id, min_score, max_results,
			check_interval, last_run_at, last_video_published_at, is_active,
			created_at, updated_at
...
func (r *SearchQueriesRepository) GetByID(ctx context.Context, id string) (*media.SearchQuery, error) {
...
func (r *SearchQueriesRepository) InsertResult(ctx context.Context, res *media.SearchQueryResult) error {
	_, err := r.db.ExecContext(ctx, `
...
func (r *SearchQueriesRepository) ListResultsByQuery(ctx context.Context, queryID string) ([]*media.SearchQueryResult, error) {
	rows, err := r.db.QueryContext(ctx, `
...
func scanQueries(rows *sql.Rows) ([]*media.SearchQuery, error) {
	var results []*media.SearchQuery
	for rows.Next() {
...
func scanQuery(row *sql.Row) (*media.SearchQuery, error) {
	return scanQueryFields(row)
}

func scanQueryFields(scanner interface {
	Scan(dest ...any) error
}) (*media.SearchQuery, error) {
	q := &media.SearchQuery{}
...
func scanResults(rows *sql.Rows) ([]*media.SearchQueryResult, error) {
	var results []*media.SearchQueryResult
	for rows.Next() {
...
func scanResult(scanner interface {
	Scan(dest ...any) error
}) (*media.SearchQueryResult, error) {
	r := &media.SearchQueryResult{}
