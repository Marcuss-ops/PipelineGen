...
func (r *Repository) Register(ctx context.Context, cmd job.RegisterWorkerCommand) (*job.WorkerSession, error) {
	now := time.Now().UTC()
...
func (r *Repository) Heartbeat(ctx context.Context, workerID, sessionID string, ttl time.Duration) (*job.WorkerSession, error) {
	now := time.Now().UTC()
...
func (r *Repository) Get(ctx context.Context, workerID string) (*job.WorkerSession, error) {
...
func (r *Repository) IsSessionActive(ctx context.Context, workerID, sessionID string) (bool, error) {
	row := r.db.QueryRowContext(ctx, `
