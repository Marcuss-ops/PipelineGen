...
func (r *ScriptRepository) GetSectionByID(ctx context.Context, id int64) (*scriptSectionRow, error) {
	...
	).Scan(&s.ID, &s.ScriptID, &s.SectionType, &s.SectionTitle, &s.Content, &s.SortOrder, &s.WordCount, &s.Status)
	if err != nil {
		return nil, err
	}
	return &s, nil
...
func (r *ScriptRepository) GetAdjacentSections(ctx context.Context, scriptID int64, currentSortOrder int) (prev *scriptSectionRow, next *scriptSectionRow, err error) {
	...
	).Scan(&p.ID, &p.ScriptID, &p.SectionType, &p.SectionTitle, &p.Content, &p.SortOrder, &p.WordCount, &p.Status)
	if err == nil {
		prev = &p
	...
	err = r.db.QueryRowContext(ctx,
		...
		scriptID, currentSortOrder,
	).Scan(&n.ID, &n.ScriptID, &n.SectionType, &n.SectionTitle, &n.Content, &n.SortOrder, &n.WordCount, &n.Status)
	if err == nil {
		next = &n
	} else if err != sql.ErrNoRows {
		return nil, nil, err
