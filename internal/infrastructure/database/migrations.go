...
//
// scope-aware migrations (June 2026) — implemented per
// architecture/current.yaml#PR-TODO-1-CLIP-DISPATCH CUTOVER
// prerequisite. The runner reads each file's first non-blank SQL
// comment line for the optional header
...
//
// scope-aware migrations (June 2026) — targetDB names the
...
// parameter's meaning.
//
// scope-aware migrations (June 2026).
...
// meaning.
//
// scope-aware migrations (June 2026) — the loop below skips
...
func migrateAll(db queryable, log *zap.Logger, targetDir, targetDB string) error {
	...
	for _, m := range migrations {
		// scope-aware (June 2026) — skip out-of-scope migrations BEFORE the
		...
		if prev, ok := applied[m.version]; ok {
			if prev.checksum != checksum {
				// scope-aware (June 2026) — one-time checksum upgrade
				...
				if m.version == 109 && targetDB == "primary" {
					// scope-aware (June 2026) — the shim ONLY fires when
					...
					if !strings.Contains(string(content), shimMarker) {
						return fmt.Errorf(
							"storage: migration %03d (%s) checksum mismatch — "+
								"missing scope-aware shim marker %q in file. The shim "+
