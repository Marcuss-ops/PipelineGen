...
### Added
- **[Audit Blocco 3.2 closure — DeletionReconciler for stuck media_assets, July 2026]** `feat(reconciler)` + `fix(dispatcher)` — periodic ticker that re-emits the canonical outbox event for any `media_assets` row stuck in `{DELETE_REQUESTED, DRIVE_DELETE_PENDING, INDEX_DELETE_PENDING}` past a configurable threshold (default 30min). Two-commit chain on `origin/main` (per AGENTS.md Pattern 2 EXPAND/BACKFILL/CUTOVER/CONTRACT discipline):

- **Commit 1/2 (test pinning + `updated_at` prerequisite)** — `42a2e5aa feat(test-pin): lifecycle IsValidTransition + EnqueueDriveDelete body + DriveDeleteHandler pre-flight table + updated_at prerequisite (Blocco 3.2 commit 1/2)`: closes the `updated_at` SQLite-defaulting assumption that would have blocked the Reconciler threshold query in commit 2/2. 3 NEW test files pin the state machine + envelope + handler pre-flight; 2 MODIFIED production files add explicit `updated_at = ?` stamping on state-flip (SQLite does NOT auto-update `updated_at` on UPDATE without an explicit trigger — the prior docstring claim was false-availability per godlike/07).
- **Commit 2/2 (DeletionReconciler implementation + wiring)** — `35d9e1e9 feat(reconciler): DeletionReconciler for stuck media_assets (Blocco 3.2 commit 2/2)`: 8 NEW files (5 in `internal/application/assets/deletion/reconciler/` covering types/ports/scanner/service/12 tests; 3 in `internal/infrastructure/database/sqlite/deletion/` covering Scanner adapter + 5 tests + Pattern-0 metrics adapter) + 4 MODIFIED production wiring files (`dispatcher_delete.go` gains `EnqueueIndexDelete` method with explicit CIRCUIT-BREAKER docstring; `metrics_media.go` adds 5 `deletion_reconciler_*` counters + drops orphan unexported adapter duplicate; `platform/config/media.go` adds `JobsConfig.DeletionReconcilerInterval` + `DeletionReconcilerStuckThreshold` + matching env vars; `internal/app/lifecycle.go` adds deletion-reconciler StartupStep under `runMaintenance` with graceful ctx-cancel).

**Design decisions (Pattern 0 single-owner per fact):**

1. **Port alignment**: `OutboxEnqueuer` interface signatures align 1:1 with production `*outbox.Dispatcher` methods (`EnqueueDriveDelete(ctx, assetID, permanently)` + `EnqueueIndexDelete(ctx, assetID)`). No thin shim, no `*Stamp` suffix. Reconciler's requeue_drive path always passes `permanently=false` (safe-fallback: Trash route) — reconciliation is by definition a recovery operation; original user-intent is not preserved on a stuck row (operator override via untrash is the correct manual workflow). Safe-fallback is **locked by a test assertion** (`enq.driveCalls[0].permanently != false`) — a future refactor that drops the safe-fallback fails `TestReconcileOnce_HappyPathMixedActions` per godlike/07 no-fake-availability.

2. **Metrics adapter collapse**: canonical Pattern 0 owner is `ReconcilerMetricsAdapter{}` in `internal/infrastructure/database/sqlite/deletion/metrics_adapter.go`. The previously-orphaned unexported `deletionReconcilerMetricsAdapter` in `internal/infrastructure/observability/metrics_media.go` was removed (two concretes satisfying one port = Pattern 0 single-owner violation). The observability package now only declares the package-level Prometheus counter variables consumed via the canonical adapter.

3. **`EnqueueIndexDelete` CIRCUIT-BREAKER**: the method re-stamps `updated_at = <now>` on re-emit so the row exits the stuck-threshold window immediately. This is a deliberate rate-limit on retries (avoids hot-looping on a permanently-failing Drive API). Trade-off: if the worker processes the event PARTIALLY (e.g. Qdrant succeeded, SoftDelete write crashed mid-flight), the row remains in `INDEX_DELETE_PENDING` with fresh `updated_at` and won't be re-emitted for `threshold` minutes — accepted trade-off; partial failures are caught by the IndexDeleteHandler's idempotent `clip_id` ledger on the next genuine re-emission. The `updated_at` re-stamp behavior is NOT yet pinned by a per-method test (forward-pointer: folds into a future `dispatcher_delete_test.go` addition per godlike/07 audit-honesty principle).

**Tests passing (this closure)**: 12 reconciler unit tests + 5 stuck_row_scanner adapter tests + 3 commit-1 follow-up verification tests = 20 tests, all green.

**Pre-existing build drift surfaced post-rebase (out of scope, NOT introduced by Blocco 3.2 commits)**: `internal/application/transcripts/cache.go:116` — `c.Inner.GetTranscript undefined (type monitor.TranscriptProvider has no field or method GetTranscript)`. Root cause: upstream `60a1f922 refactor(monitor): Step 6 deep CONTRACT cleanup` removed the legacy `GetTranscript`/`Score`/`Classify`/`FindSegments` methods from the monitor's `TranscriptProvider` interface. Files I have NOT touched; remediation lands in a separate transcript-cache adapter consolidation wave.
