...
- **[P0-COMPL-5-WIRE-NAMING (4030081a + bb4a19e8, 2026-07-03)]** `refactor(api+jobs)+feat(completion)` — wire-format rename + canonical `PublishAndCompleteUseCase` single-backbone orchestrator + 3 round-trip TDD tests (4030081a). Followup typo fixup in wire-naming drift (bb4a19e8). Cross-reference: `architecture/current.yaml#P0-COMPL-5-WIRE-NAMING`. No follow-up; the `StagedArtifacts` rename (P0-COMPL-3) is a separate forward-pointer (PENDING 2026-08-01).
...
- **[P0-COMPL-3 STAGED — PENDING 2026-08-01 (forward-pointer registered, 2026-07-03)]** `fix(completion)` — wave-tracker entry registered for the artifact-contract rename (`PublishedArtifacts` → `StagedArtifacts` in the HTTP wire-format). No implementation commit on origin/main today; PENDING per COMPLETION-CUTOVER-P0 action plan. Closure entry registered WITHOUT `ship_sha` per godlike/07 no-fake-availability — flippable to `status: shipped` only via the implementer PR. Cross-reference: `architecture/current.yaml#P0-COMPL-3-STAGED`.
