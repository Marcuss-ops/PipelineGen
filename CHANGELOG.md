...
- **[ART-001 — FASE-6 reversal: Artlist routes restored (POST /run, GET /stats, GET /search, GET /search/live, GET /diagnostics, POST /recommend, POST /sync-catalogs return 2xx, July 2026)]** `feat(artlist)` — restore `internal/app/registry_internal_modules.go::registerArtlist` (was log-only stub returning `/api/artlist/*` = 404 since FASE-6 cutover, per `git show origin/main`'s `// wireArtlistModule stubbed`) to invoke the canonical `artlist.NewService(deps)` + `artlist.Build(deps)` Pattern 0 entrypoint. Production-side surface: 3 mandatory fail-closed gates (`Publisher` / `outbox.Dispatcher` / `cfg+repos.ClipsRepo+Jobs.Service`) + 2 inline typed adapter structs (compile-time pinned via `var _ artlistPkg.X = (*Adapter)(nil)` per godlike/06 SSOT) — `artlistDispatchAdapterInline` (bridges `*outbox.Dispatcher` → `artlist.Dispatcher`; `SaveDiscoveredAsset` is chip-2 forward-pointer no-op) + `artlistIndexerAdapterInline` (bridges `clipindexer.Service` via `IsEnabled() bool`-only structural interface → `artlist.Indexer`; `IndexClip` is forward-pointer no-op). `jobsFacadeFromJobsBundle` retyped to `jobdomain.Service` (was broken `interface{}` placeholder from prior FASE-6 cutover).
  - **godlike/07 fail-closed routing (no-fake-availability)**: any mandatory gate nil → `registerArtlist` logs Warn + skips route registration + returns nil — preserving a healthy boot AND an honest `404` rather than a nil-deref panic. The 8 remaining forward-pointer nil fields (`MetadataWriter`, `ScraperSearcher`/`PixabaySearcher`/`PexelsSearcher`, `Stager`, `MediaProcessor`, `LifecycleService`, `AssetProcRepo`/`AssetVerRepo`/`AssetLocRepo`, `CatalogSync`, `ClipResolver`) are intentionally nil in this commit; each is filed as a follow-up `linked_issue` under `architecture/current.yaml#ART-001` (deadline range 2026-07-25..2026-08-15). Until those wire up, the `/run` and `/recommend` paths may surface nil-deref on forward-pointer fields — acceptable per godlike/07 EXPAND-phase discipline (canonical surface live, runway follow-ups close the runtime incrementally). Read-only endpoints (`/stats`, `/diagnostics`, `/search/live`) are unaffected.
  - **godlike/06 SSOT audit-pin (one canonical owner per fact)** — 2 NEW adapter structs are the canonical composition-root surfaces for the `outbox.Dispatcher ↔ artlist.Dispatcher` AND `clipindexer.Service ↔ artlist.Indexer` port shape conversions; each struct has its own compile-time assertion, so future drift in either port signature is a build failure at the canonical concrete site. `assets.ClipsRepository` is consumed DIRECTLY as `AssetStore` (PR2.5 added the missing methods to the port to mirror the repo 1:1; no extra adapter needed).
  - Co-authored-by: PipelineGen Agent <agent@pipelinegen.local>. Cross-reference: `architecture/current.yaml#ART-001` (wave-tracker anchor + 8 follow-up `linked_issues`) + AGENTS.md §Pattern 0 (port + compile-time pin) + §Git-Lesson-2 (direct-to-main commit, no `--force`). Pre-existing 5-item build-issue carry-forward unchanged (out of scope per godlike/07 forward-pointer convention): `monitor/enqueue.go` `strings.ToLower` undefined + `monitor/scheduler.go` `NewUnboundJobEnqueuer` undefined + `internal/application/assets/providers/stock/stockpipeline/run_upload.go` syntax error + `internal/app/module_media.go` `clips.Deps.MutationsDispatcher` literal + `internal/application/images/routing` import cycle. The 2 other unstaged-tracked files (`internal/api/jobs/handler_workers.go` + `internal/infrastructure/remote/jobbrokerclient/client.go`) belong to the parallel-agent `CompleteWithArtifacts` job-broker surface and carry pre-existing `driveutil.ErrLegacySurfaceRetired` undefined errors; they are NOT modified by this commit and remain untracked WIP for the dealing agent.
