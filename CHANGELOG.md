...
- **[ART-002 P2.3 — E2E fallback-degradation test (Node 5xx → exec, July 2026)]** `test(e2e)` — new end-to-end test for the Artlist fallback path. Test lives in new file `tests/e2e/artlist_fallback_test.go` (same `tests/e2e/` directory as P2.1/P2.2, same `e2e` package). Self-contained: mock Node server returns 500 (triggers `ErrTransportFallback` per `scraper.go:122-130`) → real `scraper.Provider` falls back to spawning the Node script via the canonical process runner. The test asserts the FALLBACK PATH was taken (not the specific outcome — exec may succeed locally with `node` in PATH or return `ErrUnavailable` in CI without it; both outcomes are valid per godlike/07 honest-limitation). **Honest scope-lock (godlike/07):** the test pins the BEHAVIORAL contract (fallback invoked) not the OUTCOME — operators reading the test should understand this is a smoke test of the fallback logic, not a happy-path functional test. **godlike/06 SSOT:** real production `scraper.Provider` (no stub); **godlike/07 no-fake-availability:** real `scraper.go:108-117` exec branch (no mock). Tight timeouts (1s HTTP + 2s exec + 5s ctx) so the test fails fast in CI deadlock scenarios. **Verification:** `gofmt -l` clean; `go test -short -count=1 -v ./tests/e2e/` PASS; pre-existing voiceover build-issue carry-forward reproduces unchanged. Co-authored-by: PipelineGen Agent <agent@pipelinegen.local>. AGENTS.md Git-Lesson-3.

- **[ART-002 P2.2 — E2E full-run + Drive-upload test (July 2026)]** `test(e2e)` — new end-to-end test for the Artlist full-run pipeline. Test lives in new file `tests/e2e/artlist_full_run_test.go` (same `tests/e2e/` directory + `e2e` package as P2.1). Self-contained: real `scraper.Provider` (P2.1) + `httptest.NewServer` download source + in-process mock Drive publisher (records the upload call). Asserts the 3-stage contract: (1) search returns 1 candidate; (2) HTTP GET on the candidate's `SourceRef` returns the mock content byte-for-byte; (3) mock publisher records the upload with the right `LocalPath` + returns a non-empty `PublicURL`. **godlike/07 honest-limitation:** uses `http.Get` directly for the download stage (not the production `Downloader`) to avoid pulling in `*config.Config` wiring (which has pre-existing build issues per `architecture/current.yaml#PRE-EXISTING-BUILD-ISSUES-2026-07-04`); the production Downloader is exercised by the integration test suite. The mock publisher mirrors `delivery.Publisher` port shape (PublishRequest.LocalPath + PublishResult.PublicURL) so the upload seam matches production wiring. **Verification:** `gofmt -l` clean; `go test -short -count=1 -v ./tests/e2e/` PASS; pre-existing voiceover build-issue carry-forward reproduces unchanged. Co-authored-by: PipelineGen Agent <agent@pipelinegen.local>. AGENTS.md Git-Lesson-3.

- **[ART-002 P2.1 — E2E live-search test (Node ON scenario, July 2026)]** `test(e2e)` — new end-to-end test for the Artlist live-search path. Test lives in new file `tests/e2e/artlist_live_search_test.go` (new directory `tests/e2e/`, new package `e2e`). Self-contained test: real `scraper.Provider` (from `internal/infrastructure/artlist/scraper`) talking to a mock Node scraper (`httptest.NewServer` returning the canonical Response JSON shape with 2 clips). 4 subtests pin the response contract: `happy_path_2_candidates` (2 clips → 2 Candidates + field mapping) + `4xx_invalid_response` (404 → `ErrInvalidResponse`, no exec fallback per `scraper.go:114`) + `ok_false_empty_result` (200 + ok=false → `ErrEmptyResult`) + `empty_clips_empty_result` (200 + clips=[] → `ErrEmptyResult`). The 5xx → `ErrTransportFallback` → exec path is covered separately in P2.3. **godlike/06 SSOT:** real production `scraper.Provider` (compile-time-pinned via `var _ artapp.Searcher = (*Provider)(nil)`); **godlike/07 no-fake-availability:** the mock server returns the canonical Node scraper response shape (ok=true, clips=[…]) so the real adapter path is exercised end-to-end, not a stubbed response. **Verification:** `gofmt -l` clean; `go vet ./tests/e2e/` exit 0; `go test -short -count=1 -v ./tests/e2e/` PASS; pre-existing voiceover build-issue carry-forward reproduces unchanged. Co-authored-by: PipelineGen Agent <agent@pipelinegen.local>. AGENTS.md Git-Lesson-3.

- **[ART-002 P1.3 — Node scraper health probe (60s tick + 3-consecutive-failures alert, July 2026)]** `feat(artlist) + feat(observability) + feat(app) + test(artlist)` — new SRE surface for the Artlist Node scraper server liveness. Probe lives in new package `internal/infrastructure/artlist/health/probe.go` (NewProbe struct + Options + Start/Stop + ticker-driven `tick(ctx)` method); 3 TDD tests in new `probe_test.go` (`TestProbe_Healthy` + `TestProbe_TransportError_IncrementsStreak` + `TestProbe_ThreeConsecutiveFailures_TriggersAlert`, the last pin the alert-once-per-streak semantic: 3 failures = 1 alert, 6 failures = 2 alerts, streak resets to 0 after each alert). Probe target: HTTP GET on the scraper server URL (any HTTP response = alive, only transport errors count as unhealthy — mirrors the IsLiveProbe convention; per-attempt 5s HTTP timeout, 60s tick cadence, 3-failure threshold as per user spec). 2 new Prometheus metrics in `internal/infrastructure/observability/metrics_artlist.go`: `artlist_scraper_probe_total{result="success|failure"}` (1 label, 2 series max — per-attempt counter) + `artlist_scraper_health_alerts_total` (no labels, 1 series — per-alert counter, alert-once-per-streak semantic). Pattern-0 `health.Metrics` adapter mirrors the `DriveValidatorMetrics` P1.4 / `downloader.Metrics` P1.1 contract (nil-receiver + nil-field safe). Composition-root integration in `internal/app/lifecycle_scheduler.go::buildSchedulerSteps`: new `artlist-scraper-health-probe` StartupStep registered when BOTH `cfg.Features.ArtlistEnabled=true` AND `cfg.External.ArtlistScraperServerURL!=""` (the P0.1 fail-closed composition-time gate, mirroring `validateArtlistScraperURL`); step is OMITTED (not ErrCapabilityDisabled) when Artlist is off — the capability is intentionally absent in that case, not "disabled at startup" (no fake-availability per godlike/07). Start closure calls `ap.Start(startCtx)` (probe's own ticker goroutine, no SafeGo wrap needed); Stop closure calls `ap.Stop(stopCtx)` with the parent lifecycle's stop context (5s wait cap per the probe's internal ctx). **Verification:** `gofmt -l` clean on all 5 touched files; `go vet ./internal/infrastructure/artlist/health/ ./internal/infrastructure/observability/ ./internal/app/` exit 0; 3/3 TDD tests pass; pre-existing voiceover build-issue carry-forward reproduces unchanged (NOT regressions). Co-authored-by: PipelineGen Agent <agent@pipelinegen.local>. AGENTS.md Git-Lesson-3.

- **[ART-002 P1.1 — Prometheus `artlist_download_path_total{path}` counter (July 2026)]** `feat(observability) + feat(downloader) + test(downloader) + feat(app)` — new SRE surface for the Artlist download-path distribution. Counter (1 label `path` with 4 canonical values: `browser` | `yt-dlp` | `http` | `hls`) declared in new file `internal/infrastructure/observability/metrics_artlist.go` (promauto-registered, surfaced via `api/routes.go::/metrics`). The `Help` field explicitly documents the per-attempt-not-per-completion discipline (a single Download that retries 3 times on a flaky HLS CDN adds 3 to the counter; rate() over this metric is the attempt rate, not the success rate; dashboards MUST treat rate() as attempt pressure, not completion volume). Pattern-0 adapter `downloader.Metrics` (new file `internal/infrastructure/artlist/downloader/metrics.go`) mirrors the canonical `DriveValidatorMetrics` precedent (P1.4): `NewMetrics()` returns a struct pointing at the promauto global; `incDownloadPath(path)` is nil-safe at both receiver and field level. The 4 path-label consts (`PathBrowser` / `PathYTDLP` / `PathHTTP` / `PathHLS`) are the canonical call-site surface — mis-spelled values create new time-series silently (textbook Prometheus footgun, so consts are mandatory at every call site). `Provider.Download` (modified in `downloader.go`) fires `PathYTDLP` for HLS via yt-dlp and `PathHTTP` for progressive HTTP — `PathBrowser` and `PathHLS` are reserved for forward-compat with `PR-ARTLIST-DOWNLOAD-SURFACE-UNIFY`. Composition root wiring in `internal/app/build_bundles_artlist.go::WireArtlist` passes `downloader.NewMetrics()` as the 3rd arg of `downloader.New` (1 call-site update). Cardinality: 4 series max (1 per path label, populated lazily per Prometheus CounterVec convention). 2 TDD tests in new `downloader_metrics_test.go`: `TestMetrics_IncDownloadPath_LabelRouted` (4 Path* consts routed + cumulative-counter discipline) + `TestMetrics_NilSafety` (nil-receiver + nil-field no-ops; mirrors `TestDriveValidatorMetrics_NilReceiverNoOp_P1_4`). Code-reviewer round-1 fixes (2): (a) Help text documents per-attempt-not-per-completion discipline so SRE consumers can discover it from the `/metrics` endpoint output, not just from the source comment; (b) CHANGELOG entry moved from `### Fixed` to `### Added` (P1.1 introduces new files + new struct + new const + new metric, so it's unambiguously an Added entry — the prior `### Fixed` placement was incorrect). **Verification:** `gofmt -l` clean on all 5 touched files; `go vet ./internal/infrastructure/artlist/downloader/ ./internal/infrastructure/observability/ ./internal/app/` exit 0; pre-existing voiceover build-issue carry-forward reproduces unchanged (NOT regressions). Co-authored-by: PipelineGen Agent <agent@pipelinegen.local>. AGENTS.md Git-Lesson-3.
...
- **[VO-DECOMPOSITION-2026-07-04 — Voiceover subsystem decomposition wave-tracker entry (8 net-new linked_issues, July 2026)]** `chore(architecture) + docs(plan)` — register the canonical wave-tracker anchor `architecture/current.yaml#VO-DECOMPOSITION-2026-07-04` for the Italian Phase 1 (read-only) audit of the voiceover subsystem pasted to the orchestrator on 2026-07-04. The audit identified a 6-file decomposition chain (3 split-per-stage + 1 TTS persistent worker + 1 DRY pair + 1 parent state machine) + 1 typed-primitive quick win (TextHash + Language + StyleGroup typed, single PR per half-migration risk mitigation per audit §2.4 #3). 8 net-new slim-shape `linked_issues` filed (per godlike/06 SSOT discipline, one canonical owner per fact): 5 P0 (`PR-VO-TTS-PERSISTENT-WORKER` deadline 2026-08-15 cross-language wire, `PR-VO-STAGES-SPLIT` 2026-08-01 5-file split of 605-LoC `stages.go`, `PR-VO-FINALIZER-STEP6-EXTRACT` 2026-08-01 outbox cleanup extraction from 538-LoC `finalizer.go`, `PR-VO-PARENT-AGGREGATOR-SPLIT` 2026-08-01 4-file state-machine + poll split of 469-LoC `parent_aggregator.go`, `PR-VO-USECASE-PROCESS-DRY` 2026-08-15 DRY canonical PipelineExecutor neutral struct for batch/per-item pair), 1 P1.2 (`PR-VO-PARENT-STATE-COLUMN` 2026-08-01 typed column migration, forward-pointer already filed in `generate_handler.go:267`), 1 P1.1 (`PR-VO-TYPED-PRIMITIVES` 2026-07-25 quick win), 1 hotspot cross-ref (`VO-DECOMPOSITION-HOTSPOT-CROSSREF` 2026-08-15 mirrors GODOBJ-2026-07-03 `PR-GODOBJ-HOTSPOT-CROSSREF` pattern). **Execution order** (per the audit's "ordine consigliato"): P1.1 typed-primitive quick win first (unblocks the typed-error work in P0 #5 DRY pair); then 4 P0 splits (stages.go + finalizer.go + parent_aggregator.go + DRY pair, all sharing compile-time lock on `stages.go` + `finalizer.go` because `process_voiceover_item.go` reads both); then P0 #1 TTS persistent worker (cross-language wire, longest lead time); P1.2 state column runs in parallel after P0 #4 split. **Carry-forward preservation** (godlike/07 honest-limitation): audit-pins at `usecase.go:478,511` + `stages.go:594-595` + `ports.go:158` + `useCaseTTSAdapter` panic-on-nil (P0 #4 fail-closed-at-startup contract) + Replace-Mode cleanup triplo in `finalizer.go:88-95` (P0.7 Step 10/12 atomic swap-and-cleanup) + Tick pattern in `parent_aggregator.go` (polling intentional, Phase 1 wiring). **Pre-existing 5-item build-issue carry-forward** unchanged (out of scope per CHANGELOG forward-pointer convention). **No migration / no gofmt touch / no test churn in this commit**: documentation-only across 3 surfaces (CHANGELOG this entry + AGENTS.md §Recent cross-cutting closures mirror + `architecture/current.yaml` wave-tracker entry + `architecture/action-plans/2026-07-04-voiceover-decomposition.md` narrative companion). Per AGENTS.md Git-Lesson-2: each per-item PR lands **directly on `main`** per-file (auto-sufficient granularity); Co-authored-by trailer per Git-Lesson-3. Subject line per AGENTS.md Git-Lesson-3 (`chore` = standard for non-functional architectural metadata). **Cross-reference:** `architecture/current.yaml#VO-DECOMPOSITION-2026-07-04` (wave-tracker anchor + 8 net-new `linked_issues` + exit_gate) + `architecture/action-plans/2026-07-04-voiceover-decomposition.md` (narrative + kill-candidate matrix + expected split topologies + godlike/07 honest limitations) + AGENTS.md §Recent cross-cutting closures (June 2026) closure mirror entry.
...
- **[PR-QDRANT-CONFIG-MISMATCH-GATE closure (QDRANT-CHAIN-VERIFY-2026-07-04 P0 fail-closed gate, 2026-07-04)]** `feat(app)+test(app)+chore(arch)` — close the RED POINT surfaced by the QDRANT-CHAIN-VERIFY-2026-07-04 audit: when `qdrant.enabled=true` AND `clipindexer.enabled=false`, the outbox marks `asset.index.requested` as COMPLETED without writing to Qdrant (IndexClip short-circuits to nil — false-success path). NEW canonical composition-root fail-closed helper `validateQdrantIndexerCompatibility(cfg *config.Config) error` at `internal/app/build_bundles_qdrant_gates.go` (~170 LoC, mirror of ART-002 P0.1 `validateArtlistScraperURL`). **godlike/06 SSOT (one-canonical-owner-per-fact)**: the helper is the SOLE canonical owner of the Qdrant+ClipIndexer compatibility check; wired at 4 defence-in-depth composition sites via exactly one call line each: (1) `internal/app/build_process_qdrant.go::buildQdrantDeps` (retired BLOCKER #3 inline check, replaced with helper call), (2) `internal/app/build_bundles_process.go::BuildOutboxBundle` (top-of-fn gate before outbox handler registry), (3) `internal/app/wire_services.go::WireServices` (before EnsureSchema startup step, with `coreClean()` rollback on fail), (4) `internal/app/build_bundles_core.go::buildHealthService` (consistency-check, error discarded since the fn does not return error). 4 TDD tests in `internal/app/build_bundles_qdrant_gates_test.go` cover the contract (nil-cfg / both-disabled-OK / both-enabled-OK / Qdrant=true-ClipIndexer=false FailClosed asserting the canonical 5-substring envelope: "QdrantEnabled=true" + "ClipIndexerEnabled=false" + "QDRANT-CHAIN-VERIFY-2026-07-04 P0" + "VELOX_FEATURE_CLIP_INDEXER_ENABLED=true" + "VELOX_FEATURE_QDRANT_ENABLED=false"). godlike/07 no-fake-availability: fail-closed at composition time on BOTH directions of mismatch. 2 canary tests in `internal/app/composition_test.go` updated to pin the new contract surfaces (Direction B flipped from false-success-OK to fail-closed-Error; Direction A substring assertions updated to canonical camelCase format). **Verification**: gofmt clean on all 7 surfaces; 4 new TDD tests + 2 updated canary tests pass. **Pre-existing 6-item build-issue carry-forward unchanged**: the `processor.NewProcessor: publisher is required` panic reproduces on origin/main HEAD (verified via stash + reset-to-origin + retest before this PR's commit) — NOT a regression from this PR per `architecture/current.yaml#PRE-EXISTING-BUILD-ISSUES-2026-07-04` discipline. **Cross-ref lockstep**: `architecture/current.yaml#PR-QDRANT-CONFIG-MISMATCH-GATE status: pending -> shipped` + AGENTS.md §Recent cross-cutting closures mirror entry + this CHANGELOG entry. Co-authored-by: PipelineGen Agent <agent@pipelinegen.local>. AGENTS.md Git-Lesson-3.
### Refactor
