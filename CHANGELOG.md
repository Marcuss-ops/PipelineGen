...
1. **P1-COMPL-12 PLACEHOLDER-CAPABILITY closed** — closure of the post-CUTOVER P1 audit item 12 (godlike/07 no-fake-availability). `fix(app)` `commit bd11f6be` deletes the placeholder `voiceover.generate_item` registration from `internal/app/creator_runtime.go` and replaces it with a godlike/07-compliant tombstone comment. Placeholder SHAPE preserved under build-tag `voiceover_placeholder` (two affordance files: `creator_runtime_placeholder_test_only.go` + `creator_runtime_placeholder_opt_in_test.go`) for opt-in test fixtures ONLY. `chore(architecture)` `commit <closure-sha>` records the wave-tracker entry `COMPLETION-HARDENING-P1P2-2026-07-04` linking 6 P1 + 2 P2 forward-pointer items per godlike/06 SSOT (no duplication vs `COMPLETION-CUTOVER-P0-2026-07-04`). `chore(ci)` adds forward-prevention via `Check 56` in `scripts/ci-architectural-checks.sh` — rg-gate scans `internal/app/creator_*.go` for live string literal `"voiceover.generate_item"` or constant `*.TypeVoiceoverGenerateItem` references; tombstone-comment lines (Go `//`-prefixed) are excluded via awk post-filter (Pattern C). See `architecture/action-plans/2026-07-04-completion-cutover.md` for the full P1/P2 audit-pin surface.
- **[P0 #4 audit closure — script.generate child-job fan-out (6-commit chain, July 2026)]** `fix(scripts) + fix(jobs) + fix(app) + refactor(scripts)` — canonical `script.generate → script.generate_item` child-job fan-out architecture with parent aggregator + lifecycle ownership (10 aggregator tests + 4 handler tests green; 13/13 P0 #4 scenarios covered). Each batch item is a separate broker-emitted child job with independent retry envelope + deterministic ActiveKey idempotency; the parent aggregator TerminalFlip drives the parent broker status based on the aggregate via domain StateMachine. 6-commit chain on `origin/main`: `98d768aa` (restore child handler + composition wiring), `18151c0a` (cut batch generation over to child-job fanout), `63d4fcb6` (scope aggregate-parent query by job type + `partial_success` terminal + nil-child guard), `cd866457` (lifecycle-own script parent aggregator via StartupStep), `de8a7321` (remove fanout compatibility residue — `ChildJobIDsJSON` / dead marshal / unused `cfg` param), `7336699d` (gofmt + update batch-failure test for fan-out contract). Closes 9 P0 issues from the Italian audit snapshot: P0 #2 (nested functions + nil deref), P0 #4 (fan-out not executed), P0 #5 (double-marshalling + wrong ID field + missing ActiveKey), P0 #6 (all-enqueue-failed false success), P0 #7 (`partial_success` infinite aggregation loop), P0 #8 (`ProducesArtifacts=true` on child), P0 #9 (`agg.Start(context.Background())` outside lifecycle), P0 #10 (nil-child panic), P0 #3 (query hardcoded to voiceover only). Cross-reference: `architecture/current.yaml#PR-SCRIPTING-CANONICAL-FRAMEWORK` + AGENTS.md §P0 #4 audit.
...
- **[P0-COMPL-5-WIRE-NAMING — Rename `PublishedArtifacts` → `StagedArtifacts` in complete-with-artifacts wire-format + canonical `PublishAndCompleteUseCase` (Sender MUST convert `StagedArtifactReference` → `PublishedArtifact`)]** `refactor(api) + refactor(jobs-domain) + feat(completion) + test(completion)` — wire-format rename in 2 layers (HTTP + domain command): the HTTP wire at `internal/api/jobs/handler_workers.go::CompleteArtifactsRequest.ArtifactManifest` → `StagedArtifacts []*remote.StagedArtifactReference \`json:"staged_artifacts"\`` (typed slice, NOT opaque bytes) + the domain command at `internal/application/jobs/domain_commands.go::CompleteWithArtifactsCommand.PublishedArtifacts` → `StagedArtifacts json.RawMessage \`json:"staged_artifacts"\`` (renamed; type preserved for worker-side byte-stability). The caller now ships `StagedArtifactReference` (canonical 3-field envelope at `internal/domain/remote/staged_artifact_reference.go`: ArtifactID + Destination + optional SHA256 hint, plus 9-key CanonicalDestinationKeys directory + 2 typed sentinels `ErrStagedArtifactReferenceMissingFields` / `ErrStagedArtifactReferenceInvalidDestination` + Validate() godlike/07 typed-error contract). The NEW canonical Sender-side surface `PublishAndCompleteUseCase` (at `internal/application/jobs/completion/publish_and_complete_use_case.go`) is the EXPAND-phase converter: Step 1 validates `req + staged`, then for each `StagedArtifactReference` calls `ArtifactPreparation.Prepare(ctx, *VerifiedArtifact)` (canonical publish-pipeline post-P0-COMPL-4) producing `finalization.PublishedArtifact` (10-field canonical envelope with Drive `Location{Provider, FileID, WebViewLink, DownloadLink, FolderID, FolderPath, Action}`); Step 2 routes the resulting `[]*PublishedArtifact` slice to the canonical `WithArtifactsService.CompleteWithArtifacts` single-TX atomic terminal. 2 godlike/07 typed sentinels added (`ErrPublishAndCompleteUseCaseNotConfigured` + `ErrPublishAndCompleteUseCasePrepareFailed`). **3 round-trip TDD tests pinned** in `internal/application/jobs/completion/publish_and_complete_use_case_test.go` (3 user-spec'd + 1 supporting): T1 `TestPublishAndCompleteUseCase_RoundTrip_ThreeRefsBecomeThreePublishedWithLocation` — 3 StagedRefs → 3 distinct Drive Locations populated; T2 `TestPublishAndCompleteUseCase_ConversionCorrectness` — 10-field + 5-Location envelope byte-stable round-trip; T3 `TestPublishAndCompleteUseCase_InvalidStagedFailsTypedError` — invalid input surfaces typed-error sentinels via errors.Is (empty ArtifactID → ErrStagedArtifactReferenceMissingFields; out-of-directory Destination → ErrStagedArtifactReferenceInvalidDestination); T4 supporting `TestWireFormat_StagedArtifactsRoundTrip` — wire-body `staged_artifacts` JSON key round-trips byte-stable through encoding/json. Existing 4 tests in `handler_workers_test.go` migrated to typed slice + new field name. Honest scope-lock (godlike/07): the handler-routing to `PublishAndCompleteUseCase` is the BACKFILL-phase forward-pointer (`TODO(P0-COMPL-5-HANDLER-WIRE)`, deadline 2026-08-01); in this commit the handler still routes through the legacy worker-side pipeline (cmd.StagedArtifacts is json.RawMessage marshaled from req.StagedArtifacts). `BuildStageDriveBundle.ArtifactPreparation` remains a forward-pointer pending the finalization.PublisherPort adapter design (separate concern from P0-COMPL-4-PUBLISH-DEDUPE forward-pointer `P0-COMPL-4-BUNDLE-LIVE`). Pre-existing monitor DTO splits + image-routing import cycle + the `verification/verified.go` pre-existing drift are OUT OF SCOPE per AGENTS.md minimal-change.


- **[P0-COMPL-4-PUBLISH-DEDUPE — Remove `publisher` from `completion.Service` (godlike/06 SSOT, completion dedup)]** `refactor(completion)` + `test(completion)` — the prior Service struct held a `Publisher` field that published the artifact AGAIN after `ArtifactPreparation.Prepare` had ALREADY published it (via Prepare→s.publisher.Publish internally). That was a godlike/06 SSOT violation: TWO owners of the canonical "Drive write" fact. Closed by removing the `Publisher` interface + `Publisher` field + 3rd NewService arg + the duplicate `s.publisher.Publish` retry block in `publishOne`; Preparer IS the canonical publish seam (its internal `finalization.PublisherPort` handles validate+sha256+Drive-upload as a single owner). `publishOne` now wraps `s.preparer.Prepare(ctx, *va)` with `pkg/retry.Do(retry.DefaultOptions())` — transient errors from Prepare bubble through the wrapper's retry loop. **5 dedup-invariants TDD tests pinned** (`internal/application/assets/completion/dedup_tdd_test.go`, 5 new test functions): (1) `TestDedup_NoPublisherFieldOnServiceStruct` — reflection-based godlike/06 SSOT drift-detection pinning EXACTLY 2 fields on Service (`preparer` + `bookkeeper`); any future re-introduction of `publisher`/`pub`/`notifier` fails this test at build-failure. (2) `TestDedup_SinglePrepareCallPerArtifact` — pin 1 valid artifact → 1 Prepare call (no double-publish). (3) `TestDedup_NoDoublePublishOnTransientRetry` — transient-then-retry: cumulative Preparer calls == 2 (1 transient + 1 retry-success); the Drive write happens INSIDE the second Prepare, not via a separate Publish. (4) `TestDedup_IdempotentReplayZeroPrepareAndZeroRecord` — already-published triple → 0 Preparer calls + cached envelope returned + ErrAlreadyPublished surfaces via errors.Is. (5) `TestDedup_SingleCanonicalRecordPerTriple` — 1 valid artifact → exactly 1 Bookkeeper.RecordPublished per triple + replay adds 0 records. Pre-existing 5 tests in `publish_verified_test.go` updated to drop `mockPublisher` + use 2-arg `NewService(prepMock, bkMock)` (signature change). Honest scope-lock (godlike/07): `BuildStageDriveBundle.ArtifactPreparation` remains a forward-pointer pending the `finalization.PublisherPort` adapter design (a SEPARATE backward concern — forward-pointer `P0-COMPL-4-BUNDLE-LIVE`, deadline 2026-07-25). Pre-existing `verification/verified.go` drift (forward-pointer `PRE-EXISTING-BUILD-ISSUES-2026-07-04`, deadline 2026-08-01) is OUT OF SCOPE.


  - **[P0-COMPL-2-COMPOSITION-WIRE closure `2a0556f0` ship 2026-07-04, July 2026]** `feat(composition) + docs(architecture)` -- closes the canonical Stage→Drive production composition bundle per `architecture/current.yaml#COMPLETION-CUTOVER-P0-2026-07-04` (action plan PR-4, P0-COMPL-2-COMPOSITION-WIRE). Wires the 5 components in pipeline order: [1] `StagedResolver` (`staged.StagedArtifactResolver`) + [2] `Verifier` (`verification.Verifier`; aliases the user-spec `VerifiedArtifactProjector`) + [3] `ArtifactPreparation` (forward-pointer TODO `P0-COMPL-4-PUBLISH-DEDUPE`, deadline 2026-07-25) + [4] `Publisher` (`delivery.Publisher`) + [5] `WithArtifactsService` (forward-pointer TODO `P0-COMPL-5-SINGLE-BACKBONE`, deadline 2026-08-15). godlike/06 SSOT: this bundle is the SINGLE canonical entry-point for Stage→Drive production composition -- zero parallel `New*` constructors in `internal/application/`. godlike/07 typed-error contract: `ErrStageDriveInsufficientForCompletion` is the shared sentinel for BOTH forward-pointer components (`ArtifactPreparation` + `WithArtifactsServiceAccessor`); accessors return the typed sentinel via DIRECT return (`errors.Is`-compatible without unwrap chain). Auto-sufficient micro-PR: file compiles in isolation (`go vet ./internal/app/build_stage_drive_bundle.go` exits 0); pre-existing drift in `internal/application/assets/verification/verified.go` (`sa.LocalPath` / `sa.SizeBytes` doesn't match `staged.StagedArtifact` fields `Path` / `Bytes`) is OUT OF SCOPE per AGENTS.md minimal-change + CHANGELOG forward-pointer convention -- forward-pointer `PRE-EXISTING-BUILD-ISSUES-2026-07-04` in `architecture/current.yaml` carries the resolution deadline (2026-08-01). godlike/07 no-fake-availability: forward-pointer components return typed sentinel on call (not `nil` pointer) so callers can `errors.Is` without nil-deref panic. This entry flips `architecture/current.yaml#P0-COMPL-2-COMPOSITION-WIRE.status` from `pending` to `shipped`. Co-authored-by: PipelineGen Agent <agent@pipelinegen.local>.
