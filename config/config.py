...
# Use cross-platform paths with environment variable overrides
BACKGROUND_MUSIC_PATHS = os.environ.get("BACKGROUND_MUSIC_PATHS", "").split(os.pathsep)
BACKGROUND_MUSIC_PATHS = [p for p in BACKGROUND_MUSIC_PATHS if p]  # Remove empty strings
if not BACKGROUND_MUSIC_PATHS:
    # Default to empty list - user will populate via Gradio
    BACKGROUND_MUSIC_PATHS = []

BACKGROUND_MUSIC_VOLUME = 0.5
USE_GPU = True

# Master test-mode allowlist (default). Used by main.py if VELOX_TEST_WORKER_ALLOWLIST is not set.
# DISABLED: Allow all workers to pick up jobs without restriction
# TEST_WORKER_ALLOWLIST_DEFAULT = "51.91.11.105,57.128.245.200"
TEST_WORKER_ALLOWLIST_DEFAULT = ""  # Empty = no allowlist
...
# Chrome user data - use environment variables or sensible defaults
USER_DATA_DIR = os.environ.get("CHROME_USER_DATA_DIR", "")
if not USER_DATA_DIR:
    # Cross-platform default - empty on Linux, user should configure
    USER_DATA_DIR = ""

PROFILE_DIRECTORY = os.environ.get("CHROME_PROFILE", "Profile 8")
TEMP_PROFILE_DIR = os.environ.get("TEMP_CHROME_PROFILE", os.path.join(os.path.expanduser("~"), ".velox_chrome_profile"))

# Font paths - use environment variables or empty list
FONT_PATH_DEFAULT = os.environ.get("DEFAULT_FONT_PATH", "")
FONT_PATHS = os.environ.get("FONT_PATHS", "").split(os.pathsep)
FONT_PATHS = [p for p in FONT_PATHS if p]  # Remove empty strings
if not FONT_PATHS:
    FONT_PATHS = []
...
DRIVE_ROOT_FOLDER_ID = "1xVCe3zglpQc9MVF-10zHyTOgy9Dp7nID" # Root folder for YouTube Groups
...
SOUND_EFFECT_CONFIG = {
    "frequency": 7,  # Add sound effect every N segments (changed from 3 to 7)
    "volume": 0.15,  # Sound effect volume reduced to prevent overpowering voiceover (0.0 to 1.0)
    "max_duration": 0.3,  # Maximum sound effect duration in seconds
...
SPECIAL_NAMES_SOUND_CONFIG = {
    ...
    "volume": 0.15,  # Balanced volume that doesn't overpower voiceover
...
def ensure_assets_best_effort() -> None:
    """
    Best-effort bootstrap assets (SFX/fonts/transitions).
    IMPORTANT: questa funzione può fare download e richiedere rete; preferire eseguirla in background.
    """
    try:
        ...
        try:
            ...
            global FONT_PATHS, FONT_PATH_DEFAULT
...
# NOTE: nei worker questo può rallentare l'avvio e impedire heartbeat; si può disabilitare via env.
AUTO_DOWNLOAD_ASSETS_ON_IMPORT = os.environ.get("VELOX_AUTO_DOWNLOAD_ASSETS_ON_IMPORT", "1") == "1"
if AUTO_DOWNLOAD_ASSETS_ON_IMPORT:
    ensure_assets_best_effort()
...
class ConfigWrapper:
    ...
    def __init__(self, settings_dict: Dict[str, Any]):
        ...
        self._defaults = {
            ...
            # Sound effect configuration
            "sound_effect_frequency": 2,  # Add sound effect every N segments (reduced for testing)
            ...
            # 🎬 Stock Transition Configuration (sostituisce primi secondi dello stock con transizione)
            "add_transition_to_stock": False,  # Disabilita la transizione di default sugli stock clips
            "stock_transition_duration": 0.6,  # Durata transizione che sostituisce i primi secondi dello stock (default 0.6s)
            
            # 🎵 Stock Sound Effect Configuration (effetto sonoro mixato con voiceover)
            "add_sound_effect_to_stock": True,  # Abilita effetto sonoro all'inizio degli stock clips con voiceover
            "stock_sound_effect_duration": 0.8,  # Durata effetto sonoro mixato all'inizio (default 0.8s)
        }
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self._settings:
            config_dict.update(self._settings)
        return config_dict
