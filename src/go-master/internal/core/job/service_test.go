...
func TestUpdateJobStatus(t *testing.T) {
	...
	err := service.UpdateJobStatus(context.Background(), job.ID, models.StatusQueued, 0, nil, "")
	if err != nil {
		t.Fatalf("UpdateJobStatus to Queued failed: %v", err)
	}
	
	err = service.UpdateJobStatus(context.Background(), job.ID, models.StatusRunning, 50, nil, "")
	if err != nil {
	...
	if updated.Status != models.StatusRunning {
		t.Errorf("expected status %s, got %s", models.StatusRunning, updated.Status)
	}
...
func TestInvalidStatusTransition(t *testing.T) {
	...
	// Go to running first
	service.UpdateJobStatus(context.Background(), job.ID, models.StatusQueued, 0, nil, "")
	service.UpdateJobStatus(context.Background(), job.ID, models.StatusRunning, 0, nil, "")
	
	// Try invalid transition from Running back to Pending
	err := service.UpdateJobStatus(context.Background(), job.ID, models.StatusPending, 0, nil, "")
...
func TestIsValidStatusTransition(t *testing.T) {
	...
	tests := []struct {
		...
		{models.StatusPending, models.StatusCancelled, true},
		{models.StatusQueued, models.StatusRunning, true},
		{models.StatusRunning, models.StatusCompleted, true},
		{models.StatusRunning, models.StatusFailed, true},
		{models.StatusCompleted, models.StatusRunning, false},
...
func TestCompleteJobFlow(t *testing.T) {
	...
	err = service.UpdateJobStatus(context.Background(), job.ID, models.StatusCompleted, 100, map[string]interface{}{"output": "/path/to/video.mp4"}, "")
	if err != nil {
...
func TestJobRetry(t *testing.T) {
	...
	service.UpdateJobStatus(context.Background(), job.ID, models.StatusQueued, 0, nil, "")
	service.UpdateJobStatus(context.Background(), job.ID, models.StatusRunning, 0, nil, "")
	service.UpdateJobStatus(context.Background(), job.ID, models.StatusFailed, 0, nil, "First attempt failed")
