...
func initSyncServices(
	...
	if clips.StockDB != nil && driveClient != nil {
		statePath := filepath.Join(cfg.Storage.DataDir, "drive_sync_state.json")
		if clips.ClipDB != nil {
			driveSync = stocksync.NewDriveSyncWithClips(driveClient, clips.StockDB, clips.ClipDB, cfg.Drive.StockRootFolderID, statePath)
			log.Info("DriveSync initialized with separate StockDB + ClipDB")
		} else {
			driveSync = stocksync.NewDriveSync(driveClient, clips.StockDB, cfg.Drive.StockRootFolderID, statePath)
	...
	if driveClient != nil {
		artlistStatePath := filepath.Join(cfg.Storage.DataDir, "artlist_sync_state.json")
