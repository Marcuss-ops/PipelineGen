...
func main() {
	...
	// Read credentials
	credBytes, err := os.ReadFile(cfg.GetCredentialsPath())
	if err != nil {
...
log.Fatalf("Failed to parse credentials: %v", err)
	...
	// Read existing token
	tokenData, err := os.ReadFile(cfg.GetTokenPath())
	if err != nil {
...
log.Fatalf("Failed to parse token: %v", err)
	...
	if err := os.WriteFile(cfg.GetTokenPath(), newTokenData, 0600); err != nil {
