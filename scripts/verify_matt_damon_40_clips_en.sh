...
PAYLOAD="$ROOT/ops/jobs/matt_damon_40_clips_en.generate.json"
POLL_SECONDS="${MATT_DAMON_POLL_SECONDS:-5}"
TIMEOUT_SECONDS="${MATT_DAMON_TIMEOUT_SECONDS:-3600}"
...
RUN_ID="matt-damon-40-clips-en-$(date -u +%Y%m%d-%H%M%S)-$$"
BODY=$(mktemp)
...
echo "localized_renders_count=$(jq -r '((.result.data.result // .result.data.result.result // .result.data.items[0].result // .result.items[0].result // .result.output // .result).localized_renders // []) | length' "$BODY.poll")"
