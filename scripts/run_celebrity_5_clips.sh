...
PAYLOAD="$ROOT/ops/jobs/celebrity_5_clips.generate.json"
POLL_SECONDS=3
TIMEOUT_SECONDS=600
...
RUN_ID="celebrity-5-clips-$(date -u +%Y%m%d-%H%M%S)-$$"
BODY=$(mktemp)
trap 'rm -f "$BODY" "$BODY.poll"' EXIT INT TERM

echo "Submitting 5-clip job to $LOCAL_BASE_URL/api/script/generate..."
START_TIME=$(date +%s%N)
