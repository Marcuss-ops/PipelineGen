...
class TestTranslateTextOllama(unittest.TestCase):
    ...
    @mock.patch('scripts.book_processor.llm.get_cache_dir')
    @mock.patch('scripts.book_processor.llm.urllib.request.urlopen')
    def test_translation_success(self, mock_urlopen, mock_get_cache):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = translate_text_ollama("Hello world", "es", model="gemma4:e4b")
            self.assertEqual(result, "Hola mundo")
    
    @mock.patch('scripts.book_processor.llm.get_cache_dir')
    @mock.patch('scripts.book_processor.llm.urllib.request.urlopen')
    def test_translation_fallback_on_error(self, mock_urlopen, mock_get_cache):
...
class TestPDFGeneration(unittest.TestCase):
    """Tests for PDF generation with various styles."""
    
    def test_pdf_generation_styles(self):
        """Test generating PDF with different styling options."""
        import tempfile
        from pathlib import Path
        from scripts.book_processor.pdf_gen import generate_pdf
        
        with tempfile.TemporaryDirectory() as tmpdir:
            tmpdir_path = Path(tmpdir)
            text_file = tmpdir_path / "test_book.txt"
            text_content = "Chapter 1: The Beginning\n\nThis is a sample paragraph for testing the PDF generation layout and custom style parameters."
            text_file.write_text(text_content, encoding="utf-8")
            
            for style in ["default", "modern", "classic", "academic", "colorful"]:
                pdf_file = tmpdir_path / f"book_{style}.pdf"
                success = generate_pdf(text_file, pdf_file, title="Test Book Title", style_name=style)
