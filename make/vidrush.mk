...
verify-vidrush-query-planning:
	@$(GO) test -count=1 ./internal/application/scripts/adapters -run 'Query|Artlist|InternetImages'
...
verify-vidrush-artlist-search verify-vidrush-artlist-download verify-vidrush-artlist-persist verify-vidrush-artlist-index:
	@$(GO) test -count=1 ./internal/application/scripts/adapters -run 'Artlist|ClipSearch'
...
verify-vidrush-image-search verify-vidrush-image-download verify-vidrush-image-validation verify-vidrush-image-persist verify-vidrush-image-index:
	@$(GO) test -count=1 ./internal/application/scripts/adapters -run 'InternetImages|VidRushImage'

verify-vidrush-image-generation verify-vidrush-image-generation-cache verify-vidrush-image-generation-persist:
	@$(GO) test -count=1 ./internal/application/scripts/adapters -run 'Generation|Image'
...
benchmark-vidrush:
	@$(GO) test -run '^$$' -bench 'VidRush' ./internal/application/scripts/adapters

doctor-vidrush:
	@$(GO) test -count=1 ./internal/application/scripts/adapters -run 'VidRushAssetProviderRegistry|VidRushImage'
