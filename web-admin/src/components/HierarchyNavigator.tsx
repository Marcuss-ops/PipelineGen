...
export function HierarchyNavigator({ breadcrumb, onNavigate }: HierarchyNavigatorProps) {
  return (
    <nav className="flex items-center space-x-1 text-sm text-zinc-500 mb-4 bg-zinc-50 p-3 rounded-2xl border border-zinc-200">
      <Button 
        variant="ghost" 
        size="sm" 
        className="h-8 px-2"
      ...
      {breadcrumb.map((node, index) => (
        <div key={node.id} className="flex items-center">
          ...
          <Button 
            variant="ghost" 
            size="sm" 
            className={`h-8 px-2 ${index === breadcrumb.length - 1 ? 'text-zinc-900 font-bold' : ''}`}
