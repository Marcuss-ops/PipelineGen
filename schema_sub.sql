...
CREATE INDEX idx_clips_drive_file_id ON clips(drive_file_id) WHERE drive_file_id != '';
CREATE INDEX idx_clips_duration ON clips(duration);
CREATE INDEX idx_clips_resolution ON clips(width, height) WHERE width > 0 AND height > 0;
CREATE INDEX idx_clips_size ON clips(size_bytes DESC) WHERE size_bytes > 0;
CREATE INDEX idx_clips_fps ON clips(fps) WHERE fps > 0;
CREATE INDEX idx_clips_processing_stage ON clips(processing_stage) WHERE processing_stage IS NOT NULL;
CREATE INDEX idx_clips_retry_count ON clips(retry_count DESC) WHERE retry_count > 0;
CREATE INDEX idx_clips_processed_at ON clips(processed_at DESC) WHERE processed_at IS NOT NULL;
...
CREATE UNIQUE INDEX ux_clips_file_hash ON clips(file_hash) WHERE file_hash IS NOT NULL AND file_hash != '';
...
CREATE INDEX idx_clip_folders_status ON clip_folders(status) WHERE status != '';
