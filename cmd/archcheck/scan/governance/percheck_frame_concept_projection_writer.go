...
// writer:
//
//	media_frames   → internal/platform/qdrant/indexing/mediamemory/qdrant_frame_indexer.go
//	media_concepts → internal/platform/qdrant/indexing/mediamemory/qdrant_indexer.go
//
...
var frameConceptAuthorizedWriters = map[string]string{
	"internal/platform/qdrant/indexing/mediamemory/qdrant_frame_indexer.go": "FrameCollectionName",
	"internal/platform/qdrant/indexing/mediamemory/qdrant_indexer.go":       "ConceptCollectionName",
}
...
const frameConceptWriterNote = "forbidden point write to a frame/concept projection outside its respective projection writer (PR-HASH-SEMANTICS item 8/16, August 2026); godlike/06 SSOT requires media_frames writes to originate ONLY in internal/platform/qdrant/indexing/mediamemory/qdrant_frame_indexer.go and media_concepts writes ONLY in internal/platform/qdrant/indexing/mediamemory/qdrant_indexer.go, through the generic ProjectionWriter. A write from any other site (including a frame writer targeting the concept collection, or vice-versa) bypasses the fail-closed envelope and risks corrupting the projection boundary."
