...
func TestScanFrameConceptWriter_RespectiveWriterExempt(t *testing.T) {
	root := t.TempDir()
	makeFrameConceptFile(t, root, "internal/platform/qdrant/indexing/mediamemory/qdrant_frame_indexer.go", frameWriterBody)
	makeFrameConceptFile(t, root, "internal/platform/qdrant/indexing/mediamemory/qdrant_indexer.go", conceptWriterBody)
	rep := &report.Report{}
	ScanFrameConceptProjectionWriter(root, nil, rep, true)
	if got := len(rep.Violations); got != 0 {
...
func TestScanFrameConceptWriter_CrossWriteTrips(t *testing.T) {
	...
	root := t.TempDir()
	makeFrameConceptFile(t, root, "internal/platform/qdrant/indexing/mediamemory/qdrant_frame_indexer.go",
		`package qdrantmm
