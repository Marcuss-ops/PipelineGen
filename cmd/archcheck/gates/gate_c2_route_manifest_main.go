...
// with generated API docs. Its ceiling is derived from the artifacts themselves —
// HEAD^ when history is available, otherwise the structural maximum implied by
// the route manifest plus the generated docs — and may only be lowered by a due
// dated milestone. There is deliberately no hand-maintained --baseline constant:
// a fixed number silently rots as routes are added and hides real regressions.
package main
...
func main() {
	...
	// Ceiling derived from the artifacts, never a hand-maintained constant.
	// computeDiff emits at most one violation per manifest route and one per docs
	// route, so manifestCount+docsCount is the structural maximum and the only
	// honest bootstrap ceiling when no parent commit exists. With history
	// available, HEAD^ is authoritative. A due dated milestone may only lower it.
	ceiling := manifestCount + docsCount
	ceilingSource := "route-manifest-derived structural maximum (no parent commit available)"
	if parentOK {
		ceiling = parentCount
