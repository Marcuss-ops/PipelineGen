...
func RunMultilingualBenchmark(args []string) error {
	...
	for _, id := range assetIDs {
		inputs, err := buildBenchmarkInputs(cli.CmdContext(), root.Repos.ClipsRepo, root.Repos.TextTrackRepo, root.Repos.SubtitleArtifactRepo, cfg, id, srcLang, targetLangs, log)
		if err != nil {
			return err
		}
		if len(inputs) == 0 {
		...
		for _, n := range levels {
			sample := benchmarkLevel(cli.CmdContext(), renderer, inputs, n, *repeats)
