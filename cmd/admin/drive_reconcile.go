...
func runDriveReconcile(args []string) error {
	...
	*rootID = strings.TrimSpace(*rootID)
	...
	if !*apply {
		...
		uploader, err := buildDriveAdminForCLI(cmdContext(), cfg, log)
		if err != nil {
			return fmt.Errorf("drive-reconcile: init Drive client: %w", err)
		}
		files, listErr := uploader.ListFiles(cmdContext(), *rootID)
		if listErr != nil {
	...
	// --apply path.
	return executeReconcile(cmdContext(), cfg, log, *rootID, *maxDepth, *syncAssets, *dbPath)
}
