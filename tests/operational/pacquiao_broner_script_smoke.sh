...
SCRIPT=$(jq -r '.result.output.text // .result.items[0].output.text // ""' "$SMOKE_LAST_BODY")
...
WORD_COUNT=$(jq -r '.result.output.word_count // .result.items[0].output.word_count // 0' "$SMOKE_LAST_BODY")
...
ACCEPTED_COUNT=$(jq -r '((.result.source.accepted_clip_ids // .result.items[0].source.accepted_clip_ids // []) | length)' "$SMOKE_LAST_BODY")
...
if ! jq -e '
    (.result.mode_info.requested_mode // .result.items[0].mode_info.requested_mode) == "clip_native"
    and (.result.mode_info.used_mode // .result.items[0].mode_info.used_mode) == "clip_native"
    and ((.result.mode_info.fallback_used // .result.items[0].mode_info.fallback_used) == false)
