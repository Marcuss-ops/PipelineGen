#!/usr/bin/env bash
# Poppins + stronger-shadow variant of cliprender_matt_damon_5_benchmark.sh.
# Subtitle style: Poppins 58px with a heavier soft shadow (0.90 / blur 16 / y 8)
# and the registered ASS preset 'matt-damon-benchmark-v1-poppins' (Poppins, shadow 6).
...
RUN_ID="matt-damon-5-cliprender-poppins-$(date -u +%Y%m%d-%H%M%S)-$$"
...
build_payload() {
  ...
  jq -n --arg source "$source_id" --arg wm_text "$WATERMARK_TEXT" \
    ...
    --arg subtitles_mode "$SUBTITLES_MODE" '
    {
      ...
      subtitles:{
        enabled:true, mode:$subtitles_mode, style_id:"matt-damon-benchmark-v1-poppins",
        style:{font:"Poppins",size:58,color:"#FFFFFF",position:"bottom_center",
          shadow:{color:"#000000",opacity:0.90,blur_px:16,offset_x:0,offset_y:8}}
      },
...
failed=0
# Default is one job at a time: Chronon/Vulkan memory is intentionally bounded
# and the sequential benchmark must never create five simultaneous render
# surfaces. Set CLIPRENDER_PARALLEL=1 to submit all jobs concurrently (mirrors
# a multi-slot worker pool rendering the whole batch at once, e.g. for
# batch-vs-batch throughput comparisons).
if [[ "${CLIPRENDER_PARALLEL:-0}" == "1" ]]; then
  pids=()
  for i in "${!SOURCE_IDS[@]}"; do
    idx=$((i + 1))
    (
      if submit_and_poll "$idx" "${SOURCE_IDS[$i]}"; then
        echo 0 > "$RUN_DIR/tmp/clip-$idx.exit"
      else
        echo 1 > "$RUN_DIR/tmp/clip-$idx.exit"
      fi
    ) &
    pids+=("$!")
  done
  ...
  for ((i=1; i<=EXPECTED_COUNT; i++)); do
    if [[ ! -f "$RUN_DIR/tmp/clip-$i.exit" || "$(cat "$RUN_DIR/tmp/clip-$i.exit")" != "0" ]]; then
      failed=1
    fi
  done
else
