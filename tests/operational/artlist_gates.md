...
> Inherits `smoke_*` from `lib/common.sh`, the canonical SSOT `artlist_drive_resolve / artlist_qdrant_assert / artlist_replay_run` from `lib/artlist.sh`, the typed-sentinel helpers `velox_artlist_search_live / velox_artlist_detail / velox_artlist_download` from `lib/velox_domain.sh` (synthesize the SESSION_EXPIRED / AUTH_REQUIRED / SCRAPER_UNAVAILABLE / SEARCH_TIMEOUT / STREAM_NOT_FOUND typed-sentinel vocabulary used by Gate 10), and the thin-delegator `velox_*` (drivedrive_resolve / drive_resolve / pipeline_run) from `lib/velox_domain.sh` (backward-compat for vidrush_media_e2e.sh — see lib/_artlist_common.sh SSOT guard for the canonical set).
...
| 6 | Drive resolve-by-id hard gate   | hard PASS | **`tests/operational/artlist/06_drive_resolve.sh::gate_drive_resolve`** — pre-flight `[[ -n "${ARTLIST_ROOT_FOLDER:-}" ]]` (fail-closed with handler-typed sentinel ROOT_FOLDER_UNSET otherwise; AGENTS.md "Never represent an unavailable backend as a successful no-op") → consume `${WORK_DIR}/clip_ids.txt` hand-off (Gate 4 wrote it, shared $WORK_DIR across all 5 gates in `run_all.sh`) → for each clip_id: `smoke_sqlite_query $DB_PATH "SELECT drive_file_id FROM media_assets WHERE id='<clip_id>'"` (Gate 5 cross-check that drive_file_id is non-empty) → `artlist_drive_resolve <drive_file_id>` (lib/artlist.sh CANONICAL SSOT via the umbrella's guard list; velox_drive_resolve is now a thin delegator for the 28 backward-compat vidrush_media_e2e.sh callers; rc=0 only on canonical shape contract `.ok AND .resolved_count>=1 AND .resolved[0].trashed==false AND .resolved[0].size>0`; writes body to `${WORK_DIR}/artlist_drive_<id>.json`) → INLINE jq -e parent membership: `.resolved[0].parents // [] | any(. == $ARTLIST_ROOT_FOLDER)` (raw Drive folder ID string-equal; empty parents[] fails closed) → INLINE `curl -sS --max-time 6 -o /dev/null -w '%{http_code}' -I <webViewLink>` link-probe (2xx OR 3xx = PASS — Drive's public-share view often returns 302 → accounts.google.com; 4xx/5xx/timeout=000 = FAIL) → ALL clips must pass (DoD forbids partial-pass — mirrors Gates 4/5/7). Sub-battery file is the canonical owner of Gate 6; the prior `tests/operational/artlist/06_drive.sh` STUB is deleted in the same reorg (no duplicate logic per AGENTS.md Simplicity). MINOR bump in this reorg (3 artlist_* names added to the umbrella guard list, velox_* kept for backward-compat).
| 7 | SQLite + outbox integrity       | hard PASS | **`tests/operational/artlist/07_outbox_integrity.sh::gate_outbox_integrity`** — consumes `${WORK_DIR}/clip_ids.txt` (Gate 4 wrote it; shared $WORK_DIR across all 5 gates in `run_all.sh`) → per clip_id **5 hard invariants** (promote-to-HARD per DoD refactor): **inv-1** `smoke_sqlite_query $DB_PATH "SELECT COUNT(*) FROM media_assets WHERE id='<clip_id>'"` MUST be **exactly 1** (no duplicates); **inv-2** `smoke_sqlite_query $DB_PATH "SELECT file_hash, local_path FROM media_assets WHERE id='<clip_id>'"` → `file_hash` MUST equal `sha256sum local_path | awk '{print $1}'` (one-shot SELECT for both columns, default pipe-delimited split); **inv-3** `smoke_sqlite_query $DB_PATH "SELECT COUNT(*) FROM asset_locations WHERE asset_id='<clip_id>' AND location_kind='local'"` MUST be ≥ 1 (per migration 055_ASSET_LOCATIONS.SQL); **inv-4** same SELECT with `location_kind='drive'` MUST be ≥ 1 (Drive mirror row); **inv-5** `smoke_sqlite_query $DB_PATH "SELECT SUM(CASE WHEN status='completed' THEN 1 ELSE 0 END) AS completed, SUM(CASE WHEN status='superseded' THEN 1 ELSE 0 END) AS superseded, SUM(CASE WHEN status='dead_letter' THEN 1 ELSE 0 END) AS dead_letter, COUNT(*) AS total FROM outbox_events WHERE event_type='asset.index.requested' AND aggregate_id='<clip_id>'"` → jq -e composite: `(completed+superseded) >= 1 AND dead_letter == 0 AND total >= 1` (per-clip inline per AGENTS.md no-duplicate-classification — accepts SUPERSEDED as terminal per migration 092 + outboxevents.MarkSuperseded writer); **post-loop forensic**: `smoke_outbox_chain_verify $DB_PATH $clip_file || true` (helper's rc=1 on SUPERSEDED is stricter than Gate 7's DoD surface; `|| true` makes it diagnostic-only without false-failing); **ALL clips must pass** (DoD forbids partial-pass — mirrors Gates 4/5/6 contract). Sub-battery file is the canonical owner of Gate 7; `tests/operational/artlist/07_index.sh` was stripped of its Gate 7 stub in the same reorg and now owns Gate 8 only. | 7 |
...
| 10 | Negative-path probes (3x HARD)   | hard PASS | **`tests/operational/artlist/10_negative_path.sh::gate_negative_path`** — 3 hard probes per DoD spec (each is independently a HARD gate, ALL must pass — partial-pass forbidden per Gates 4/5/6/7 aggregate contract): **probe-1 SESSION_EXPIRED** — `velox_artlist_search_live` (lib/velox_domain.sh) called through PipelineGen with `SMOKE_TOKEN` forced to an invalid value (`EXP_PROBE_INVALID_<ns>`) so PipelineGen returns HTTP 401 → helper synthesizes `body._transport_kind ∈ {SESSION_EXPIRED, AUTH_REQUIRED}` per the canonical transport-kind classification (curl_rc + http_code → sentinel kind). Invariants: rc=2, body._transport_kind accepted by case-matcher, body.clips=[] (no false results in sentinel body), `SELECT COUNT(*) FROM jobs WHERE status='SUCCEEDED'` BEFORE/AFTER snapshot→ `delta == 0` (no production job accidentally succeeded during the probe — defense against an auth-misroute logic bug); **probe-2 STREAM_NOT_FOUND** — `velox_artlist_detail --phase miss` (lib/velox_domain.sh) with a fabricated non-existent `clip_page_url` (`https://artlist.io/stock-footage/clip/INVALID_NONEXISTENT_FOR_PROBE_<ns>`) → rc=0 (canonical miss-contract pass — NOT rc=1; rc=0 means the contract itself passed for phase=miss) → jq -e composite accepts `.ok == false AND .error == "STREAM_NOT_FOUND" AND clip_id (or .clip.clip_id) non-empty AND stream_urls[] == []`. Pre-flight fail-closed with typed sentinel SCRAPER_URL_UNSET if `${SCRAPER_URL:-}` is empty (matches Gate 0 pre-flight pattern); **probe-3 SCRAPER_UNAVAILABLE** — operator pre-condition: scraper on 9123 MUST be unreachable (asserted via direct curl probe `${SCRAPER_URL:-http://127.0.0.1:9123}/health` returning HTTP '000' = connect-refused; if reachable the probe fails-closed with typed sentinel PROBE_NOT_APPLICABLE per AGENTS.md 'forbidden to represent an unavailable backend as a successful no-op'). On pre-condition pass: `velox_artlist_search_live` through PipelineGen (which proxies to the operationally-dead scraper) → PipelineGen returns 5xx → helper synthesizes `body._transport_kind=SCRAPER_UNAVAILABLE` per the canonical taxonomy + body.clips=[] + (conditional part-B): POST `/api/artlist/run` → smoke_poll_terminal bounded 30s timeout → terminal status ∈ {FAILED, CANCELLED, DEAD_LETTER} (NOT SUCCEEDED — that would be a prod-side false-positive) + jobs.ledger. 0 rows WHERE id=probe_run_id AND status='RETRY_WAIT' (DoD forbids infinite backoff). Reuses ONLY: `velox_artlist_search_live / velox_artlist_detail` (lib/velox_domain.sh typed-sentinel synthesizers) + `smoke_curl / smoke_sqlite_query / smoke_poll_terminal` (lib/common.sh primitives) + `log_*` family (lib/artlist_runtime.sh via umbrella). NO new helpers introduced. The prior `tests/operational/artlist/10_negative_tests.sh` STUB is DELETED in this same reorg (no duplicate logic per AGENTS.md Simplicity, mirrors Gate 6's 06_drive.sh → 06_drive_resolve.sh pattern). PATCH bump only (no new helper added — same PATCH pattern as Gate 6's 1.3.1→1.3.2). | 10 |
...
| qdrant_assert       | `artlist_qdrant_assert` (SSOT in `lib/artlist.sh`); thin delegator `velox_qdrant_assert` retained for 6 backward-compat callers | **No** (reused)         |
| drive_resolve       | `artlist_drive_resolve` (SSOT in `lib/artlist.sh`); thin delegator `velox_drive_resolve` retained for N backward-compat callers | **No** (reused)         |
