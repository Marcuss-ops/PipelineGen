...
> Inherits `smoke_*` from `lib/common.sh`, the canonical SSOT `artlist_drive_resolve / artlist_qdrant_assert / artlist_replay_run` from `lib/artlist.sh`, and the thin-delegator `velox_*` from `lib/velox_domain.sh` (backward-compat for vidrush_media_e2e.sh — see lib/_artlist_common.sh SSOT guard for the canonical set).
