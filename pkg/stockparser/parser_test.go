...
//
// The parser extracts the Title from the line prefix (the text
// before the timestamp range, AFTER the "Round N - " round
// prefix), so the Slug is derived from the Title alone — the
// round prefix is NOT part of the Slug output. This matches
// the pre-existing TestParseTimestampClipSpecs_SingleLineRoundAndRange
...
func TestParseTimestampClipSpecs_SlugRoutesThroughCanonicalHelper(t *testing.T) {
	cases := []struct {
		...
		{"lowercase_round", "Round 7 - Broner barcolla [00:16:33] - [00:17:28]", "broner-barcolla"},
		{"ascii_only", "Round 1 - Test Title [00:00:32] - [00:03:51]", "test-title"},
		{"hyphenated_already", "Round 1 - already-hyphenated [00:00:32] - [00:03:51]", "already-hyphenated"},
		{"digits_in_title", "Round 12 - Top 10 Knockouts [00:00:32] - [00:03:51]", "top-10-knockouts"},
		{"colon_in_title_stripped", "Round 1 - title: with colon [00:00:32] - [00:03:51]", "title-with-colon"},
		...
		{"apostrophe_stripped", "Round 1 - Pacquiao's jab [00:00:32] - [00:03:51]", "pacquiaos-jab"},
		{"punctuation_stripped", "Round 1 - Step 3: \"The Answer\" [00:00:32] - [00:03:51]", "step-3-the-answer"},
		{"mixed_case_lowered", "Round 1 - UPPERCASE Title [00:00:32] - [00:03:51]", "uppercase-title"},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got[0].Slug != tc.wantSlug {
				t.Errorf("Slug: got %q, want %q (PR-SLUG-HELPER-EXTRACT contract: parser-side must byte-equal pkg/slug.SlugifyTitle output for the Title extracted by extractRoundAndTitleBeforeRange, else time-range literal)", got[0].Slug, tc.wantSlug)
			}
		})
	}
}
