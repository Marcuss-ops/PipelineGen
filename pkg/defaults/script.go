...
//
//   - WordsPerMinute = 140
//     (internal/infrastructure/ai/ollama/types/constants.go).
//   - DefaultDuration = 600
...
//     (the formula `(words * 60) / 140`).
//
// Every consumer MUST read from DefaultScriptConfig() rather than
// re-implementing these literals inline. A future "switch the
// default language from Italian to Spanish" or "tune WPM to 150 for
// Italian speakers" change is then a one-line edit; pre-fix it
...
type ScriptConfig struct {
	...
	// to estimate script duration from word count. Legacy inlined
	// value: 140. The SSOT is intentionally a single "base" value
	// shared across locales; per-language overrides are out of
	// scope for this SSOT and belong in a follow-on per-locale
	// override layer when one is implemented.
	WordsPerMinute int
	...
	// DefaultTone is the canonical script tone applied when the
	// caller does not specify one. Legacy inlined value:
