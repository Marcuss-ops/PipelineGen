...
type SwitchReport struct {
	...
	// MissingChannelForMediaType (QDRANT-008 TODO 12, June 2026):
	// per-manifest channel-minimum gate. A scrolled point whose
	// payload lacks the embedding_version_<channel> key for a
	// channel that the manifest REQUIRES for its media_type (e.g.
	// video without embedding_version_transcript) bumps this
	// counter. Non-zero blocks the alias switch via the Ready gate.
	MissingChannelForMediaType int `json:"missing_channel_for_media_type,omitempty"`
	// MissingChannelForMediaTypeDetail (QDRANT-008 TODO 12): per-channel
	// breakdown of which channel name was missing for which points.
	// Key is the channel (e.g. "transcript", "visual", "audio"); value
	// is the count of scrolled points missing that channel for their
	// media_type.
	MissingChannelForMediaTypeDetail map[string]int `json:"missing_channel_for_media_type_detail,omitempty"`
