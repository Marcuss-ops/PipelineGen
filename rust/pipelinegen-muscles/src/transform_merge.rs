...
pub(super) fn execute(request: Request) -> Response {
    ...
    if inputs.is_empty() { return failed_response(None, "input_paths are required".to_string()); }
    ...
    let list_path = std::env::temp_dir().join(format!("pipelinegen_concat_{}.txt", std::process::id()));
    let content = inputs.iter().map(|path| format!("file '{}'", path.replace('\\', "\\\\").replace('\'', "'\\''"))).collect::<Vec<_>>().join("\n");
    if let Err(error) = fs::write(&list_path, content) { return failed_response(Some(inputs[0].to_string()), format!("write concat list: {error}")); }
    let ffmpeg = request.ffmpeg_path.as_deref().unwrap_or("ffmpeg");
    let part = part_path(output);
    let mut command = FFmpegRunner::from_ffmpeg_path(ffmpeg).ffmpeg();
    command.args(["-hide_banner", "-loglevel", "error", "-y", "-f", "concat", "-safe", "0", "-i"]);
    ...
    match result {
        Ok(result) if result.status.success() => match publish_output(&part, output) {
            Ok(()) => Response { ok: true, operation: "merge_inputs".to_string(), source_path: Some(inputs[0].to_string()), items: Vec::new(), metadata: None, error: None },
            Err(error) => failed_response(Some(inputs[0].to_string()), error),
        },
        Ok(result) => { let _ = fs::remove_file(&part); failed_response(Some(inputs[0].to_string()), format!("merge_inputs failed: {}", String::from_utf8_lossy(&result.stderr).trim())) }
        Err(error) => { let _ = fs::remove_file(&part); failed_response(Some(inputs[0].to_string()), format!("merge_inputs failed to start: {error}")) }
    }
}
