...
pub(super) fn concat_source(
    inputs: &[String], output: &str, ffmpeg: &str, suffix: &str,
) -> Result<(String, bool), String> {
    if inputs.len() == 1 { return Ok((inputs[0].clone(), false)); }
    let list_path = format!("{output}.rust.{suffix}.concat.txt");
    let concat_path = format!("{output}.rust.{suffix}.concat.mp4");
    let lines = inputs.iter()
    ...
    match result {
        ...
        Ok(result) => {
            ...
            Err(format!("{suffix} concat failed: {}", String::from_utf8_lossy(&result.stderr).trim()))
        }
        Err(error) => {
            let _ = fs::remove_file(&concat_path);
...
pub(super) fn render_stock_copy_only(
    request: &Request, inputs: &[String], output: &str, ffmpeg: &str,
) -> Response {
    ...
    if !request.keep_audio.unwrap_or(false) { command.arg("-an"); }
    command.args(["-map", "0:v:0", "-c", "copy", "-movflags", "+faststart", &part]);
    ...
    match result {
        ...
        Ok(result) => {
            ...
            failed_response(None, format!("copy-only stock render failed: {}", String::from_utf8_lossy(&result.stderr).trim()))
        }
        Err(error) => {
            let _ = fs::remove_file(&part);
            failed_response(None, format!("copy-only stock render failed to start: {error}"))
        }
    }
}
