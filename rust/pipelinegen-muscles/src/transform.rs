...
fn transform(request: Request, operation: &str) -> Response {
    if operation == "merge_inputs" {
        return merge_inputs(request);
    }
    if operation == "mux_audio_copy" {
        return mux_audio_copy(request);
    }
...
fn mux_audio_copy(request: Request) -> Response {
    let inputs = request.input_paths.as_deref().unwrap_or_default();
    if inputs.len() != 2 {
        return failed_response(
            None,
            "mux_audio_copy requires exactly video and final audio inputs".to_string(),
        );
    }
    ...
    match command.output() {
        Ok(result) if result.status.success() => match publish_output(&part, output) {
            Ok(()) => Response {
                ok: true,
                operation: "mux_audio_copy".to_string(),
        ...
        Err(error) => {
            ...
            failed_response(
                Some(inputs[0].to_string()),
                format!("mux_audio_copy failed to start: {error}"),
            )
        }
    }
}

fn merge_inputs(request: Request) -> Response {
    let inputs = request.input_paths.as_deref().unwrap_or_default();
    let output = match request.output_path.as_deref() {
        Some(path) if !path.is_empty() => path,
    ...
    match result {
        Ok(result) if result.status.success() => match publish_output(&part, output) {
            Ok(()) => Response {
                ok: true,
                operation: operation_name("merge_inputs"),
                source_path: Some(inputs[0].to_string()),
                items: Vec::new(),
                metadata: None,
                error: None,
