...
- **Data-layer unification SSOT (August 2026):** the sync direction is ALWAYS and ONLY `SQLite → Outbox → Qdrant`. Bidirectional sync is forbidden. The canonical write gate for `media_assets` + `asset_locations` + `outbox_events` is `persistence.AssetCommitter` (`SQLiteAssetCommitter`). Every asset commit (YouTube, Artlist, local, voiceover, images, recovery) MUST route through `AssetCommitter.CommitAndIndex` / `CommitTx`. Direct SQL writes to `media_assets` outside the canonical committer are banned and enforced by the `percheck_media_assets_writer_canonical` CI gate. The runtime Qdrant collection is `media_assets` (alias `media_assets_current`); recovery/test/synthetic collections are forbidden at runtime (`schema.IsRuntimeCollection` gate). An empty `media_assets` projection means `INDEX_UNAVAILABLE/REBUILD_REQUIRED`, never a fallback to a recovery collection. The `recover-registry-from-qdrant` tool is EMERGENCY ONLY (disaster recovery / migration forensics) and lives in `cmd/admin/emergency/`.
