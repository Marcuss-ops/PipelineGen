...
- **Media-domain SSOT cutover (September 2026):** PostgreSQL + pgvector is the durable authority for the media domain (`media_assets`, `asset_locations`, `media_asset_features`, `media_embeddings`). The canonical write gate is `persistence.AssetCommitter` — implemented by `PostgresMediaCommitter` (media SSOT mode) and by `SQLiteAssetCommitter` (migration-only, non-media/staged deployments). Every asset commit (YouTube, Artlist, local, voiceover, images, recovery) MUST route through `AssetCommitter.CommitAndIndex` / `CommitTx`; direct SQL writes to `media_assets` outside the canonical committer are banned and enforced by the `percheck_media_assets_writer_canonical` CI gate. Vector search on the media plane is owned by pgvector (`internal/platform/postgres/media.MediaSearcher` implements the canonical `search.VectorStorePort`); **Qdrant media reads and writes are forbidden during and after cutover** — the composition root resolves the pgvector plane whenever `PIPELINEGEN_MEDIA_POSTGRES_ENABLED=true`, fail-closed (no silent downgrade). The media index-request consumer is `pgmedia.PostgresIndexWorker` (embed → pgvector upsert → INDEXED → outbox completed, all in the media SSOT); in PG mode NO Qdrant media indexing handler is registered (`QDRANT_MEDIA_WRITES=0`, `QDRANT_MEDIA_READS=0`). The generic outbox stays for Drive/webhook/async side effects only. Derived surfaces (`media_asset_features`, `media_embeddings`) are written only by the enrichment pipeline through `pgmedia.VectorSurfaceWriter`, and embedding families are fail-closed gated by the `media_embedding_families` registry + DB trigger. **Demolition status:** `SelectMediaAssetCommitter` (caller-supplied adapter-pair decision) is REMOVED — the single engine decision point is `newCanonicalAssetCommitterCfg` / `canonicalCommitterForRoot`. Qdrant itself is NOT globally removed: mediamemory, maintenance DR, and admin audit tools still legitimately use it outside the media domain. The SQLite media committer family remains as the staged-migration adapter and is demolition-debt pending non-media SQLite retirement. Certification: `make certify-media-cutover` (16 gates, prints `POSTGRES_MEDIA_SSOT=TRUE` only when all are green).
