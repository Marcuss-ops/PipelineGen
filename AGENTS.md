...
- **[VO-DECOMPOSITION-2026-07-04 — Voiceover subsystem decomposition wave-tracker entry (8 net-new linked_issues, July 2026)]** `chore(architecture) + docs(plan)` — mirror entry for the Italian Phase 1 (read-only) audit of the voiceover subsystem pasted to the orchestrator on 2026-07-04. Per the user-pasted audit: the voiceover subsystem is **2 services + 1 use case + 1 fanout use case + 1 parent aggregator** (already well-divided per AGENTS.md Pattern 0/5); the audit identified 6 high-priority decomposition targets (3 split-per-stage + 1 TTS persistent worker + 1 DRY pair + 1 parent state machine) + 1 typed-primitive quick win. **Priority bands (3):** Band 1 P0 absolute (6 file targets: `internal/infrastructure/audio/processor.go` + `stages.go` 605 LoC + `finalizer.go` 538 LoC + `parent_aggregator.go` 469 LoC + DRY pair `usecase.go` 523 ↔ `process_voiceover_item.go` 380); Band 2 P1.2 structural (parent_aggregator_state typed column migration, already forward-pointed in `generate_handler.go:267`); Band 3 P1.1 quick win (TextHash + Language + StyleGroup typed primitives, single PR per half-migration risk mitigation per audit §2.4 #3). **8 net-new slim-shape `linked_issues`** filed in `architecture/current.yaml#VO-DECOMPOSITION-2026-07-04.linked_issues` (5 P0 + 1 P1.2 + 1 P1.1 + 1 hotspot cross-ref `VO-DECOMPOSITION-HOTSPOT-CROSSREF` deadline 2026-08-15 mirrors GODOBJ-2026-07-03 `PR-GODOBJ-HOTSPOT-CROSSREF` pattern). **Per-band deadlines:** P0 #1 TTS worker 2026-08-15 (cross-language wire, longest lead time); P0 splits 2026-08-01 (mechanical, mirrors PR-CHROME-PROVIDER-SPLIT cadence); P0 #5 DRY pair 2026-08-15 (semantic refactor); P1.2 state column 2026-08-01; P1.1 typed-primitive 2026-07-25. **Execution order** (per the audit's "ordine consigliato"): P1.1 typed-primitive quick win first (unblocks P0 #5 DRY pair typed-error work); then 4 P0 splits (stages.go + finalizer.go + parent_aggregator.go + DRY pair, all sharing compile-time lock on `stages.go` + `finalizer.go` because `process_voiceover_item.go` reads both); then P0 #1 TTS persistent worker (cross-language wire: Go subprocess + Python sidecar HTTP server mirroring `internal/application/images/slide_worker_process.go::ensureStarted` precedent from PR-CHROME-PROVIDER-SPLIT 2026-07-04); P1.2 state column runs in parallel after P0 #4 split. **godlike/06 SSOT (one canonical owner per fact):** the 6 P0 items map 1:1 onto 4 owner capabilities (`internal/infrastructure/audio` for P0 #1; `internal/application/voiceover` for P0 #2, P0 #3, P0 #5; `internal/application/voiceover/jobs` for P0 #4 + P1.2; `internal/application/voiceover` for P1.1). No cross-package fan-out. **godlike/07 carry-forward preservation (honest-limitation):** audit-pins at `usecase.go:478,511` + `stages.go:594-595` + `ports.go:158` (godlike/06/07 audit-pin discipline — toccare = rompere la chain di audit); `useCaseTTSAdapter` panic-on-nil at `internal/app/adapters_voiceover_use_case.go:82` (fail-closed at startup, P0 #4 contract — crasha l'app se mal configurato, comportamento voluto); Replace-Mode cleanup triplo in `finalizer.go:88-95` (`oldDriveFileID` + `oldLocalPath` + `oldCleanedPath` — intricato ma richiesto da P0.7 Step 10/12 per guarantire atomic swap-and-cleanup); Tick pattern in `parent_aggregator.go` (polling intenzionale vs signal-driven, Phase 1 wiring). **CI gate Check 58** (forward-prevention, lands with P0 #1): bans `exec.CommandContext("python3", ...)` callers in `internal/infrastructure/audio/` outside the canonical persistent-worker surface (mirrors Check 54's posture for the `monitor/` infra-import ban). **Pre-existing 5-item build-issue carry-forward** unchanged: `monitor/enqueue.go` `strings.ToLower` undefined + `monitor/scheduler.go` `NewUnboundJobEnqueuer` undefined + `internal/application/assets/providers/stock/stockpipeline/run_upload.go` syntax error + `internal/app/module_media.go` pre-existing `clips.Deps.MutationsDispatcher` literal + `internal/application/images/routing` import cycle (out of scope per CHANGELOG forward-pointer convention). **No migration / no gofmt touch / no test churn** in this commit: documentation-only across 3 surfaces (CHANGELOG `## Unreleased → ### Added` + AGENTS.md this mirror entry + `architecture/current.yaml` wave-tracker entry + `architecture/action-plans/2026-07-04-voiceover-decomposition.md` narrative companion). Per AGENTS.md Git-Lesson-2: each per-item PR lands **directly on `main`** per-file (auto-sufficient granularity); Co-authored-by trailer per Git-Lesson-3. **Cross-reference:** `architecture/current.yaml#VO-DECOMPOSITION-2026-07-04` (wave-tracker anchor + 8 net-new `linked_issues` + exit_gate) + `architecture/action-plans/2026-07-04-voiceover-decomposition.md` (narrative + kill-candidate matrix + expected split topologies + godlike/07 honest limitations) + CHANGELOG.md `## Unreleased → ### Added` mirror entry.
...
- **[PR-QDRANT-CONFIG-MISMATCH-GATE closure (QDRANT-CHAIN-VERIFY-2026-07-04 P0 fail-closed gate, July 2026)]** `feat(app) + test(app) + chore(arch)` — register the canonical composition-root fail-closed gate that closes the RED POINT surfaced by the QDRANT-CHAIN-VERIFY-2026-07-04 audit (when `qdrant.enabled=true` but `clipindexer.enabled=false`, outbox marks `asset.index.requested` as COMPLETED without writing to Qdrant — IndexClip short-circuits to nil, false-success path). NEW helper `validateQdrantIndexerCompatibility(cfg *config.Config) error` at `internal/app/build_bundles_qdrant_gates.go` (~170 LoC) — mirror of ART-002 P0.1 `validateArtlistScraperURL` pattern (godlike/06 SSOT, godlike/07 no-fake-availability). **godlike/06 SSOT one-canonical-owner-per-fact:** the helper is the SOLE canonical owner of the Qdrant+ClipIndexer compatibility check; wired at 4 defence-in-depth composition sites via exactly one call line each — (1) `internal/app/build_process_qdrant.go::buildQdrantDeps` (retired the pre-existing BLOCKER #3 inline fmt.Errorf check, replaced with helper call); (2) `internal/app/build_bundles_process.go::BuildOutboxBundle` (top-of-fn gate before outbox handler registry); (3) `internal/app/wire_services.go::WireServices` (before EnsureSchema startup step, with `coreClean()` rollback on fail); (4) `internal/app/build_bundles_core.go::buildHealthService` (consistency-check, error-discarded since the fn does not return error — defensive belt-and-suspenders against partial-composition rollback). 4 TDD tests at `internal/app/build_bundles_qdrant_gates_test.go` (mirrors `build_bundles_artlist_test.go` shape): `TestValidateQdrantIndexerCompatibility_NilCfg_ReturnsError` + `_BothDisabled_ReturnsNil` + `_BothEnabled_ReturnsNil` + `_QdrantEnabledNoClipIndexer_FailsClosed` (the canonical 5-substring envelope: "QdrantEnabled=true" + "ClipIndexerEnabled=false" + "QDRANT-CHAIN-VERIFY-2026-07-04 P0" + "VELOX_FEATURE_CLIP_INDEXER_ENABLED=true" + "VELOX_FEATURE_QDRANT_ENABLED=false"). **godlike/07 no-fake-availability:** fail-closed on BOTH directions of Qdrant+ClipIndexer mismatch (Direction A: ClipIndexer=true+Qdrant=false — the pre-PR inline check; Direction B: Qdrant=true+ClipIndexer=false — the CRITICAL RED POINT). 2 canary tests updated in `internal/app/composition_test.go`: (a) `TestComposition_QdrantEnabledNoClipIndexer_WriterAndDeleterWired` flipped from pin-OLD-false-success-OK to pin-NEW-fail-closed-Error per godlike/07 no-fake-availability (the OLD test was the bug, not just an outdated canary); (b) `TestComposition_ClipIndexerEnabledNoQdrant_FailClosed` substring assertions updated to canonical camelCase format that matches the helper's error envelope. **Verification:** `gofmt` clean on all 7 surfaces; 4 new TDD tests + 2 updated canary tests pass; `processor.NewProcessor: publisher is required` panic reproduces on `origin/main` HEAD (stash + reset-to-origin + retest verified) — NOT a regression of this PR per `architecture/current.yaml#PRE-EXISTING-BUILD-ISSUES-2026-07-04` carry-forward discipline. **Cross-ref lockstep (`godlike/06` 3-surface):** `architecture/current.yaml#PR-QDRANT-CONFIG-MISMATCH-GATE status: pending → shipped` + `CHANGELOG.md ## Unreleased → ### Fixed → PR-QDRANT-CONFIG-MISMATCH-GATE closure` + `AGENTS.md §Recent cross-cutting closures` (this entry). **Honest scope-lock (godlike/07):** `buildHealthService` wire site uses error-discard (`_ = validateQdrantIndexerCompatibility(cfg)`) since the function does not return error; the 3 upstream wire sites already enforce fail-closed at composition (this 4th site is purely defensive — unreachable in the success path). **Wave-tracker cross-reference:** `architecture/current.yaml#QDRANT-CHAIN-VERIFY-2026-07-04.linked_issues[PR-QDRANT-CONFIG-MISMATCH-GATE]` flipped per godlike/06 SSOT lockstep. Co-authored-by: PipelineGen Agent <agent@pipelinegen.local>. AGENTS.md Git-Lesson-3.
