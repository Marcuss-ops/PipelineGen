...
~5–15 min** depending on the `./tests/...` size at HEAD.
...
### Shared dependencies (all 4 batteries)

Every battery requires:

- **Chrome** — for live HTTP probes with cookie-managed sessions
  (the Artlist scraper hands off through Chrome; image battery uses
  headless Chrome for content sniffing).
- **scraper** — running + reachable for all `/api/artlist/`
  endpoints. Per `tests/operational/lib/artlist.sh` startup probe.
- **Drive** — the canonical Google Drive surface, gated upstream by
  `make auth-check`.
- **Qdrant** — v3 collection reachable + `QDRANT_API_KEY` exported
  (or sourced from `scripts/with-velox-auth`-managed env).

Each battery is also gated by `make auth-check` (operator pre-flight
against `/api/artlist/job-consumer`) and routes through
...
| | |
|---|---|
| **Script** | `tests/operational/images_e2e.sh` |
...
| **Surface** | `script.generate` dispatch + worker pull + finalizer **WITHOUT** the full Vid Rush media path (no Chrome hit, no FFmpeg) |
...
- The canonical loader is `scripts/with-velox-auth` — locate with
  `ls -la scripts/with-velox-auth` and confirm it is present and valid.
  If missing at HEAD, the live tier is structurally broken — restore
  from the latest commit that added it (`grep` the git log) before
  invoking any `*-live` target.
...
- `scripts/with-velox-auth` — canonical token loader (mode 0644 with
  shebang `#!/usr/bin/env bash`; invocation per AGENTS.md Authentication SSOT).
