...
| `SCRAPER_CONNECT_TIMEOUT_SECONDS` | required  | `5` (in-script per bash wrapper; doc-public default per §11.0; operator override honoured) | Scraper's TCP connect (Chromium launch + first page navigation) is enforced via `curl --connect-timeout` (a separate flag from `--max-time`); the connect budget guarantees a fail-loud on a dead socket without waiting for the full SCROLL_TIMEOUT. The Node `handleSearch` logs `BUDGET connect=N total=M` per request so operators can correlate fail-loud timings against the budget. | `tests/operational/artlist_live_e2e_verify.sh:165` (snippet); every curl invocation across the 5 wrapper bash scripts; `.env.example`; `docker-compose.yml` `artlist-scraper.environment`; `node-scraper/artlist_server.js::handleSearch` log line |
