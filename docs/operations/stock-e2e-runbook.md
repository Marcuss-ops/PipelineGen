...
| Job terminal status: `FAILED stock.stage_sources` | `PR-STOCK-STAGER-WIRE` | `internal/capabilities/assets/providers/stock/stockpipeline/stager_adapter.go` (or canonical SourceStager) | Verify `SourceStager` adapter is bound; composition root must inject the concrete adapter |
...
| `PR-STOCK-STAGER-WIRE` | `internal/capabilities/assets/providers/stock/stockpipeline/stager_adapter.go` | forward-pointer |
...
| `status=WAITING_CHILDREN` / `FINALIZING` (non-terminal, parent aggregation)     | Parent aggregation in flight                                                  | Observe the parent's aggregator per `internal/capabilities/scripts/jobs/parent_aggregator.go`. Do NOT cancel. |
...
|------------------------------------------------------------------------------------------|----------------------------------|------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------|
| `error="no candidates found"` (or message contains `no candidates found`)                 | `PR-ARTLIST-NO-CANDIDATES`       | `internal/capabilities/assets/providers/artlist/run_orchestrator_stages.go::stageDiscoverClips` (line 52: `resp.Error = "no candidates found"`) | Inspect Artlist searcher chain (`internal/capabilities/assets/providers/artlist/search_core.go::buildSearcherChain`); confirm the search term yields ≥1 result; verify the live scraper (`node-scraper/artlist_search.js`) `SCROLL_TIMEOUT ≥ 120` per the post-rotation timeouts. |
...
- Artlist forward-pointers → `internal/capabilities/assets/providers/artlist/{run_orchestrator_stages.go, job_core.go, run_service.go, search_core.go}` per row in §11.5.
...
- **Canonical measurement pin (receipt)**: `internal/capabilities/assets/providers/stock/stockpipeline/step_extract_clips_test.go::TestStockExtractClips_ThirtyClips_SingleCutRequest`
- **Canonical production code (post-fix fold)**: `internal/capabilities/assets/providers/stock/stockpipeline/step_extract_clips.go::StockExtractClipsStep.Run` (1 `CutRequest` per SourceID group)
...
- **Source-probe skip (per-clip -> group)**: `internal/capabilities/assets/providers/stock/stockpipeline/step_extract_clips_validation.go::validateAndProbeSourceDuration` populates `CutRequest.SourceDuration` so the per-source ffprobe probe at `cutter.Cut` start collapses to 0 (validated by `TestFFmpegCutter_SourceDurationSkipsProbe`).
