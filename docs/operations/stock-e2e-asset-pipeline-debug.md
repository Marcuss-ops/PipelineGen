...
| Job / broker Enqueue (hang at submit) | STEP 3 attempt-N HTTP 000 / "no job_id" — handler accepted the request but never returned a job_id | `internal/platform/sqlite/jobs/repository.go::Create` (+ `internal/capabilities/jobs/enqueue_service.go`) | `PR-STOCK-COMPOSITION-WIRE` (envelope) |
...
| Outbox / Qdrant index | STEP 11 ≥1 hit expected but empty | `internal/capabilities/jobs/delivery.go` | `PR-STOCK-OUTBOX-QDRANT-INDEX` |
