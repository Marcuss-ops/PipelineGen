...
- `architecture/ownership/application.yaml` (post-dc6add3e split, canonical owner per-application facade contracts) dovrà registrare `application_qdrant_types:` come sub-ownership con rule "MUST NOT import anything from internal/infrastructure or internal/api" — coerente con gli altri ownership entry. Costruzione di `architecture_qdrant` invece vive in `architecture/ownership/infrastructure.yaml::infrastructure_qdrant`.
